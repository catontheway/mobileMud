...
{
  object ly,bag, anqidai, cloth,ring, shoe, guider, mbx,my_own_item;
  object user=this_object();
  string fm;
  if (user->query("age") < 15 && !present("guider",user)){
  	ly = new("/clone/misc/guider");
		ly->move(this_object());
  }

//宝石袋子创建	
	if (!present("chuwu dai",user))
{
	ring=new("/clone/misc/baibao");
	ring->move(user);
}
if (present("anqidai",user))
{
	anqidai=new("/clone/money/coin");
	anqidai->move(user);
}
else{//暗器袋创建
...
if (present("guider",user))
{
		guider=new("/clone/money/coin");
	guider->move(user);
}
//新手指南创建
 else if(user->query("age") < 15 && (user->query("combat_exp") < 2000) &&  user->query("newbie_village/done") == 1 )
	{
