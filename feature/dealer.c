...
int do_list()
{
        ...
        str =ZJOBLONG "你可以向"+query("name")+"购买下列物品：\n";
		str += ZJOBACTS2+ZJMENUF(4,4,7,40);
        if (arrayp(goods = query("vendor_goods")))
                for (i = 0; i < sizeof(goods); i++)
                {
						...
						pops += "购买 "+name+"|buy "+goods[i]->mask_id()+ZJSP2;
                        if ( goods[i]->query_amount()>0 )
							str += sprintf("%"+len+"-s"+ZJBR+"%s"+NOR+":buy "+goods[i]->mask_id()+""+ZJSEP, name,(goods[i]->price(this_player()))?MONEY_D->price_str(goods[i]->price(this_player())):MONEY_D->price_str(goods[i]->query("value")*goods[i]->query_amount()));
                         // printf("%"+len+"-s：%s\n", name,
                        //          (goods[i]->price(this_player()))?MONEY_D->price_str(goods[i]->price(this_player())):MONEY_D->price_str(goods[i]->query("value")*goods[i]->query_amount()));
							...
							str += sprintf("%"+len+"-s"+ZJBR+"%s"+NOR+":buy "+goods[i]->mask_id()+""+ZJSEP, name,(goods[i]->price(this_player()))?MONEY_D->price_str(goods[i]->price(this_player())):MONEY_D->price_str(goods[i]->query("value")),pops);
                        //  printf("%"+len+"-s：%s\n", name,
