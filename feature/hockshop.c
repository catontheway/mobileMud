...
string use_help()
{
   string str;
	...
	return "公平交易\n"ZJOBACTS+ZJMENUF(1,4,10,32)+"当卖东西:pawn"ZJSEP+"买东西:list\n";
...
int do_pawn(string arg)
{
    ...
    string str,msg,term, *terms,pops,popa;
    totalnum=0;
...
//      return notify_fail("指令格式：pawn|dang [<物品ID>]\n");
    {
        ...
        if (!sizeof(inv))
		{
			write(ZJOBLONG"你身上什么都没有，哪来东西典当或者赎回？\n");
			return 1;
		}
      ...
      if( !mapp(pawn) || !sizeof(pawn) )
      {
        ...
        write(ZJOBLONG "你并没有典当任何物品,你一共可以典当"+pawn_num+"样"+2*pawn_num+"件物品。\n");
        msg=ZJOBACTS+ZJMENUF(4,4,10,32);
        
      for (i = 0; i < sizeof(inv); i++)
         {
           
          pops = ZJPOPMENU+ZJMENUF(1,3,9,30);
          pops += "典当 "+inv[i]->query("name")+"|pawn "+inv[i]->query("id")+ZJSP2;
           pops += "估价 "+inv[i]->query("name")+"|value "+inv[i]->query("id")+ZJSP2;
           pops += "卖断 "+inv[i]->query("name")+"|sell "+inv[i]->query("id")+ZJSP2;
        msg += sprintf(inv[i]->query("name")+ZJBR+inv[i]->query("id")+NOR+":"+pops+ ZJSEP);
       }
      ...
      { 
        ...
        // str += ZJOBACTS+ZJMENUF(1,2,7,40);
         str = ZJOBLONG"你目前典当的物品有：  你一共可以典当"+pawn_num+"样"+2*pawn_num+"件物品:\n"+ZJOBACTS2+ZJMENUF(2,2,7,40);
          msg=ZJOBACTS+ZJMENUF(1,4,10,32);
      ...
      for (i = 0; i < sizeof(inv); i++)
        ...
        for (i=0 ;i<sizeof(terms) ;i++ )
        {
          popa = ZJPOPMENU+ZJMENUF(1,3,9,30);
          popa += "赎回 "+pawn[terms[i]]+"|shu "+terms[i]+ZJSP2;
          popa += "放弃赎回 "+pawn[terms[i]]+"|discard "+terms[i]+ZJSP2;
           str += sprintf(terms[i]+"："+pawn[terms[i]]+ZJBR+"共有"+npawn[terms[i]]+"件"+ZJBR+" 赎银："+value_string(vpawn[terms[i]]* 4/5)+NOR+":"+popa+ ZJSEP);
         //  msg += inv[i]->query("name")+ZJBR+inv[i]->query("id")+popb+ ZJSEP;
...
int do_list(string arg)
{
   ...
   string tlist,pops;
    ...
    //list = ZJOBLONG "你可以购买下列这些东西：\n";
    
  //write(list);
   // list = "";
    list = ZJOBLONG "你可以购买下列这些东西：\n"+ZJOBACTS2+ZJMENUF(4,4,6,45);
   ...
   //  list += ZJOBACTS2+ZJMENUF(4,4,7,40);
      if(!goods[name[i]]) 
      {
      ...
      {
        pops = ZJPOPMENU+ZJMENUF(1,3,9,30);
          pops += "购买 "+ob->query("name")+"|buy "+ob->query("name")+ZJSP2;
        //list += ZJOBACTS2+ZJMENUF(2,2,7,40);
      //tlist = get_name(name[i]);
        tlist = name[i]+NOR;
        colorlen = color_len(tlist);
        indent = sprintf("%%-%ds",colorlen+30);
        if ( goods[name[i]]->query_amount()>0 )
          list += sprintf(indent+ZJBR+"%s"+ZJBR+"还剩%s%s"+NOR+":buy "+ob->query("name")+ZJSEP, tlist,
