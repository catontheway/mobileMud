...
// 添加对临时技能的支持。by lordstar 20180514
...
varargs int query_temp_skill(string skill);
...
varargs void map_skill(string skill, string mapped_to)
{
        ...
        if( !mapped_to && mapp(skill_map) ) {
                ...
                if ( stringp(map_to) &&
                     mapp(bufmap=SKILL_D(map_to)->buffer_removed(ob, skill)) )
       ...
       && (!find_object(PLAYER_SKILL_D(skill)) && file_size(PLAYER_SKILL_D(skill)+".c") < 0 )//自创武功
        ...
        if (( !mapp(skills) || undefinedp(skills[mapped_to]) ) && query_temp_skill(mapped_to) <= 0 )
                return;
        ...
        if ( stringp(mapped_to) &&
             mapp(bufmap=SKILL_D(mapped_to)->buffer_applied(ob, skill)) )
          apply_buffer(ob, bufmap);
        //切换内功后恢复历史数据
        if ( userp(ob) && 
             skill=="force" && 
...
//仅支持查询根技能的等级
//在增加临时技能之后，原来的 query_skill() 改名为 query_real_skill()
varargs int query_real_skill(string skill, mixed raw)
{
  int s, level, lvc, lp;
  string esk;
...
//添加临时技能
// time 有效时间；pos 有效地点；del 离开有效地点之后删除该技能或继续保留（但不生效）
varargs int set_temp_skill(string skill, int level, int time, string pos, int del)
{
	set_temp(TEMPSKILL+"/"+skill+"/level", level);
	if (time>0)
		set_temp(TEMPSKILL+"/"+skill+"/available_before", time()+time);
	else
		delete_temp(TEMPSKILL+"/"+skill+"/available_before");
	if (strwidth(pos)>0)
		set_temp(TEMPSKILL+"/"+skill+"/available_position", pos);
	else
		delete_temp(TEMPSKILL+"/"+skill+"/available_position");
	if (del>0)
		set_temp(TEMPSKILL+"/"+skill+"/delete_at_outside", 1);
	else
		delete_temp(TEMPSKILL+"/"+skill+"/delete_at_outside");
}

//删除临时技能
varargs int delete_temp_skill(string skill)
{
	mapping skp, skill_map;
	string fneili, *sname;
	int i;
	delete_temp(TEMPSKILL+"/"+skill);
	if (query_real_skill(skill)) return 1;
	skp = query_skill_prepare();
	if (mapp(skp))
	{
		sname=keys(skp);
		for (i=0; i<sizeof(sname);i++)
		{
			if (sname[i] == skill || skp[sname[i]] == skill )
				prepare_skill(sname[i]);
		}
	}
	skp = query_wprepare();
	if (mapp(skp))
	{
		...
		{
			if (sname[i] == skill || skp[sname[i]] == skill )
				prepare_wskill(sname[i]);
		}
	}
	skill_map = query_skill_map();
	if ( mapp(skill_map) )
	{
		sname=keys(skill_map);
		for(i=0;i<sizeof(sname);i++)
		{
			if (sname[i] == skill || skill_map[sname[i]] == skill )
				map_skill(sname[i]);
		}
	}
	if (SKILL_D(skill)->valid_enable("force"))
	{
		if ( !stringp(fneili=SKILL_D(skill)->translate(skill)) )
			fneili=skill;        
		if ( fneili==skill && query("backup_neili/"+fneili+"/max_neili")>0 )
			delete("backup_neili/"+fneili);
		if ( fneili!=skill &&
				query("backup_neili/"+fneili+"/max_neili")>0 &&
				query("backup_neili/"+fneili+"/force")==skill )
			delete("backup_neili/"+fneili);
	}	
	tell_object(this_object(), HIM"你的临时技能「"+to_chinese(skill)+HIM"」失效了！\n"NOR);
	return 1;
}
...
varargs int query_temp_skill(string skill)
{
	...
	int len;
	string pos, env;
	sk = query_temp(TEMPSKILL+"/"+skill);
	if (!mapp(sk)) return 0;
	if (!undefinedp(sk["available_before"]) && sk["available_before"]<time())
	{
		delete_temp_skill(skill);
		return 0;
	}	
	if (!undefinedp(sk["available_position"]) && strwidth(sk["available_position"]) > 0)
	{
		pos = sk["available_position"];
		len = strwidth(pos);
		env = base_name(environment(this_object()));
		if (strwidth(env)>= len && env[0..len-1] == pos)
		{
			delete_temp(TEMPSKILL+"/"+skill+"/pna");
			return sk["level"];
		}
		else 
		{
			if (sk["delete_at_outside"]) 
				delete_temp_skill(skill);
			else
				set_temp(TEMPSKILL+"/"+skill+"/pna", 1);
			return 0;
		}
	}
	return sk["level"];
...
// 增加临时技能支持
  ...
  {
	ts = query_temp_skill(skill);
    if ( mapp(skills[skill]) )
      return ts + skills[skill]["level"];
    else
      return ts + skills[skill];
  }
  return query_temp_skill(skill);
}
...
varargs void improve_skill(string skill, int amount, int weak_mode, int type)
{
    ...
    if( (!weak_mode || !userp(this_object())) )
    {
        ...
        learned[skill]<0 )
      {
        ...
        if ( skills[skill]<=0 )
        {
          SKILL_D(skill)->skill_deleted(this_object());
          map_delete(skills, skill);
          map_delete(learned, skill);
