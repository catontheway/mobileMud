...
int main(object me, string arg)
{
	...
	if ((!(fam = me->query("family")) || fam["family_name"] != "大轮寺"))
		return notify_fail("只有大轮寺弟子才能行使慑心术！\n");
	if (me->query_skill("huanxi-chan",1) < 40)
		return notify_fail("你的欢喜禅还太浅薄了，没法行使慑心术。\n");
...
//	if (!ob = get_object( dest ) )
  ...
  */
	if( ob->query("no_get") || ob->query("no_get_from") )
		return notify_fail("你对"+ob->name() + "行使慑心术可不成。\n");
	if( wizardp(ob) ) return notify_fail("对巫师慑心，你肯定是脑袋发昏了。\n");
...
不由自主的凝视着他。\n", me, ob);
	if( random(me->query_skill("huanxi-chan",1)) < random(ob->query_skill("force",1)))
	{
