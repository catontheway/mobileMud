...
void realtime_map(object me,object env)
{
    ...
    mapping alldirs=([      
        ...
        //add for save time
	if (me->query("combat_exp")>1&&me->query("antirobot/deactivity")>=2)
...
string inventory_look(object obj, int flag)
{
   ...
   /* if( obj->query("equipped") )
	{
	if (obj->query("env/zbxianshi") == 3)
                {
					str = HIC "  □" NOR + do_query(obj)+"\n"+HIR ""ZJURL("cmds:set zbxianshi 2")ZJSIZE(30)"关闭装备显示"NOR" \n";
            
					//str += "\t\t" + HIR ""ZJURL("cmds:set zbxianshi 2")ZJSIZE(30)"关闭装备显示"NOR" \n";
    }
	else if (obj->query("env/zbxianshi") == 2)
                {
                str = HIC ""ZJURL("cmds:set zbxianshi 3")ZJSIZE(30)"打开装备显示"NOR" \n";
                }
   ...
   /* if( sizeof(inv) ) {
        ...
        if( sizeof(inv) )
		{
			str += sprintf( obj->is_corpse() ? "%s的遗物有：\n%s\n" : "%s身上带著：\n%s\n"+ HIR ""ZJURL("cmds:set zbxianshi 2")ZJSIZE(30)"关闭装备显示"NOR" \n";,
                    pro, implode(inv, "\n") );
		if (obj->query("env/zbxianshi")== 2)
                {
            
					str += "\t\t" + HIR ""ZJURL("cmds:set zbxianshi 2")ZJSIZE(30)"关闭装备显示"NOR" \n";
    }
	else if (obj->query("env/zbxianshi")== 3)
                {
                str += HIC ""ZJURL("cmds:set zbxianshi 3")ZJSIZE(30)"打开装备显示"NOR" \n";
                }
		}
...
{		
	...
	for(int i=0;i<sizeof(des);i++)
	{
		if(is_chinese(des[i]))
		{
			if(member_array(des[i..i],biaodian)==-1)
			{
...
int mudlook_room(object me, object env,int busy)
{
    ...
    str = sprintf( ZJTITLE"%s\n"ZJLONG"%s%s%s\n",
            env->query("short"),
           // wizardp(me)? file_name(env): "",
            replace_string(env->query_temp("mixed_long"),"\n",""),
    ...
    if (mapp(exits = env->query("exits")))
	{
		...
		if (wizardp(me))
		{
			str += "update here:"HIW"刷新"NOR+ZJSEP;
		}
		 if (me->query_temp("chaojing/can_write"))
		 {
			str += "write jing:"HIW"抄经"NOR+ZJSEP;
		}
		if(area->query("dig_money"))
		{
			str += "digmoney where:"HIG"地点"NOR+ZJSEP;
			str += "digmoney:"HIG"挖宝"NOR+ZJSEP;
		}
		if (me->query("age") < 16 && !wizardp(me))
        {
  	
    if (present("lu yin",me))
		{
			str += "travel:"HIM"叫马"NOR+ZJSEP;
		}
         }
        if (present("chuwu dai",me))
		{
			str += "jiaoche:"HIM"叫车"NOR+ZJSEP;
		}
		if (present("zhu lou",me))
		{
			str += "catch:"HIM"捕蛇"NOR+ZJSEP;
			str += "free all:"HIR"放蛇"NOR+ZJSEP;
		}
		if (present("yao fang",me))
		{
			str += "peiyao:"HIM"配药"NOR+ZJSEP;
		
		}
		if (present("tie qiao",me))
		{
			str += "dig:"HIM"挖工事"NOR+ZJSEP;
		
		}
		if (present("zhu zi",me))
		{
			str += "xiao zhu zi:"HIM"削竹子"NOR+ZJSEP;
		
		}
		if (present("yuan mu",me))
		{
			str += "xiao:"HIM"削圆木"NOR+ZJSEP;
		
		}
		if (present("da futou",me))
		{
			str += "pi:"HIM"砍树"NOR+ZJSEP;
		
		}
			if (present("tiechan",me))
		{
			str += "dig:"HIM"挖宝"NOR+ZJSEP;
		
		}
			if (present("sao ba",me))
		{
			str += "dasao:"HIM"打扫"NOR+ZJSEP;
		
		}
		if (present("kan dao",me))
		{
			str += "xiao:"HIM"削竹子"NOR+ZJSEP;
		
		}
				if (present("xiao chanzi",me))
		{
			str += "chan:"HIM"挖"NOR+ZJSEP;
		
		}
					if (present("xiaoqiu",me))
		{
			str += "guan:"HIM"制霹雳弹"NOR+ZJSEP;
		
		}
		if (present("fengjiang ping",me))
		{
			str += "qufeng:"HIM"驱蜂"NOR+ZJSEP;
		
		}
		if (present("shui piao",me))
		{
			str += "yao 雪:"HIM"舀雪"NOR+ZJSEP;
		
		}
				if (present("shui piao",me))
		{
			str += "chaxun xin wu:"HIM"信物"NOR+ZJSEP;
		
		}
		if (present("li hua",me))
		{
        str += "dian li hua:"HIM"燃放礼花"NOR+ZJSEP;
		}
				if (present("fire",me))
		{
        str += "hua huozhezi:"HIM"火折子"NOR+ZJSEP;
		}
		if (present("qin",me))
		{
        str += "play yanyu qin:"HIM"弹琴"NOR+ZJSEP;
		}
		if (present("mutong",me))
		{
        str += "pi:"HIM"劈开木桶"NOR+ZJSEP;
		}
		if (present("chai dao",me))
		{
        str += "pi:"HIM"劈柴"NOR+ZJSEP;
		}
		if (present("chai huo",me))
		{
        str += "jiachai:"HIM"加柴"NOR+ZJSEP;
		}
	
		if (present("pu shan",me))
		{
        str += "shanfeng:"HIM"扇风"NOR+ZJSEP;
		}
		if (present("liquan ling",me))
		{
        str += "remake:"HIM"沥泉灵"NOR+ZJSEP;
		}
		if (present("wa guan",me))
		{
        str += "fill wa guan:"HIM"装水"NOR+ZJSEP;
		 str += "dao wa guan:"HIM"倒水"NOR+ZJSEP;
		}
		if (present("beisu",me))
		{
        str += "use beisu:"HIM"用悲酥清风"NOR+ZJSEP;
		 str += "feng beisu:"HIM"盖上盖子"NOR+ZJSEP;
		}
		if (present("zhiqian",me))
		{
			str += "shao fire:"HIM"点纸钱"NOR+ZJSEP;
			str += "sa zhiqian:"HIM"撒纸钱"NOR+ZJSEP;
		
		}
		if (present("bing zhu",me))
		{
			str += "make sword:"HIM"冰柱捏剑"NOR+ZJSEP;
			str += "make dao:"HIM"冰柱捏刀"NOR+ZJSEP;
		
		}
		if (present("tongtian jing",me))
		{
			str += "detect:"HIM"探察宝物"NOR+ZJSEP;
			str += "dig:"HIM"挖宝"NOR+ZJSEP;
		...
		for (i=0;i<sizeof(dirs);i++)
		{
            ...
            else str+=sprintf("%s:%s",dirs[i],"有错误");
...
string desc_of_objects(object *obs)
{
	...
	if (obs && sizeof(obs) > 0)
	{
		...
		for (i = 0; i < sizeof(obs); i++)
		{
			...
			{
				//str = obs[i]->query("qi") + "\n";
				str += obs[i]->name()+":look "+file_name(obs[i]);
...
int mudlook_item(object me, object obj)
{
   ...
   // write(obj->long());
    ...
    str += ZJOBLONG;
	str += obj->long()+"\n";
	//str += sprintf(HIW"【名称】："+NOR+obj->query("name")+NOR+"("+obj->query("id")+")\n");
	//str += sprintf(obj->long());
    if( itype==IDF_EQUIP || 
        ( itype==IDF_SIMPLE &&
          idf==IL_FULL ) )
    {
    ...
    if (rig > 0)	//已经坏了的就不管了。
        if (rig > 100)
			str += obj->name() + "完好无损，宛如新的一般。\n";
           // message("vision", obj->name() + "完好无损，宛如新的一般。\n";
        else if (rig > 80)
            str += obj->name() + "已经轻微磨损，表面布满了细细的划痕。\n";
        else if (rig > 40)
           str += obj->name() + "多次与其他兵器撞击，表面已是遍体鳞伤，但仍然可以凑合着用。\n";
        else if (rig > 10)
            str += obj->name() + "已是严重受损，几道明显的裂纹已经危及了它的寿命。\n";
        else if (rig > 0)
           str += obj->name() + "被几道深深的裂痕分割成几部份，几乎马上就要断了。\n";
...
if(sizeof(inv))
		...
		{
			if(look_msg3!="")
				look_msg3 += ZJSEP"砍头:zhan " + obj->query("id");
			else
				look_msg3 += "砍头:zhan " + obj->query("id");
		}
			...
			if(present(obj,me))
	{
		...
		else if (stringp(obj->query("skill_type")))
		{
			...
			look_msg3 += "捐赠 "+obj->query("name")+":donate "+obj->query("id")+ZJSEP;
		}
...
look_msg3 += "捐赠 "+obj->query("name")+":donate "+obj->query("id")+ZJSEP;
			...
			look_msg3 += "下来 "+obj->query("name")+":unride "+obj->query("id")+ZJSEP;	

        if( obj->query("auto_load") )
			look_msg3 += "增强 "+obj->query("name")+":enhance "+obj->query("id")+ZJSEP;
		if(obj->query("can_be_enchased")&&obj->query("enchased_need"))
			look_msg3 += "合成 "+(obj->query("can_be_enchased"))->query("name")+":combine "+obj->query("id")+ZJSEP;

		if (obj->query("food_remaining"))
			look_msg3 += "吃 "+obj->query("name")+":eat "+obj->query("id")+ZJSEP;
			else if (obj->query("no_store"))
			look_msg3 += "吃 "+obj->query("name")+":eat "+obj->query("id")+ZJSEP;
		else if (obj->query("only_do_effect"))
			look_msg3 += "使用 "+obj->query("name")+":use "+obj->query("id")+ZJSEP;
		else if (obj->is_hsrq())
			...
			look_msg3 += "快捷研读 "+obj->query("name")+":alias kjyd study "+obj->query("id")+" for 50"+ZJSEP;
		}
		else if (obj->query("id")=="kuihua baodian")
			look_msg3 += "研读 "+obj->query("name")+":study kuihua baodian"+ZJSEP;
if(objectp(present("chuwu dai",me)))
			look_msg3 += "存入 "+present("chuwu dai",me)->query("name")+":mystore put "+obj->query("id")+ZJSEP;
		      look_msg3 += "查看 "+obj->query("name")+":lookmore "+obj->query("id")+ZJSEP;

		look_msg3 += "出售 "+obj->query("name")+":sellob "+obj->query("id")+ZJSEP;
		look_msg3 += "丢弃 "+obj->query("name")+":drop "+obj->query("id")+ZJSEP;
		//if( arrayp(autoloads = obj->query("auto_load")) || member_array( filename, autoloads ) == -1 )
		//look_msg3 += "销毁 "+obj->query("name")+":destroy "+obj->query("id")+ZJSEP;
		if ((obj->is_gems()||!obj->is_gem())&&obj->is_parts()){
			look_msg3 += "修复 "+obj->query("name")+":repair "+obj->query("id")+ZJSEP;
			look_msg3 += "拆解 "+obj->query("name")+":disassemble "+obj->query("id");
		}
		//look_msg3 += "拆解 "+obj->query("name")+":disassemble "+obj->query("id");
...
//set("target_name", me->query_temp("huashan/songxin/target_name"));
  //      set("target_id", me->query_temp("huashan/songxin/target_id"));
	if(mapp(inq = obj->query("inquiry")))
	{
		...
		{
			str += jiaoliu[i]+":ask "+obj->query("id")+" about "+jiaoliu[i];
			if(i<(sizeof(jiaoliu)-1)) str += ZJSEP;
		}
	}
	if (present("letter",me))
	{
		if(str!=ZJOBACTS) str += ZJSEP;
		str += "送信:songxin "+obj->query("id");
	}
	if (present("fo jing",me))
	str += "送佛经:song "+obj->query("id");
	if (present("li wu",me))
	{
		if(str!=ZJOBACTS) str += ZJSEP;
		str += "问好:wenhao "+obj->query("id");
	}
	
	/*if(obj->is_quester())
	{
		if(str!=ZJOBACTS) str += ZJSEP;
		str += "历练:ask "+obj->query("id")+" about 历练";
	}*/
	if (me->query("family/family_name")=="星宿派")
	{if(str!=ZJOBACTS) str += ZJSEP;
		str += "化字大法:exert hua "+obj->query("id");
	}
	if (me->query("family/family_name")=="天龙寺")
	{
		if(str!=ZJOBACTS) str += ZJSEP;
		str += "化缘:huayuan "+obj->query("id");
	//	str += "送佛经:song "+obj->query("id");
	}
/*	if(me->query_temp("tlnewbiejob/adv1/pin")>0)
	{
		if(str!=ZJOBACTS) str += ZJSEP;
		str += "化缘:huanyuan "+obj->query("id");
	}*/
	if( me->query("自创门派") )
	{
		if(str!=ZJOBACTS) str += ZJSEP;
		str += "收徒:accept "+obj->query("id");
	}
	if( obj->query("自创门派") )
	{
		if(str!=ZJOBACTS) str += ZJSEP;
	...
	if(me->query_temp("apprentice/step") >0)
	{
		if(str!=ZJOBACTS) str += ZJSEP;
		str += "请安:qingan "+obj->query("id");
	}
...
if(obj->is_corpse())
{   str += "内力灌注:zhishi " + obj->query("id")+ZJSEP;
	str += "斩:zhan " + obj->query("id")+ZJSEP;
	str += "搜刮:get all from " + obj->query("id");
				}
else{	
//if(me->query_temp("apprentice/step") >0)
//str += "请安:qingan "+obj->query("id")+ZJSEP;
if(obj->is_vendor())
	{
		str += "购物:list"ZJSEP;
	}
	if(obj->is_waiter())
	{
	 ...
	 str += "乞讨:beg " + obj->query("id")+ZJSEP;
 if (me->query("marry/lover") == obj->query("id")) 
    str += "亲热:loving " + obj->query("id")+ZJSEP;
	if(!environment(obj)->query("no_fight"))
	{
		str += "切磋:fight " + obj->query("id")+ZJSEP;
		str += "打击:hit " + obj->query("id")+ZJSEP;
		str += "偷袭:touxi " + obj->query("id")+ZJSEP;
		str += "就地正法:kill " + obj->query("id")+ZJSEP;
		str += "组队:team " + obj->query("id")+ZJSEP;
		str += "装备:arm " + obj->query("id")+ZJSEP;
	}
	if(obj->is_character())
	{
	...
	if(obj->query("race")=="人类")
	{
		...
		str += "随机装备给予:bgive " + obj->query("id")+ZJSEP;
	}
	if(obj->is_quester())
	{
		str += "领取任务:quest " + obj->query("id")+ZJSEP;
	}
	if (present("hehan san",me))
	{
		str += "撒阴阳合欢散:sa "+obj->query("id")+ZJSEP;
	}
	if(wizardp(me))
	{
		str += "闪电:smash " + obj->query("id")+ZJSEP;
		str += "复制:copyskill " + obj->query("id")+ZJSEP;
...
//如果头上戴着面具则自动去除面具
    ...
    str =  ZJOBLONG+obj->short();
			str +=  "\n""──────────────────────"NOR"\n";
			if(obj->query("long")){
			 str += obj->query("long")+"\n"+ZJURL("cmds:focus "+obj->query("id"))"锁定(开团的时候只打他)"NOR"\n";
			 str +=  "\n""──────────────────────"NOR"\n";
			 }
else str +=  "";
    ...
    if(region_name != "不明区域" && region_name != "未知区域")
    {
		if(!pl->query("found_region/"+dir_name) && time() - pl->query("found_region/last_time") > (random(600) + 5))
        {
            ...
            if(pl->query("antirobot/deactivity") > 1)
            {
				...
				tell_object(pl,ZJFORCECMD("fullme"));
            }
