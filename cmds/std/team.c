...
string msg,pops;
	...
	if ( !arg )
	{
		if ( !arrayp( team ) )
			...
			msg = ZJOBLONG+"现在你所在队伍的成员有：\n" ZJOBACTS2+ZJMENUF(1,1,7,40);
		//write( "现在你所在队伍的成员有：\n\n" );
     //   msg += sprintf("  ※  %-14s %-14s %-14s\n", "姓名", "ＩＤ", "气血");
		for (int i=0; i<sizeof(team); i++){
			pops = ZJPOPMENU+ZJMENUF(1,3,9,30);
			pops += "开除 "+team[i]->query("name")+"|team "+team[i]->query("id")+ZJSP2;
			pops += "脱离队伍 |team none"+ZJSP2;
			pops += "查看队伍 |team"+ZJSP2;
		
            msg += sprintf("  ※  %-14s %-14s "+ZJBR+"%d"+NOR+":%s"+ZJSEP,team[i]->query("name"), "("+team[i]->query("id")+")", team[i]->query("qi"),pops);
		}
		 write(msg+"\n");
		return 1;
	}
