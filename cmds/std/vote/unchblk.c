...
int vote(object me, object victim)
{
  ...
  my_name = me->name()+"("+me->query("id")+")";
  if (me == victim) v_name = "自己";
  	else  v_name = victim->name()+"("+victim->query("id")+")";

  if (df>1)
  {
	shout( HIG "【表决】"+my_name+HIG+"投票打开" +v_name +HIG+"的"+channel+"频道，还差"
			+sprintf("%d", df)+"票。\n" NOR);
	write( HIG "【表决】"+my_name+HIG+"投票打开" +v_name +HIG+"的"+channel+"频道，还差"
			+sprintf("%d", df)+"票。\n" NOR);
  ...
  } else 
  {
  	if (me != victim)
  	{
	shout( HIG "【表决】"+my_name+HIG+"投票打开" +v_name+HIG+"的"+channel+"频道。"
		+v_name+HIG+"的"+channel+"频道被打开了！\n" NOR);
	write( HIG "【表决】"+my_name+HIG+"投票打开" +v_name+HIG+"的"+channel+"频道。"
		+v_name+HIG+"的"+channel+"频道被打开了！\n" NOR);
	} else 
	{
	shout( HIG "【表决】"+my_name+HIG+"投票打开自己的"+channel+"频道。"
		+my_name+HIG+"的"+channel+"频道被打开了！\n" NOR);
	write( HIG "【表决】"+my_name+HIG+"投票打开自己的"+channel+"频道。"
		+my_name+HIG+"的"+channel+"频道被打开了！\n" NOR);
	}		
