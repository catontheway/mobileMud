// update.c
// modified by Zine 2011 用临时文件编译，保证系统稳定性
#include "/doc/help.h"
inherit F_CLEAN_UP;
...
//printf("test\n");
	me->set("cwf", file);
    parts = explode(file,"/");
    tmp_file=parts[sizeof(parts)-1];
    tmp_file="/d/wizard/update/"+tmp_file;
    cp(file,tmp_file);
    if (obj = find_object(tmp_file)) 
      destruct(obj);
//printf("test\n");
    if (obj) 
...
//printf("test\n");
        write("重新编译 " + tmp_file + "：");
        err = catch( call_other(tmp_file, "???") );
        if (err)
        {
            printf( "发生错误：\n%s\n删除临时"+tmp_file+",保留原始文件"+file+"。\n", err );
            rm(tmp_file);
            if (obj = find_object(tmp_file))
              destruct(obj);
        }
            ...
            {
                write("成功！\n");
                write("使用"+tmp_file+"代替"+file+"，删除临时文件"+tmp_file+"。\n");
                //cp(tmp_file,file);
                rm(tmp_file);
                if (obj = find_object(tmp_file))
                  destruct(obj);
