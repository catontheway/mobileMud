...
int main(object me,string arg)
{
		...
		if (arg=="-a")
	{
		 msg = ZJOBLONG;
		 msg += "升级要求："ZJBR+NOR;
		 msg += "升级将"+HIR+"消耗"+NOR+"经验，声望以及金钱"+ZJBR+NOR;

		if ( level>0 && level <= 100 )
	{
		msg += HIC"当前经验："+jy+"\t\t\t\t至少需要"+level*level*level*400+"点经验！"ZJBR+NOR;	
	
	}
	if ( level>100  )
	{
		msg +=HIC"当前经验："+jy+"\t\t\t\t至少需要"+chinese_number(400+(level-100)*(level-100)+10)+"兆经验！\n"NOR;	
	
	}
	if ( level>0 && level <= 100 )
	{
		msg +=HIG"当前声望："+sw+"\t\t\t\t至少需要"+level*20000+"点声望！"ZJBR+NOR;
	}
	if ( level > 100 )
	{
		msg +=HIG"当前声望："+sw+"\t\t\t\t至少需要"+level*200000+"点声望！"ZJBR+NOR;
		
	}
	if ( level>0 && level <= 100 )
	{
		msg +=HIY"当前存款："+ck+"\t\t\t\t至少需要"+level*10+"两黄金的存款！"ZJBR+NOR;
		
	}
	if ( level > 100 )
	{
		msg +=HIY"当前存款："+ck+"\t\t\t\t至少需要"+level+"块金块的存款！"ZJBR+NOR;
		
	}
		msg += NOR"\n"ZJOBACTS2+ZJMENUF(1,1,10,32);
				
				msg += HIR"升级:levelup"ZJSEP;
				msg += "\n";
                write(msg + "\n");
		return 1;
	}
