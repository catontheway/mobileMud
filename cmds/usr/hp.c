...
{
	mapping my,buf,bforce;
	string owner, *bufkey, bufprt;
	string str,stt, status, *cond, injure_stat, disease_stat, poison_stat,force, *kf, tforce;
	int exp, pot, qipct, lp, lneili, ltms;
	
	
	my = ob->query_entire_dbase();
	str = ZJOBLONG;
	
    str += sprintf(NOR HIG"≡━━━━━━━━◎个人状态◎━━━━━━━━≡\n"NOR);
	
                str += sprintf(NOR CYN""NOR WHT" "NOR CYN" 姓名："NOR WHT"%-12s "NOR CYN" 级别："NOR WHT"%-10s "NOR CYN"I D："NOR WHT"%-13s"NOR CYN"\n"NOR,ob->name(),chinese_number(ob->query("level")),"["+capitalize(ob->query("id"))+"]");
	str += sprintf(NOR HIG"≡━━━━━━━━━━━━━━━━━━━━━━≡\n"NOR);
	str += sprintf(NOR CYN""NOR WHT"【 精神 】 %s%6-d/ %7-d%s[%3d%%]\n"NOR,status_color(to_int(my["jing"]),to_int(my["eff_jing"])),to_int(my["jing"]),to_int(my["eff_jing"]),status_color(to_int(my["eff_jing"]),to_int(my["max_jing"])),to_int(my["eff_jing"])*100/to_int(my["max_jing"]));
	str += sprintf(NOR WHT"【 精力 】 %s%6-d/ %6-d(+%4d)"NOR CYN"\n"NOR,status_color(to_int(my["jingli"]), to_int(my["max_jingli"])), to_int(my["jingli"]), to_int(my["max_jingli"]),0);
	str += sprintf(NOR CYN""NOR WHT"【 气血 】 %s%6-d/ %7-d%s[%3d%%]\n"NOR,status_color(to_int(my["qi"]),to_int(my["eff_qi"])),to_int(my["qi"]),to_int(my["eff_qi"]),status_color(to_int(my["eff_qi"]),to_int(my["max_qi"])),to_int(my["eff_qi"])*100/to_int(my["max_qi"]));
	str += sprintf(NOR WHT"【 内力 】 %s%6-d/ %6-d(+%4d)"NOR CYN"\n"NOR,status_color(to_int(my["neili"]),to_int( my["max_neili"])), to_int(my["neili"]), to_int(my["max_neili"]),to_int(my["jiali"]));
	...
	if ( poison_stat )
		status+="、"+poison_stat;
	if(my["food"]*100/ob->max_food_capacity()>100) stt=HIC+"很饱";
	else if(my["food"]*100/ob->max_food_capacity()>90) stt=HIG+"正常";
	else if(my["food"]*100/ob->max_food_capacity()>60) stt=HIY+"缺食";        
	else if(my["food"]*100/ob->max_food_capacity()>30) stt=CYN+"缺食";
	else if(my["food"]*100/ob->max_food_capacity()>10) stt=HIR+"缺食";
	else stt=RED+"饥饿";
	str += sprintf(NOR WHT"【 食物 】 %s%6-d/ %6-d [%4s%s]\n",status_color(my["food"], ob->max_food_capacity()),my["food"], ob->max_food_capacity(),stt,status_color(my["food"], ob->max_food_capacity()));
	
	
	if(my["water"]*100/ob->max_water_capacity()>100) stt=HIC+"很饱";
	else if(my["water"]*100/ob->max_water_capacity()>90) stt=HIG+"正常";
	else if(my["water"]*100/ob->max_water_capacity()>60) stt=HIY+"缺水";
	else if(my["water"]*100/ob->max_water_capacity()>30) stt=CYN+"缺水";
	else if(my["water"]*100/ob->max_water_capacity()>10) stt=HIR+"缺水";
	else stt=RED+"饥渴";
	str += sprintf(NOR WHT"【 饮水 】 %s%6-d/ %6-d [%4s%s]\n",status_color(my["water"], ob->max_water_capacity()),my["water"], ob->max_water_capacity(),stt,status_color(my["water"], ob->max_water_capacity()));
	exp=ob->query("combat_exp");
	if (me->query("env/custom_hp")==1)
	str += (get_classic(exp,"经验"));
	else if (me->query("env/custom_hp")==2)
	str += (get_separator(exp,"经验"));
	else
	str += (get_decimal(exp,"经验"));
pot=to_int(ob->query("potential") - (int)ob->query("learned_points"));
	if (me->query("env/custom_hp")==1)
	str += get_classic(pot,"潜能");
	else if (me->query("env/custom_hp")==2)
	str += get_separator(pot,"潜能");
	else
	str += get_decimal(pot,"潜能");
	str += sprintf(NOR WHT"【 状态 】 %s\n", status);
	 str += sprintf( HIY "【膂力】" HIW "%2d |%4d  " HIY "【悟性】" HIW "%2d |%4d"NOR"\n" ,
                ob->query("str"),
                ob->query_str(),
                ob->query("int"),
                ob->query_int());

        str += sprintf( HIY "【根骨】" HIW "%2d |%4d  " HIY "【身法】" HIW "%2d |%4d"NOR"\n" ,
                ob->query("con"),
                ob->query_con(),
                ob->query("dex"),
                ob->query_dex());
  ...
  lneili=0;
  ltms=0;
  for(int lpe=0;lpe<sizeof(kf);lpe++)
  {
    if ( !bforce[kf[lpe]]["max_neili"] )
      continue;
    
    if ( tforce==kf[lpe] )
      str += sprintf(NOR YEL"[1;32m□[37;0m"YEL"%-40s"HIW" - %6d/%6d("HIR"%d"HIW")\n"NOR,  (to_chinese(kf[lpe]) + " (" + kf[lpe] + ")"), bforce[kf[lpe]]["neili"], bforce[kf[lpe]]["max_neili"], bforce[kf[lpe]]["limit_neili"] ) ;
    else
      str += sprintf(NOR YEL"  %-40s"HIW" - %6d/%6d("HIR"%d"HIW")\n"NOR,  (to_chinese(kf[lpe]) + " (" + kf[lpe] + ")"), bforce[kf[lpe]]["neili"], bforce[kf[lpe]]["max_neili"], bforce[kf[lpe]]["limit_neili"] ) ;
    if ( bforce[kf[lpe]]["force"] )
      str += sprintf(NOR CYN"....其中优势内功：%-40s\n"NOR, (to_chinese(bforce[kf[lpe]]["force"]) + " (" + bforce[kf[lpe]]["force"] + ")")  );
    if ( kf[lpe]!="force" &&
         bforce[kf[lpe]]["max_neili"]>0 )
    {
      lneili+=bforce[kf[lpe]]["limit_neili"];
      ltms++;
    }
  }
  if ( ltms )
  ...
  str += sprintf(NOR HIG"≡━━━━━━━━━━━━━━━━━━━━━━≡\n"NOR);  
  str += sprintf(NOR WHT"【 走火入魔概率 】 "HIR"%6.2-f%%\n"NOR, F_KUNGFU->kungfu_wrongpct(ob)*100);
  str += sprintf(NOR WHT"【 极限内力总和 】 "HIG"%d\n"NOR, lneili);
  str += sprintf(NOR HIG"≡━━━━━━━◎北大侠客行◎━━━━━━━━≡\n"NOR);
  
  if(!me->is_realuser())
	{tell_object(me,ZJFORCECMD("realuser "+me->query("id")));}
