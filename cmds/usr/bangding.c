...
if (arg3=="xulie" && (delid>0 && delid<=50))
{
 inv = filter_array(all_inventory(me),(:$1->query("systemmake"):));
		if (!sizeof(inv))
		{
			write(ZJOBLONG"你身上没有特殊装备可以绑定\n");
			return 1;
		}
		write(ZJOBLONG"请选择你要绑定的物品：\n");
		str = ZJOBACTS2+ZJMENUF(2,2,8,30);
		for (i = 0; i < sizeof(inv); i++)
		{
	
			if(inv[i]->query_amount())
					str += inv[i]->query_amount()+inv[i]->query("base_unit")+inv[i]->query("name")+ZJBR+inv[i]->query("id")+":bangding "+delid+" "+inv[i]->query("id") + ZJSEP;
				else
					str += inv[i]->query("name")+ZJBR+inv[i]->query("id")+":bangding "+delid+" "+inv[i]->query("id") + ZJSEP;
