...
int main(object me, string arg)
{
	...
	if (! arg)
	{
	 str = ZJOBLONG"你目前有"+me->query("jhxk")+"点侠客积分，请选择您想购买的道具：\n"ZJOBACTS2+ZJMENUF(4,4,8,37); 	
	// str += "绑定商城:myshop2"ZJSEP;
	 //str += "特价商城:myshop3"ZJSEP;	
		for(i=0;i<sizeof(all_file);i++)
		{
			file = MYSHOP_DIR+all_file[i];
			if(file_size(file)<0) continue;
			if(file->query("no_shop")) continue;
			yuanbao = file->query("jhxk");
			price = yuanbao?(yuanbao+"点侠客积分"):(MONEY_D->money_str(file->query("value")));
			str += sprintf("%s"ZJBR"%s:mydz look %s"ZJSEP,file->query("name"),price,file);
		}
		write(str+"\n");
		return 1;
	}
	...
	{
		str = ZJOBLONG"你目前有"+me->query("jhxk")+"点侠客积分，请选择您想购买的道具类别：\n"ZJOBACTS2+ZJMENUF(4,4,8,37);
		types = sort_array(keys(mtypes),(:strcmp:));
		for(i=0;i<sizeof(types);i++)
		{
			str += sprintf("%s:mydz type %s"ZJSEP,types[i],types[i]);
		}
		write(str+"\n");
		return 1;
	}
	else if (sscanf(arg,"type %s",type)==1)
	{
		...
		if(!sizeof(all_file))
			return notify_fail("商城中暂无"HIG+type+NOR"类商品出售。\n");
		str = ZJOBLONG"你目前有"+me->query("jhxk")+"点侠客积分，请选择您想购买的"HIG+types[i]+NOR"类道具：\n"ZJOBACTS2+ZJMENUF(4,4,8,37);
		for(i=0;i<sizeof(all_file);i++)
		{
	...
	else if (sscanf(arg,"look %s",file)==1)
	{
		...
		str += "购买1个:mydz buy 1 "+file+ZJSEP"批量购买:mydz buy "+file+"\n";
		write(str+"\n");
		return 1;
	}
	else if (sscanf(arg,"buy %s",file)==1)
	{
		if (sscanf(file, "%d %s", amount, file) != 2)
		{
			...
			write(INPUTTXT("你想买多少个"+file->query("name")+"？"ZJBR"请确认你的背包空间足够。","mydz buy $txt# "+file)+"\n");
			return 1;
		}
		...
		if(yuanbao)
		{
			if(!me->query("jhxk")||me->query("jhxk")<(yuanbao*amount))
				return notify_fail(HIM"你的侠客积分不足，请联系群主！！联系方式：QQ:79378313 QQ群：558979137"NOR"\n");
			me->add("jhxk",-yuanbao*amount);
			log_file("pay/myshop", sprintf("%s:%s(%s)消费%d点侠客积分，从随身商城购买%d个%s(%s)。\n",
				ctime(time()),me->query("name"),me->query("id"),yuanbao*amount,amount,file->query("name"),file->query("id")));
			me->add("xkx/jhxk_used",yuanbao*amount);
		}
		else
		{
