...
int main(object me, string arg)
{
       ...
       for(i=k; k<sizeof(text); k++)
              write(text[k] + "\n");
        write(ZJMAPTXT+replace_string(tmp,"\n",ZJBR)+"["HIC"温磬提示："HIG"("HIB"蓝底"HIG"亮绿色的代表你目前所处房间)，"HIY"(黄色的代表有不规则出口的房间)，"HIM"]\n"NOR);
       return 1;
}
...
void set_room(int x,int y,object room)
{
       ...
       // 求取无颜色的房间short描述
       name=filter_color(room->query("short"));
	...
	else
	{
      ...
      if((x==(2*X+1)/2)&&(y==(2*Y+1)/2))
                name = ZJURL("cmds:walk "+name)ZJSIZE(28)+HBBLU+HIG+name;
		//name =HBBLU+HIG+name;
 
        else if( flag )
               name = ZJURL("cmds:walk "+name)ZJSIZE(28)+BYEL+name;
		//name = HIW+name;
	else
		name = ZJURL("cmds:walk "+name)ZJSIZE(29)+BCYN+name;            
       name+=NOR;
       m[x][y]=name;
}
