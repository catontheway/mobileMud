...
private string perform_stat(mapping buf, string color, int time_flag);
...
int main(object me, string arg)
{
	...
	string str,skill_type, msg, wtype;
	...
	string *skills=({"dodge","parry","force","cuff","strike","claw","finger",
        ...
        if ( !flag || flag==1 )
        {
          ...
          if ( ob->query("last_attack_info") )
          {
	          ...
	          str += sprintf(HIC"你攻击对手%s(%s)"HIC"，有%.2f%%几率被对手躲闪，%.2f%%几率被对手招架。\n"NOR,
	          ...
	          str += sprintf(HIC"对手%s(%s)"HIC"攻击你，有%.2f%%几率被你躲闪，%.2f%%几率被你招架。\n"NOR,
              ob->query("last_attack_info/name"), ob->query("last_attack_info/id"), 
        ...
        if ( !flag || flag==2 )
        {
        	...
        	for(int lp=0;lp<sizeof(chars);lp++)
        	{
        		if ( ob->query_temp("apply/"+chars[lp]) )
        		{
        			ind++;
        			if ( ind==1 )
						str += sprintf(NOR HIG"≡━━━━━━━━◎天赋加成◎━━━━━━━━≡\n"NOR);
        				
        			
        			str+=sprintf("%s: %-6s ", chn_chars[lp], sprintf("%+d点",ob->query_temp("apply/"+chars[lp])) );
        			if ( !(ind%4) )
        				str+="\n";
        		}
        	}
        ...
        if ( !flag || flag==3 )
        {
        	...
        	for(int lp=0;lp<sizeof(skills);lp++)
        	{
        		if ( ob->query_temp("apply/"+skills[lp]) )
        		{
        			ind++;
        			if ( ind==1 )
						str += sprintf(NOR HIG"≡━━━━━━━━◎技能加成◎━━━━━━━━≡\n"NOR);
        				
        			
        			str+=sprintf("%s: %-6s \n", to_chinese(skills[lp]), sprintf("%+d级",ob->query_temp("apply/"+skills[lp])) );
        			if ( !(ind%4) )
        				str+="\n";
        		}
        	}
...
/*        	
        ...
        if ( !flag || flag==4 )
        {
          ...
          str += sprintf(HIW"[加成效果]：\n"GRN"气血加成：%+-6d  \n精神加成：%+-6d  \n"HIW"[恢复效果]：\n"GRN"内力恢复：%+-7.2f \n精力恢复：%+-7.2f\n", 
               ob->query_temp("apply/max_qi"), ob->query_temp("apply/max_jing"), ob->query_neili_heal()/9.5, ob->query_jingli_heal()/9.5);
          str += sprintf("气血恢复：%+-7.2f \n精神恢复：%+-7.2f \n"HIW"[治疗效果]：\n"GRN"气血治疗：%+-7.2f \n精神治疗：%+-7.2f\n", 
               ob->query_qi_recover()/9.5,  ob->query_jing_recover()/9.5, ob->query_qi_cure()/9.5,ob->query_jing_cure()/9.5);
          str += sprintf("内力精纯度：%+-7.2f%%\n"NOR, ob->pure_degree("neili")*100);
        }
        if ( !flag || flag==5 )
        {
			str += sprintf(NOR HIG"≡━━━━━━━━◎健康状态◎━━━━━━━━≡\n"NOR);
         
          str +=sprintf(WHT"    %s\n"NOR, user_stat(ob));
        }
        if ( !flag || flag==6 )
        {
			str += sprintf(NOR HIG"≡━━━━━━━━◎其他属性◎━━━━━━━━≡\n"NOR);
          
          str += sprintf(MAG"幸运值：%-6d\n"NOR, ob->query_temp("apply/magic"));
		  if (mapp(ob->query_temp("apply/gainexp"))&&ob->query_temp("apply/gainexp")["due"]>=time())
		  str += sprintf("%-20s",HIG+"奖励加成："+ob->query_temp("apply/gainexp")["bonus"]+"%\n"+NOR);
        }
		str += sprintf(NOR HIG"≡━━━━━━━━◎浪迹天涯◎━━━━━━━━≡\n"NOR);
