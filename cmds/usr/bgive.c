...
int main(object me, string arg)
{
	...
	{
		inv = filter_array(all_inventory(me),(:$1->query("systemmake") && !$1->query("equipped") :));
		if (!sizeof(inv))
		{
		...
		for (i = 0; i < sizeof(inv); i++)
		{
			if (!inv[i]->query("equipped"))
			{
				if(inv[i]->query_amount())
					str += inv[i]->query_amount()+inv[i]->query("base_unit")+inv[i]->query("name")+ZJBR+inv[i]->query("id")+":bgive "+ inv[i]->query("id")+" to "+arg + ZJSEP;
				else
					str += inv[i]->query("name")+ZJBR+inv[i]->query("id")+":bgive "+ inv[i]->query("id")+" to "+arg + ZJSEP;
			}
		}
		write(str+"\n");
		return 1;
  ...
  if ((int)me->query("balance",1)<= 100000)
  	return notify_fail("特殊装备转移需要收十两两黄金收续费 请把相应钱款存入你的银行！\n"); 
