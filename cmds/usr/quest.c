  ...
  if (!me->query_temp("yidao/done") && me->query("yidao/on_job"))
{
          ...
          if ( arg == "-m" )
          {
            ...
            if ( me->query("yidao/questid")=="still" )
        	    tell_object(me, "到"+me->query_temp("yidao/where")+"找"+me->query_temp("yidao/killer"));
            else if ( me->query("yidao/questid")=="maze" )
        	   tell_object(me, "到"+me->query_temp("yidao/where")+"找"+me->query_temp("yidao/suicong"));
            else
...
if (me->query_temp("hyw/start"))
{
    ...
    {
        tell_object(me,HIW"你现在要去" + me->query_temp("hyw/where") + "就地正法"+me->query_temp("hyw/name")+"。\n"NOR);
    }
}
