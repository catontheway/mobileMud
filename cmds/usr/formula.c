...
int main(object me, string arg)
{
	...
	if (!arg)
	{
		...
		msg = ZJOBLONG+HIW+"【公共药方】与【个人药方】（红色字体代表个人药方）\n"NOR+ZJOBACTS2+ZJMENUF(1,1,4,35);
		for (int i=0;i<sizeof(pres);i++)
		{
			lu_msg= pres[i]["furnace_lv"]?chinese_number(pres[i]["furnace_lv"])+"级药炉，":"";
			msg+=sprintf(HIG"%|20s"NOR+ZJBR+"炼制需要"+lu_msg+ZJBR+"练药等级%-12s"+ZJBR+"%s"+NOR+":formula "+pres[i]["name"]+ZJSEP,pres[i]["name"],"["+pres[i]["maker_level"]+"～"+pres[i]["improve_level"]+"]","『"+HIW+"配方"+NOR+"』："+"obj/remedy/reference.c"->prescription(pres[i]["raws"]));
		...
		//msg+=sprintf(HIR+"%s"+NOR+":formula"+ZJSEP,"【个人药方】");
		for (int i=0;i<sizeof(pres);i++)
		{
			lu_msg= pres[i]["furnace_lv"]?chinese_number(pres[i]["furnace_lv"])+"级药炉，":"";
			msg+=sprintf(HIR"%|20s"NOR+ZJBR+"炼制需要"+lu_msg+ZJBR+"练药等级%-12s"+ZJBR+"%s"+NOR+":formula "+pres[i]["name"]+ZJSEP,pres[i]["name"],"["+pres[i]["maker_level"]+"～"+pres[i]["improve_level"]+"]","『"+HIW+"配方"+NOR+"』："+"obj/remedy/reference.c"->prescription(pres[i]["raws"]));
		//	msg+=HIR+sprintf("%|20s","<"+pres[i]["name"]+">")+NOR;
		//	msg+=sprintf("%-12s","["+pres[i]["maker_level"]+"～"+pres[i]["improve_level"]+"]");
			if (i%2==1) msg+="";
		}
		write(msg+"\n");
