			...
			/*	if (me->query("xkxvip/times")==0)
				{
					...
					tell_object(me, "对不起，目前离线计划只对会员开放。\n");
					return 1;
				}*/
          if (me->query("balance")<10000)
			return notify_fail("你的钱太少了，还是留着买吃的吧。\n");	
                ...
                "adm/daemons/closed"->user_closed(me);
                me->set_short_desc(0);
                continue_scheme(me);
                return 1;
        }
...
int cancel_schedule(object me)
{
        ...
        "adm/daemons/closed"->user_opened(me);
...
{
	write(@HELP
指令格式 : jihua [<edit> | <start> | <clear> | <show> <玩家>]
