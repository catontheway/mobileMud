...
varargs void check_mem( string file );
...
protected varargs void check_mem( string file )
{
    ...
    for( i=sizeof(groups)-1; i>=0; i-- )
    {
        ...
        if ( (file == "-big" && (tmp_mem>1000000 || size > 100)) || file != "-big") {
	        all_mem += tmp_mem;
...
{
    write(@HELP
指令格式 : memview [-all|-big] [物件名(id)或文件名]
...
加参数 -big时：
		不加物件名参数，只显示占用内存大于1M或者复制件数量大于100个的物品信息。
