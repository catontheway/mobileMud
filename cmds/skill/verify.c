...
// Modified by iszt@pkuxkx, 2007-03-24, 修正框架对齐的问题
// Modified by Zine@pkuxkx, 2015-01-08, 增加个VerifyHidden，对应某些PFM只对部分人显示。
...
int sort_skill(string file)
{
        ...
        {
        if (file[i] == '.') continue;
        if (file == "chang2.c" || file == "chang3.c") return 1;
        if (file[i] < 'a' || file[i] > 'z') return 0;
    }
...
{
    mapping lrn;
    string exert, str, *file, *tmp=({});
    object sk;
    int i, j = 0, n;

    seteuid(getuid());

    if (!arg) return notify_fail("指令格式 : verify [武功名称]\n");

    sscanf(arg, "%s.%s", arg, str);

    if (!me->query_skill(arg, 1)) return notify_fail("你要查对什么技能？\n");

    if (file_size(SKILL_D(arg) + ".c") < 1) return notify_fail("错误！你要查对技能文件已经消失，请通知巫师。\n");
    ...
    if (SKILL_D(arg)->type() != "martial"
        || member_array(arg, keys(valid_type)) != -1) return notify_fail(str + "└─────────────────────────────────┘\n");
    ...
    if (stringp(exert = SKILL_D(arg)->exert_function_file("")))
    {
        file = get_dir(exert);
        tmp = ({});
        if (sizeof(file) > 0) file = filter_array(file,(: sort_skill :) );
        for (n=0;n<sizeof(file);n++)
        {
            if ( !function_exists("exert", load_object(exert+file[n]), 1)) tmp += ({file[n]});
        }
        if( sizeof(file)>0 && sizeof(tmp)>0 ) file -= tmp;
        j = sizeof(file);
     }
        ...
        if(j && j > 0){
                ...
                for(i=0; i < j; i++){
                        if (i % 4 == 0) str += "│\n│\t";
                        str += sprintf("%-"+(strwidth(YEL+NOR)+14)+"s ",YEL+replace_string(file[i], ".c", "")+NOR);
                        if ( i > j-2 ){
                                switch(i%4){
                                        case 0: str +="\t\t\t\t\t            "; break;
                                        case 1: str +="\t\t\t            "; break;
        ...
        if( stringp(exert = SKILL_D(arg)->perform_action_file(""))){
            ...
            for (n=0;n<sizeof(file);n++)
            {
				if (load_object(exert+file[n])->VerifyHidden(me)) file-=({file[n]});
                if ( !function_exists("perform", load_object(exert+file[n]), 1)) tmp += ({file[n]});
            }
            if( sizeof(file)>0 && sizeof(tmp)>0 ) file -= tmp;
            j = sizeof(file);
        }
        ...
        if(j && j > 0){
                ...
                for(i=0; i < j; i++){
                        if (i % 4 == 0) str += "│\n│\t";
                        str += sprintf("%-"+(strwidth(WHT+NOR)+14)+"s ",WHT+replace_string(file[i], ".c", "")+NOR);
                        if ( i > j-2 ){
                                switch(i%4){
                                        case 0: str +="\t\t\t\t\t            "; break;
                                        case 1: str +="\t\t\t            "; break;
