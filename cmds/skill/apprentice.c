...
// add by aiai
        ...
        if ((string)me->query("family/family_name") == "灵鹫宫" && me->query("lingjiu/xiulian")) 
                return notify_fail("你正在返老还童中，赶紧修炼才是正道。\n");
                
        if(!userp(me)) return notify_fail("npc不能拜师！\n");
        
// add by whuan
//        if (me->query("grandmaster") && !me->query("chushi"))
//                      return notify_fail("你通过了劫难，却没有出师，难道你想判师么？\n");                
        if( arg=="cancel" ) {
...
//              return notify_fail("对不起，现在不能拜玩家为师了！\n");
        ...
        if ( me->is_mask() )
        {
                  return notify_fail("拜师讲究诚意，你带个面罩算怎么回事？\n");
        } 
        ...
        if( me->query("class") == "officer" && ob->query("id") != "meng gong")
...
// aiai
		...
		if( me->query("family") && ob->query("family") &&
        (string)me->query("family/family_name") != (string)ob->query("family/family_name") ) 
                {
                	      return notify_fail("你必须先脱离原先的门派：" + me->query("family/family_name")+", 才能转投别的门派。\n");
                } 
...
/*                        message_vision(
                ...
                if( userp(ob) ) {
                        tell_object(ob, YEL "如果你愿意收"+ZJURL("cmds:recruit "+me->query("id"))+ me->name() + "为弟子，用 recruit 指令。\n" NOR);
                } else 
...
请注意每背叛一次师门，则可以向师傅学到的技能最高级别会减少20级

如果对你的师父使用这个指令，会变成向师父请安。
如果出现“你想拜谁谁谁为师，但是对方还没有答应“这句话
请先用apprentice cancel取消上次的拜师状态，然后重新拜
