...
//显示某个多层技能的具体技能细节
    if ( skmod )
    {
    	...
    	msg +=( "[44;1m[1;33m"+(ob==me ? "你" : ob->name()) +"[44;1m[1;33m学习的"+to_chinese(root_skill)+" ("+root_skill+")详细技能组成情况：[37;0m\n\n");
    	msg +=("[32m┌------------------------------------------------------------------┐[37;0m\n");
    	msg +=("[32m│[37;0m技能名称                         有效等级 ( 技能等级 / 技能经验 ) [32m│[37;0m\n");
    	msg +=(sprintf( show_skill_info(ob, root_skill, mapped, 1) ));
    	msg +=(sprintf( show_subskill(ob, root_skill) ));
    	msg +=("[32m└------------------------------------------------------------------┘[37;0m\n");
    	return 1;
    }    	
    
    msg +=( "[44;1m[1;33m"+(ob==me ? "你" : ob->name()) +"[44;1m[1;33m目前所学过的技能：（共"+chinese_number(sizeof(skl))+"项技能，你的技能等级最多能达到"+chinese_number(ob->query_skill_limit())+"级）[37;0m\n\n");    
    j=query_type_cnt( sname, "profession" );
    if(j>0) { msg +=(sprintf("[32m┌[42m%21s    [40m[32m-------------------------------------------┐[37;0m\n", "[37m"+chinese_number(j)+"项职业技能"));}
    for(i=0; i<sizeof(skl); i++) {
      if( query_skill_type(sname[i])=="profession" ) {
        msg +=(sprintf( show_skill_info(ob, sname[i], mapped, 0) ));
      }
    }    
    if(j>0) { msg +=("[32m└-------------------------------------------------------------------┘[37;0m\n");}
      ...
      //if( !find_object(SKILL_D(sname[i])) && file_size(SKILL_D(sname[i])+".c") < 0)//判断是否是自创武功
		{
			...
			{
				msg +=(sprintf( show_skill_info(ob, sname[i], mapped, 1) ));
			}
			continue;
		}
	...
	if (mapp(skl)){
		...
		if (j>0)
		{
			...
			msg +=(sprintf("[32m┌[42m%21s    [40m[32m----------------------------------------------┐[37;0m\n", "[37m"+chinese_number(j)+"项临时技能"));
			for(i=0; i<sizeof(sk); i++) {
				msg +=(sprintf("[32m│[37;0m%s%s%-40s" NOR " - %-10s %4d/%7d[32m│[37;0m\n",
