...
string *skillmaxim;
string skill_en_name, skill_ch_name, file;
...
if(me->query_skill(skill_base_name, 1) < 1)
    return notify_fail("你不会这种基本技能啊，怎么创得出特殊的呢？\n");
...
if(mapp(me->query("skillmaxim"))
        && sizeof(skillmaxim = keys(me->query("skillmaxim/"))) >= 3 )
{
        for (i = sizeof(keys(me->query("skillmaxim/"))) - 1; i >= 0; i--)
       	if (skill_base_name == skillmaxim[i]) flag = 1;
	if (flag != 1)
...
if(!file)
{
        if( mapp( me->query("skillmaxim") ) && me->query("combat_exp") < (sizeof(keys(me->query("skillmaxim"))) + 1) * 5000000)
	return notify_fail("你的经验不够，不能创建新的武功。\n");
        if(mapp( me->query("skillmaxim") ) && me->query("repute") < (sizeof(keys(me->query("skillmaxim"))) + 1) * 100000)
...
//             write("here be excuted???" + skill_en_name + "\n");
//                                if(!mapp(me->query("skillmaxim"))) return 1;//没有创建过武功
               return 1;
			  }
       }
else
...
至少1000两黄金，并且每次开关出室要降低10000点武林声望，所
以一旦进入闭关室，请务必潜心钻研武功。
