...
int ask_quest()
{
        	...
        	who->delete("quest/finish"); 
             ...
             if( who->query("score")>=0 &&
	           ...
	           else
	           {
             	    ...
             	    if(!who->query("score"))
               		    who->set("score",0);
                  score = 10+score * SPEWEEK_D->query_menzhong_bonus()/100; //特殊周效果
             	    who->add("score",score); 
             }
             temp = score*donenumber*3/4 + random(score*donenumber*3/4);
             if (temp == 0) temp = 1;
             tell_object(who,"你的实战经验上升了"+chinese_number(exp)+"点。\n");   
             exp = REWARD_D->add_exp(who,12*temp,"newbie");
             who->add("exp/quest",exp); 
             temp = score*donenumber*3/4 + random(score*donenumber*3/4);
             if (temp == 0) temp = 1;
             tell_object(who,"你的潜能增加了"+chinese_number(pot)+"点。\n");      
             pot = REWARD_D->add_pot(who,12*temp);
             who->add("pot/quest",pot);  
			  temp = score*donenumber*3/4 + random(score*donenumber*3/4);
             if (temp == 0) temp = 1;
             tell_object(who,"你的声望增加了"+chinese_number(pot)+"点。\n");      
            // pot = REWARD_D->add_pot(who,300*temp);
             who->add("repute",pot);  
             who->delete("quest/type");
             who->delete("quest/finish"); 
             return 1;
  ...
  if(queststring[IDX_TYPE]=="find")
    ...
    str = replace_string (str, "$w", ZJURL("cmds:tsdx "+queststring[IDX_ID])+HIR+queststring[IDX_NAME]+NOR);
...
int accept_object(object who, object ob)
{
 ...
 temp = score*donenumber/4 + random(score*donenumber/4);
  if (temp == 0) temp = 1;
  tell_object(who,"你的声望增加了"+chinese_number(temp)+"点。\n");      
 // pot = REWARD_D->add_pot(who, 2*temp);
  who->add("repute",pot);    
  who->delete("quest/type");
  who->delete("quest/finish"); 
