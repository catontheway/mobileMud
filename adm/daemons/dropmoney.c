...
//        "/d/dalicheng/",//"大理",
	...
	amount<10 ?f=amount:f=10+random(40);
...
int write_money_log(object room,int amount)
{
	int i,x;
	if(random(100)<=7) {
		CHANNEL_D->do_channel(this_object(), "jh", HIR"财神爷突然降临人间，把地上的黄金全收走了……" NOR, -1);
		rm("/log/dlog/money");
		return 0;
	}
...
void write_log(object room_o,string number)
{
    ...
    r_long=replace_string(r_long, NOR, ESC+"[0m");
	CHANNEL_D->do_channel(this_object(), "jh", HIM"风水大师预言说在某处地下埋藏了大量黄金，似乎是：\n" +ESC+"[0m" + r_long, -1);
    CHANNEL_D->do_channel(this_object(), "sys", file_name(room_o));
}
