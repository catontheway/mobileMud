...
int check_valid_read( string fname, int fd )
{
	...
	if (strncmp(fname, HOME_DIR( UNAME ), strwidth(HOME_DIR( UNAME ))) == 0
#ifdef PUB_DIR
		  || strncmp(fname, PUB_DIR, strwidth(PUB_DIR)) == 0
#endif
#ifdef FTP_DIR
		  || strncmp(fname, FTP_DIR, strwidth(FTP_DIR)) == 0
#endif
#ifdef TMP_DIR
		  || strncmp(fname, TMP_DIR, strwidth(TMP_DIR)) == 0
#endif
		  )
		return res;
	else
...
//		TP( socket_error( ret ) + "\n" );
	...
	socket_info[ new_fd ] = ([
		...
		PARENT_FD : fd,
		TYPE	  : type,
		LEN	   : (type == STRING ? strwidth(mess) : file_size(mess))
	]);
...
void data_close_callback( int fd )
{
	...
	if ( socket_info[ fd ][ TYPE ] == DOWNLOAD ) {
		...
		size = stringp( UDATA ) ? strwidth( UDATA ) : sizeof( UDATA );
...
#endif
			{
				...
				if (UNAME == "anonymous")
					log_file( LOG_FILE, sprintf("%s (%s) connected from %s.\n",
						  UNAME, str[strwidth(command[0]) + 1..<1], USITE) );
				else
#endif
