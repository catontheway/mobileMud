...
protected int process_identify(object me,string pass)
{
    ...
    if(users[fd]["obj"] == me)
    {
            str = sprintf("USER %s %s %s :%s\r\n",users[fd]["id"],
                          users[fd]["id"],query_ip_number(users[fd]["obj"]),
...
//        socket_write(fd,"PRIVMSG &bitlbee :account off "+protocol+"\r\n");
        socket_write(fd,"PRIVMSG &bitlbee :account "+protocol+" off\r\n");
//        socket_write(fd,"PRIVMSG &bitlbee :account del "+protocol+"\r\n");
        socket_write(fd,"PRIVMSG &bitlbee :account "+protocol+" del\r\n");
}
...
//    socket_write(fd,"PRIVMSG &bitlbee :account on "+protocol+"\r\n");
        socket_write(fd,"PRIVMSG &bitlbee :account msn on\r\n");
}
...
void auto_login1()
{
    ...
    register_account(me->query_temp("im_fd"),"msn","message@pkuxkx.net","xkxpku123");
