...
void report_sys_msg (string msg)
{
	CHANNEL_D->do_channel(this_object(), "sys", msg);
	log_file("monitord","["+ctime(time())+"] "+msg+"\n");
}

void report_sys_object_msg (object ob, string msg)
{
...
void report_cloned_object_msg (object ob, string msg)
{
	CHANNEL_D->do_channel(this_object(), "rumor",
		"听说" + ob->name() + "(" + capitalize(ob->query("id")) + ")" + msg);
	log_file("monitord","["+ctime(time())+"] "+
	  ob->name()+"("+capitalize(ob->query("id"))+")"+msg+"\n");
}
