...
string generaterandomcode()
{
  ...
  dict_filename = load_dict();
	...
	{
		code = read_bytes(dict_filename,random(filesize/3)*3,6);
	//	code = replace_string(code," ","");
	
	} 
	while(!is_chinese(code)){
		
	code = read_bytes(dict_filename,random(filesize/3)*3,6);
	}      
	//code="/std/bitmap_font.c"->bitmap_font(code);
	return code;	
...
/*	if(user->query_temp("MXP/image") != 1)
    ...
    tell_object(user,ZJFORCECMD("fullme "+code));
