...
{
        object *ob=({ });
        string *tuned_ch, who,whi,menpai,liupai,chtemp,location;
        int region;
...
/*        if (verb=="dasong" || verb=="dasong*")
        ...
        if (stringp(arg) && strlen(arg)>180 && living(me)&&!wizardp(me))
...
//who=me->query("name");
        if( channels[verb]["anonymous"] )
        {
                ...
                if (!me->query_temp("apply/name"))
                        ...
                        who = ZJURL("cmds:look "+me->query("id"))+HIC+me->query("name")+NOR;
...
*/        
        ...
        if( emote ) 
        {
          ...
          else if (!(verb == "act"))
          {
            if ( verb != "rumor" )
            {
              ...
              {
                if (who == "QQ精灵") //let the qq to chat channel message echo back to qq group
                {
                  arg = arg + "（回复请用QQ频道）";
          ...
          else 
          {
            ...
            {
              if (who == HIG"QQ群") //let the qq to chat channel message echo back to qq group
              {
                arg = COLOR_D->clean_color(arg);
                arg = replace_string(arg,"Ｖ","V");
                arg = COLOR_D->clean_color(arg);
	  ...
	  && verb != "mp"
  	  ...
  	  && verb != "baowei" 
  	  && verb != "dasong" 
  	  && verb != "qy"   && verb != "jy"    && verb != "qq"  //在这里把一些非玩家频道屏蔽掉,根据具体情况修改
  	  && verb != "jh")  
     //if(verb = "chat" 
 	//  || verb = "rumor" 
//  	  || verb = "sys" 
...
//  	  || verb = "yqyl")   //在这里把一些非玩家频道屏蔽掉,根据具体情况修改
      write_file("/log/channel/lt",filter_color(sprintf(channels[verb]["msg_speak"], who, arg ))+"\r\n");
        return 1;
