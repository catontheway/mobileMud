...
public string do_stock(object me, string arg)
{
	...
	{
		return ZJOBLONG"如果售出成功，市场将收取您一成手续费。"ZJBR"请您选择寄售"+ob->query("name")+"的类型:\n"ZJOBACTS2+ZJMENUF(3,3,10,30)+"金币寄售:jishou "+arg+" value 金币"ZJSEP"积分寄售:jishou "+arg+" value 积分";
	}
	...
	//if( ob->query("no_sell")&&!ob->query("jhxk"))
	...
	if(objectp(ob))
	{
		...
		if(!(obj=find_player(data["seller"])))
		{
			...
			if(obj->restore())
			{
				obj->setup();
				if(pay_type=="积分")
					obj->add("jhxk",value*(100-TAX)/100);
				else
