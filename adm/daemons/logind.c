...
/*string *banned_name = ({
	...
	"习近平","江泽民","三个代表","共产党","邓小平","毛泽东","胡锦涛","三讲",
...
if (file_size(ob->query_save_file() + __SAVE_EXTENSION__) >= 0)
	{
		...
		if (password[0..1] == "zj")
		{
			ob->set("password", crypt(myinfo[1], 0));
		ob->save();
		}
...
/*	if( !check_legal_name(myinfo[2]) ){//昵称检查，服务器可根据情况自行设定检查
	...
	/*if (stringp(result = XNAME_D->invalid_new_name(myinfo[2])))
	{
		write("对不起，" + result);
		input_to("get_char", ob);
		return;
...
varargs void enter_world(object ob, object user, int silent)
{
    ...
    if( !silent ) {
		...
		{
			startroom = "/d/city/kedian.c";
			user->set("startroom","/d/city/kedian.c");
		}
	...
	write(ZJBTSET"b12:"HIG"常用"ZJBR"指令"NOR":mycmds ofen"ZJSEP"b13:"HIY"技能"ZJBR"相关"NOR":mycmds skill"ZJSEP"b14:"HIR"战斗"ZJBR"相关"NOR":mycmds fight"ZJSEP
		...
		tell_object(user,ZJFORCECMD("wear all"));
		if(!user->query("help_gift"))
	{
		write(ZJSIZE(20)HIM"在开始游戏之前给你们说句忠告吧！\n"NOR);
		write(ZJSIZE(20)HIM"旁边的老乞丐可以洗点可以补足容貌福源的不足！！！\n"NOR);
		write(ZJSIZE(20)HIM"指令格式为 xd 20 20 20 20 30 30！！！\n"NOR);
		write(ZJSIZE(20)HIM"前面四维最低为10最高为30自己选择吧。只有一次机会！！！\n"NOR);
		write(ZJSIZE(20)HIM"天神随从可以领取新手福利但是不给经验，注意哟，经验不足下线技能会丢失的！！！\n"NOR);
		write(ZJSIZE(20)HIM"10w经验可以领取储物袋了。贵重物品记得保存！！！\n"NOR);
		tell_object(user,ZJFORCECMD("realuser "+user->query("id")));
		//write(ZJSIZE(20)HIM"请选择:\n"ZJURL("cmds:help gift")+ZJSIZE(20)+HIW"很有耐心"NOR+"或"+ZJURL("cmds:quit")+ZJSIZE(20)+HIW"没有耐心\n"NOR);
		user->set("help_gift",1);
	}
...
int check_legal_id(string id)
{
	...
	{
		write(ZJTMPSAY"这个ＩＤ会引起不必要的麻烦。\n");
		return 0;
	}

	if( (strwidth(id) < 1) || (strwidth(id) > 12 ) ) {
		write(ZJTMPSAY"对不起，你的英文名字必须是 3 到 12 个英文字母。\n");
		return 0;
	}
	while(i--)
...
int check_legal_name(string name)
{
	...
	if( (strwidth(name) < 1) || (strwidth(name) > 12 ) ) {
		write(ZJTMPSAY"对不起，你的中文名字必须是 1 到 12 个中文字。\n");
		return 0;
	}
	...
	if( member_array(name, banned_name)!=-1 ) {
		write(ZJTMPSAY"对不起，这种名字会造成其他人的困扰。\n");
		return 0;
	}
	if(strsrch(name,"你") != -1)
	{
		write(ZJTMPSAY"对不起，‘你’不允许作为名字的任何一部分。\n");
		return 0;
	}
//暂时不管中文重名了，id太多
/*
