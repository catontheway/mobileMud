...
#include <ansi.h>
string TOP5RANK(object ob,int rank)
{
	string Top5rank,*pre_rank=({"中神","东","南","西","北"}),*default_post_rank=({"通","邪","帝","毒","丐"}),*colors=({HBYEL+HIW,HBGRN+HIR,HBRED+HIY,HBWHT+HIB,HBBLU+HIG});
	if (ob->query("TOP5/rank_name")&&strwidth(ob->query("TOP5/rank_name"))==2)
		Top5rank=pre_rank[rank]+ob->query("TOP5/rank_name");
	else
		Top5rank=pre_rank[rank]+default_post_rank[rank];
	if (strwidth(Top5rank)==6)
	return colors[rank]+"【"+Top5rank[0..1]+" "+Top5rank[2..3]+" "+Top5rank[4..5]+"】"+NOR;
	else
	return colors[rank]+"【 "+Top5rank[0..1]+"  "+Top5rank[2..3]+" 】"+NOR;
}
string query_rank(object ob)
{
//	mapping fam;
	string str;
	string gender,myclass,proclass;
	string family;
	...
	if (STATISTIC_D->ret_top5()&&arrayp(STATISTIC_D->ret_top5())&&member_array(ob->query("id"),STATISTIC_D->ret_top5())!=-1)
		return TOP5RANK(ob,member_array(ob->query("id"),STATISTIC_D->ret_top5()));
	if (SECURITY_D->get_status(ob)=="(guider)")
        {
            if (sizeof(GUIDER_D->app_list(ob->query("id")))>=20&&sizeof(GUIDER_D->oldapp_list(ob->query("id")))>=20&&GUIDER_D->guider_score(ob->query("id"))>5000)
            return HIC"【至圣先师】"NOR;
            if (sizeof(GUIDER_D->app_list(ob->query("id")))>=10&&sizeof(GUIDER_D->oldapp_list(ob->query("id")))>=20&&GUIDER_D->guider_score(ob->query("id"))>2000)
            return HIC"【万世师表】"NOR;
			if (sizeof(GUIDER_D->app_list(ob->query("id")))>=10&&sizeof(GUIDER_D->oldapp_list(ob->query("id")))>=10&&GUIDER_D->guider_score(ob->query("id"))>1000)
			return HIC"【桃李满园】"NOR;
			else
			return HIC"【新手导师】"NOR;
        }
...
//by seagate@pkuxkx 2010/12/04
  if ( ob->query_temp("apply/family") )
	...
	switch(family)
	{
		...
		case "日月神教": budd = ob->query_skill("daguangming-xinfa", 1); break;
		case "古墓派": budd = ob->query_skill("yunu-xinfa", 1); break;
		case "灵鹫宫": budd = ob->query_skill("bahuang-gong", 1); break;
		case "全真派": budd = ob->query_skill("daoxue-xinfa", 1); break;
		case "天龙寺": budd = ob->query_skill("yiyang-zhi", 1); break;
...
//by seagate@pkuxkx 2010/12/04
	...
	else
	{
		...
		age = ob->query("age");
		proclass=F_PRO->pro_class(ob);
	}
	if (!wizardp(ob))
	switch(proclass)
		{
			case "alchemy":
			budd=ob->query_skill("alchemy",1);
			if (budd>220) return HIW"【 药  王 】"NOR;
			if (budd>150) return HIW"【制药大师】"NOR;
			if (budd>100) return HIW"【大 药 师】"NOR;
			if (budd>50)  return HIW"【 药  师 】"NOR;
			else return HIW"【 药  士 】"NOR; 
			case "herbalism":
			budd=ob->query_skill("herbalism",1);
			if (budd>250) return WHT"【百草药王】"NOR;
			if (budd>150) return WHT"【大采药师】"NOR;
			if (budd>50) return WHT"【采药大师】"NOR;
			else  return WHT"【采 药 师】"NOR;
			case "smithing":
			budd=ob->query_skill("smithing",1);
			if (budd>220) return HIR"【大铸剑师】"NOR;
			if (budd>150) return HIR"【大锻造师】"NOR;
			if (budd>100) return HIR"【锻造大师】"NOR;
			if (budd>50)  return HIR"【锻 造 师】"NOR;
			else          return HIR"【 铁  匠 】"NOR; 
			case "mining":
			budd=ob->query_skill("mining",1);
			if (budd>250) return YEL"【天工神匠】"NOR;
			if (budd>150) return YEL"【大 工 匠】"NOR;
			if (budd>50) return YEL"【大采矿师】"NOR;
			else  return YEL"【采 矿 师】"NOR;
		}
	switch(gender) {
	case "女性":
		switch(wizhood(ob))
		...
		case "officer":
			if (ob->query("chaoting/wang")||ob->query("chaoting/jiawang")) return HIM"【 上柱国 】"NOR;
			if(pin == 1)	return HIM"【 柱  国 】"NOR;
			if(pin == 2)	return HIM"【 上护军 】"NOR;
			if(pin == 3)	return HIM"【 护  军 】"NOR;
			if(pin == 4)	return HIM"【轻车都尉】"NOR;
			if(pin == 5)	return MAG"【 骑都尉 】"NOR;
			if(pin == 6)	return MAG"【 骁骑尉 】"NOR;
			if(pin == 7)	return MAG"【 飞骑尉 】"NOR;
			if(pin == 8)	return MAG"【 云骑尉 】"NOR;
			else		    return MAG"【 武骑尉 】"NOR;
		case "huanxi":
			if(family != "大轮寺") break;
		...
		case "桃花岛":
			...
			if(budd >= 80)	return CYN"【 逸  士 】"NOR;
			else return CYN"【 青  衣 】"NOR;
		case "神龙教" :                                         
			  if (budd >= 350) return HIB"【 摄教主 】"NOR;   
			  if (budd >= 250) return HIB"【 长  老 】"NOR;   
			  if (budd >= 180) return BLU"【 五龙使 】"NOR;   
			  return BLU"【 教  徒 】"NOR;  
    case "星宿派" :
		...
		case "白驼山":
			...
			if(budd >= 350)	return HIW"【 大总管 】"NOR;
			if(budd >= 200)	return HIW"【 总  管 】"NOR;
			if(budd >= 80)	return WHT"【 管  事 】"NOR;
			return WHT"【 侍  女 】"NOR;
		case "华山派":
		...
		case "日月神教":
			...
			if(budd >= 200)	return RED"【 总  管 】"NOR;
			if(budd >= 150)	return RED"【 长  老 】"NOR;
			if(budd >= 100)	return RED"【 堂长老 】"NOR;
			return RED"【 女教众 】"NOR;
		case "大理段家":
	...
	//	case "天龙寺":
		...
		case "天地会":
			...
			if(budd >= 300)	return HIC"【 堂  主 】"NOR;
			if(budd >= 200)	return HIC"【 香  主 】"NOR;
			if(budd >= 100)	return HIC"【 山  主 】"NOR;
			return CYN"【 帮  众 】"NOR;
	//	
	//	case "雪山派":
		}
	...
	case "男性":
		...
		case "monk":
			if(family != "天龙寺") break;
			if(budd >= 1000)	return HIR"【 神  僧 】"NOR;
			if(budd >= 600)	return HIR"【 圣  僧 】"NOR;
			if(budd >= 200)	return RED"【 尊  者 】"NOR;
			if(budd >= 120)	return RED"【 禅  师 】"NOR;
			if(budd >= 40)	return RED"【 比  丘 】"NOR;
			if(age > 20)	return RED"【 僧  人 】"NOR;
			else 		return RED"【 小沙弥 】"NOR;
		case "taoist":
			if(family != "武当派") break;
	...
	//	case "星宿派":
		...
		case "明教":
			...
			if(budd >= 20)	return RED"【 教  众 】"NOR;
                        return RED "【 侍  从 】" NOR;
		case "日月神教":
			if(budd >= 600)	return HIR"【 代教主 】"NOR;
			if(budd >= 400)	return HIR"【光明使者】"NOR;
			if(budd >= 200)	return RED"【 总  管 】"NOR;
			if(budd >= 150)	return RED"【 长  老 】"NOR;
			if(budd >= 100)	return RED"【 堂长老 】"NOR;
    ...
    case "神龙教" :                                         
      if (budd >= 350) return HIB"【 摄教主 】"NOR;   
      if (budd >= 250) return HIB"【 长  老 】"NOR;   
      if (budd >= 120) return BLU"【 五龙使 】"NOR;   
      return BLU"【 教  徒 】"NOR;                  
