...
int do_buy(string arg)
{
        ...
        if ( !(file = read_file("/d/year2k/npc/obj/caipiao.c")) )
        {
                write("读文件出错。\n");
                return 1;
        }
        ...
        if (arg == "jingli")
        {
                numberfile = read_file("/d/year2k/obj/caipiao/jlcp");
                sscanf(numberfile,"%d",i);
                i++;
                write_file("/d/year2k/obj/caipiao/jlcp",sprintf("%d",i),1);
                strName = "精力彩票";
                  me->add("max_jingli",-30);                    write("你付出了30点最大精力的代价。\n");
        }
        else    if (arg == "neili")
                {
                        numberfile = read_file("/d/year2k/obj/caipiao/nlcp");
                        sscanf(numberfile,"%d",i);
                        i++;
                        write_file("/d/year2k/obj/caipiao/nlcp",sprintf("%d",i),1);
                        strName = "内力彩票";
                         me->add("max_neili",-30);
                        ...
                        {
                                numberfile = read_file("/d/year2k/obj/caipiao/rmcp");
                                sscanf(numberfile,"%d",i);
                                i++;
                                write_file("/d/year2k/obj/caipiao/rmcp",sprintf("%d",i),1);
                                strName = "容貌彩票";
                                me->add("per",-5);
                                write("你付出了5点容貌的代价。\n");
                        }
        ...
        seteuid(ROOT_UID);
        fName = "/d/year2k/obj/caipiao/" + me->query("id") + time() + ".c";
