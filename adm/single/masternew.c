...
int valid_override( string file, string name )
{
	...
	if(name=="shutdown"&&file=="/adm/simul_efun/object")
		return 1;
	if(name=="exec"&&file=="/adm/simul_efun/object")
		return 1;
        if(name=="write"&&file=="/adm/simul_efun/message")
return 1;
...
// valid_read: read privileges; called exactly the same as valid_read()
// shure add for pkuxkx 020325
