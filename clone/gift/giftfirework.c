...
void create()
{
        ...
        set("unit", "枚");
        set("long", HIC"这是一枚神奇的烟花，用来在小胖子和小希婚礼期间燃放(fire)。\n"NOR);
        set("value",0);
        set("no_store",1);
        setup();
}
...
int fire(object me)
{
	shout(this_object()->query("name") + HIR + "噌地一声腾空而起，直蹿云霄，\n紧接着爆出五彩斑斓的火花，将美丽的夜空点缀得绚丽多彩，也映亮了你喜庆的笑容。\n"NOR);
	shout(HIR+"火花中映出几个闪光的大字：\n"+NOR);
	shout(HIR+"恭祝"+BNK+HIG+"小胖子"+HIR+"和"+HIM+"小希"+NOR+HIR+"新婚快乐！\n" + NOR);
	return 1;
}
