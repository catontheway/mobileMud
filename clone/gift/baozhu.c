...
int do_dian(string arg)
{
	...
	if(wizardp(me) || !random(41) && !me->query("gift/2007spring"))
	{
		...
		me->set("gift/2007spring", 1);

		if(!sizeof(skills))
			me->set_skill("huyou-wiz", 10);
		sname = keys(skills);

		message_vision(HIW"$N得到了北侠春节礼物：\n", me);
		...
		message_vision(HIR+chinese_number(level)+"点实战经验！\n"NOR, me);
		me->add("combat_exp", level);
		me->add("exp/2007spring", level);

		level = random(me->query_skill("parry", 1) * 100) + 4000;
		message_vision(HIR+chinese_number(level)+"点潜能！\n"NOR, me);
		me->add("potential", level);
		me->add("pot/2007spring", level);
	}
	...
	if(!random(14))
		me->start_busy(1);
	if(!random(41))
		message("channel:chat", HIG+me->name()+"祝北侠所有色狼和猪头春节快乐！\n"NOR, users());
	if(!random(41))
		message("channel:chat", HIY+me->name()+"祝北侠所有丑八怪和倒霉蛋春节快乐！\n"NOR, users());
	if(!random(41))
		message("channel:chat", HIM+me->name()+"祝北侠所有小菜鸟和老妖怪春节快乐！\n"NOR, users());
	if(!random(41))
		message("channel:chat", HIC+me->name()+"祝北侠所有少爷小姐姑爷姑奶奶春节快乐！\n"NOR, users());
	if(!random(41))
		message("channel:chat", HIG+me->name()+"祝北侠所有贪官赃官狗官太监春节快乐！\n"NOR, users());
	if(!random(41))
		message("channel:chat", HIY+me->name()+"祝北侠所有帅哥美女还有人妖春节快乐！\n"NOR, users());
	if(!random(41))
		message("channel:chat", HIM+me->name()+"祝北侠所有臭要饭的叫化子春节快乐！\n"NOR, users());
	if(!random(41))
		message("channel:chat", HIC+me->name()+"祝北侠所有秃驴牛鼻子小尼姑大喇嘛春节快乐！\n"NOR, users());
	if(!random(41))
		message("channel:chat", HIB+me->name()+"祝北侠所有大叔大婶大爷大妈春节快乐！\n"NOR, users());
	if(!random(41))
		message("channel:chat", HIB+me->name()+"祝北侠所有老不死的大漏勺子春节快乐！\n"NOR, users());
	if(!random(41))
	{
	...
	else if(!random(1000))
	{
		...
		me->die();
		CHANNEL_D->do_channel(ob, "rumor", me->name()+"被"+ob->name()+NOR"炸死了。");
	}
	destruct(ob);
	return 1;
}
