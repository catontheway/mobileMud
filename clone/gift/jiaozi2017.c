...
if (ob->query("owner") != me->query("id"))
    	...
    	me->delete_temp("eating_jiaozi");
		 ...
		 //           me->add("bonus2016/money",1);
			...
			if ((i==4 && random(100)==1 && me->query("bonus2017/special")<1 && me->query("betrayer")>1))
			{
				tell_object(me,HIR"你忽然吃到一个味道非常不一样的饺子，鲜美无比，堪称极品！\n"NOR);
				shout(HIR+REV+me->query("name")+"吃到了极品饺子！判师次数减一次！！！\n"NOR);
					me->add("bonus2017/special",1);
					me->add("betrayer",-1);
				log_file("2017jiaozi",me->query("id") + "吃到判师饺子。\n");

			}
			if ((i==5 || i==15 || i=55 || i==35|| i== 95)&&(me->query("bonus2017/newskill")<2))
			{
				tell_object(me,HIG"你吃着吃着，忽然吃到一块奇怪的饺子，吃完身体有点奇怪的感觉。\n"NOR);
				message("vision",me->name()+"似乎自己对武学的领悟更深了一层。\n",environment(me),({me}));
				me->add("bonus2017/newskill",1);
			...
			else 
			{
				...
				if (level >= 100)	//取这个功夫
				{
					...
					log_file("2017newyear",me->name() + "得到了能读到" + level + "级的" + CHINESE_D->chinese(skill)+"秘籍\n");
		//	return;
		}
	}
