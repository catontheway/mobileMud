...
//              init_sockets();
        ...
        set_name(sword_name[type - 1], sword_id[type - 1] );
        set("weapon_id", parse_command_id_list());
        //if(!query_weight())
        set("base_weight", sword_weight[2*(type - 1)] + random(sword_weight[2*type - 1]));
        //if(!stringp(query("long")))
        set("base_long", sword_long[type - 1]);
        set("material", sword_material[type - 1]);
        set("rigidity", sword_rigidity[2*(type - 1)] + random(sword_rigidity[2*type - 1]));
        set("value", sword_value[type - 1]);
        //set("wield_msg", "$N从陈旧的剑鞘中拔出一把$n握在手中。\n");
        //set("unwield_msg", "$N将手中的$n插入腰间的剑鞘。\n");
        set("weapon/min_dam", sword_damage[2*(type - 1)]);
        set("weapon/max_dam", sword_damage[2*type - 1]);
