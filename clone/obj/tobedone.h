#include <ansi.h>
#include <title.h>
int to_do000(object me)//删除导师
{
	if (GROUP_D->group_member(me)<0)
	{
		if (member_array("group",me->query("multi_title"))!=-1)
			me->delete("multi_title");
		if (me->get_title(TITLE_GROUP)&&me->query("title")==me->get_title(TITLE_GROUP))
			me->delete("title");
		me->set_title(TITLE_GROUP);
	}
}
void to_do0(object me)  //删除定义为”“的title项
{
    mapping title;
    string* ty;
    int i;
    ...
    {
            if ((!me->get_title(TITLE_QUEST1)||strsrch(me->get_title(TITLE_QUEST1), "大清铁帽子王")==-1) && (!me->get_title(TITLE_QUEST2)||strsrch(me->get_title(TITLE_QUEST2), "大清铁帽子王")==-1) && 
                (!me->get_title(TITLE_QUEST3)||strsrch(me->get_title(TITLE_QUEST3), "大清铁帽子王")==-1))
            {
    ...
    // 补quest1的侠之大者title
    else if (me->query("statistics/newbie_target/guo_finish"))
    {
    ...
    // 补quest1的降龙伏虎title
    else if (me->query("statistics/newbie_target/zc_finish"))
    {
    ...
    {
        if (title[ty[i]] == "") me->set_title(ty[i]);
    }
}
...
int to_do16(object me)
{
	...
	guider=me->query("newbie/old_guider");
    if (guider!="")
    {
		if (ob=find_player(guider))
		{
			me->set("group_title_by",ob->query("group_title_by"));
		}
		else
		{
			...
			me->set("group_title_by",ent[i]["entry"]);
		}
	}
...
int to_do17(object me)
{
	if (!me->query("song/pro")||me->query("song/pro")=="宦"||time()<1367337600)
	return 1;
	if (me->query("ming_backup")&&me->query("song/mingpin")==0&&me->query("song/credit")<=0)
	{
		me->delete("song");
		me->set_title(TITLE_SONG1);
		me->set_title(TITLE_SONG2);
		me->set_title(TITLE_SONG3);
		me->set_title(TITLE_SONG4);
		me->set_title(TITLE_SONG5);
		tell_object(me,"之前程序错误造成你转换为大宋玩家，已经修正。\n");
	}
	return 1;
}
int to_do18(object me)
{
	if (!me->query("sucked_modified"))
	{
		me->set("sucked_modified",1);
		me->set("sucked_neili",0);
	}
	if (me->query("exercised_neili"))
	return 1;
	me->set("exercised_neili",me->query("max_neili")/2);
	return 1;
}
int to_do20(object me)
{
	if (me->query_skill("jiuyang-shengong"))
	{
		me->set_skill("guangming-shenghuogong",me->query_skill("jiuyang-shengong",1));
		me->delete_skill("jiuyang-shengong");
		tell_object(me,HIM+BLINK+"你的九阳神功已经被自动转化成光明圣火功，请自行enable。\n"NOR);
	}
	if (me->query_skill("shenghuo-shengong"))
	{
		me->set_skill("shenghuo-lingfa",me->query_skill("shenghuo-shengong",1));
		me->set_skill("dagger",me->query_skill("shenghuo-shengong",1));
		me->delete_skill("shenghuo-shengong");
		tell_object(me,HIM+BLINK+"你的shenghuo-shengong已经被自动转化成shenghuo-lingfa，请自行enable。\n"NOR);
	}
	return 1;
}
int to_do14(object me)
{
	...
	if (!me->query("ming"))
	return 1;
	if ((me->query("mingpin")<1||me->query("mingpin")>9)&&!me->query("ming/tj/pin"))
	return 1;
	me->set("ming_backup",me->query("ming"));
	me->set("ming_backup/mingpin",me->query("mingpin"));
	me->set("song",me->query("ming"));
	if (me->query("ming/tj/pin"))
	me->set("song/pro","宦");
	me->delete("mingpin");
	me->delete("ming");
	me->delete("song/juewei_1");
	me->delete("song/juewei_2");
	me->delete("song/juewei_3");
	me->delete("song/title");
	me->delete("song/promote_from_depart_position");
	if (gy=present("guan yin",me))
	destruct(gy);
...
int to_do21(object me)
{
	object trans;
	if (!trans=find_object("/d/hangzhou/npc/fuyin"))
	trans=load_object("/d/hangzhou/npc/fuyin");
	trans->transfer_skill(me);
	return 1;
}
int to_do22(object me)
{
	if (me->query("family/family_name")=="朝廷"&&me->query("family/master_name")=="多隆")
	{
		me->set("family/master_name","孟珙");
		me->set("family/master_id","meng gong");
	}
	return 1;
}

int to_do23(object me)
{
	if (me->query("family/family_name")=="天龙寺"&&me->query("family/generation")==15&&strsrch(me->get_title(TITLE_RANK),"代代弟子"))
	{
		me->set("title", GRN "天龙寺" NOR + YEL "第十五代弟子" NOR);
		me->set_title(TITLE_RANK, me->query("title"));
	}
	return 1;
}
int to_do24(object me)
{
	if (me->query_skill("forging",1))
	{
		if (me->query_skill("forging",1)<me->query("forging"))
		me->add("forging",me->query_skill("forging",1));
		else
		me->set("forging",me->query_skill("forging",1));
		me->delete_skill("forging");
		tell_object(me,"因为和新锻造系统的锻造技能容易混淆，现在把forging技能转化为记号，你可以正常使用。在becool npc处query_forge，查询点数。\n");
	}
	return 1;
}
int to_do25(object me)
{
	string name;
	if( !LOGIN_D->check_legal_name(COLOR_D->uncolor(me->query("name")))||COLOR_D->uncolor(me->query("name"))=="名字非法" )
    {
		if (me->query("gender")=="男性")
...
int to_do26(object me)
{
	object ob;
	if (time()-me->query("ANTI_MOB")>3600*6)
	{
		me->set("ANTI_MOB",time());
		ob=new(__DIR__"spec_ftools");
		ob->move(me);
		tell_object(me,"你获得了一个易容包，可以disguise改变身份。\n");
	}
	return 1;
}
int to_do_list()//gemlocker是每个人都有的东西，一些全面性的修改可以加在这里，不用全部放logind里
{
	...
	to_do17(me);//检查错误转换的大宋玩家
	to_do18(me);//增加execrised_neili属性，为后续开发做准备
	to_do20(me);
	to_do21(me);//朝廷转大宋
	to_do22(me);//朝廷转大宋补
	to_do23(me);//修正天龙寺名称错误
	to_do24(me);//把skill forging换成query(forging)和新锻造系统避免冲突。
	to_do25(me);//改名
	if (time()>1422709200&&time()<1423321200)
	//to_do26(me);//反黑社会
