...
int accept_fight(object ob)
{
	...
	if (file_name(environment())!="/d/taishan/zhengqi")
		return notify_fail("罚恶使者现在身有要事，等他回了泰山再说吧。\n");
...
int checking(object me, object ob)
{
	...
	if (( (int)me->query("qi")*100 / my_max_qi) <= 30 ) {
		...
		CHANNEL_D->do_channel(this_object(), "jh", "恭喜" + ob->query("name") + "被推举为现任武林罚恶使者！");
...
int do_recover()
{
	...
	if ( me->query("mud_age") > ob->query("mud_age"))
                return notify_fail("你想用 bug 谋利，呵呵！\n");
     if ( ob->query("pked/be_pked") == "fae" || ob->query("pked/be_pked") == "none" )
               {
