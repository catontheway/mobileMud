...
#define FACTOR 15 //系数 Zine e.g. 16既为1.6倍,如需调整2倍奖励，则factor 20.
...
// 提高任务奖励。by lordstar 20171016
        ...
        exp=exp+extra*(680+random(21));
        pot=pot+extra*(500+random(16));
        rep=rep+extra*10;
...
//过滤非法任务地点和不明区域 by segate@pkuxkx
	...
	//	write("目标任务类型："+ob->query("task_type")+"\n");
		...
		if (ob->query("task_type")==8)
		{
            ...
            message_vision(HIW"$N随意指了指地图上"+ area +"的"+location+ "，说道：韩家仇人"ZJURL("cmds:tsdx "+ob->query("id"))+ob->query("name")+"("+ob->query("id")+")"+NOR"在此出现，$n"+HIW+"去把他宰了！\n" NOR,this_object(),me);
            map=gene_area(area);
		...
		//	write("进入再次图片模式："+ob->query("task_type")+"\n");
                ...
                tell_object(me,HIW"韩员外告诉你，此獠将在一分钟后在"+ area +"的"+location+ "出现。\n");
            ...
            tell_object(me,HIW"韩员外说道：韩家仇人"ZJURL("cmds:tsdx "+ob->query("id"))+ob->query("name")+"("+ob->query("id")+")"+NOR+"据传将在" + area +"的"+location+ "出现，你去将其狙杀！\n" NOR);
            tell_object(me,MAG+"其人心思缜密，你不到最后五秒不要现身，否则打草惊蛇，悔之晚矣。但也不能迟过他出现的时分，否则其人脱困而去。\n"+NOR);
            return 1;
        }
        if (ob->query("task_type")==6)
        {
			...
			tell_object(me,HIW"韩员外说道：韩家仇人"+ZJURL("cmds:tsdx "+ob->query("id"))+ob->query("name")+"("+ob->query("id")+")"+NOR+"据传在" +location+ "三处出现过，快去找到他帮我杀了他！\n" NOR);;
			me->set_temp("hyw/where","某处");
		...
		//	write("进入再次图片模式："+ob->query("task_type")+"\n");
            ...
            tell_object(me,HIW"韩员外说道：韩家仇人"+ZJURL("cmds:tsdx "+ob->query("id"))+ob->query("name")+"("+ob->query("id")+")"+NOR+"据传在『" + TASK_D->place_belong(where) + "』出现过，快去帮我杀了他！\n" NOR);
            me->set_temp("hyw/name",ob->query("name"));
            me->set_temp("hyw/where",TASK_D->place_belong(where));
            intro(me,ob);
			if (ob->query("task_type")==5)
