...
int accept_fight(object ob)
{
	...
	if (file_name(environment())!="/d/taishan/xiayi")
		return notify_fail("赏善使者现在身有要事，等他回了泰山再说吧。\n");
...
int checking(object me, object ob)
{
	...
	if (( (int)me->query("qi")*100 / my_max_qi) <= 50 ) {
		...
		CHANNEL_D->do_channel(this_object(), "jh", "恭喜" + ob->query("name") + "被推举为现任武林赏善使者！");
...
int do_recopy(object me, object ob)
{
	...
	if ( me->query("mud_age") > ob->query("mud_age"))
          ...
          {}
         else  ob->set("pked/be_pked","shangshan");
...
/* copy entire dbase values */
	...
	tell_object(ob, "状态储存完毕。\n");
        ...
        newob = new("/clone/npc/shang-shan");
...
int do_recover()
{
	...
	if ( me->query("mud_age") > ob->query("mud_age"))
      ...
      if ( ob->query("pked/be_pked") == "shangshan" || ob->query("pked/be_pked") == "none" )
               {
