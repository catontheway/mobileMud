...
void create()
{
	...
	carry_object("/d/city/obj/cloth")->wear();
                ...
                set("id", "feilong bangzhu");
                set_name(query("name"), ({"feilong bangzhu", "bangzhu", "feilong" }));
		            setup();
...
int accept_fight(object ob)
{
	...
	if (file_name(environment())!="/d/city/feilong-bang")
		return notify_fail("飞龙帮主现在身有要事，等他回了飞龙帮总部再说吧。\n");
...
int checking(object me, object ob)
{
	...
	if (( (int)me->query("qi")*100 / my_max_qi) <= 50 ) {
		...
		CHANNEL_D->do_channel(this_object(), "jh", "恭喜" + ob->query("name") + "被推举为新一任飞龙帮帮主！");
...
int do_recopy(string arg)
{
	...
	if ( me->query("mud_age") > ob->query("mud_age"))
          ...
          {}
          else  ob->set("pked/be_pked","feilong");
	me->set("name",  ob->query("name") );
	me->set("title", "第" + chinese_number(me->query("generation")) + "代飞龙帮帮主");
	me->set("short", me->query("title") + " " + me->query("name") + "(" + capitalize(ob->query("id")) + ")");
	me->delete("title");
...
/* copy entire dbase values */
	...
	tell_object(ob, "状态储存完毕。\n");
        ...
        newob = new("/clone/npc/feilong");
        newob->move("/d/city/feilong-bang");
	destruct(me);
...
//icer added for pkers

            if ( ob->query("pked/be_pked") == "feilong" || ob->query("pked/be_pked") == "none" )
      {
