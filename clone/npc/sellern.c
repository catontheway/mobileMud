...
void greeting(object ob)
{
        ...
        message_vision( "殷老板用低沉的声音向$N问道：这位" + RANK_D->query_respect(ob)
                + "，想买小动物吗？\n找我就对了，可以用ding <type> <color> <name> <English name>。
                \n殷老板偷偷对$N说道：这位" + RANK_D->query_respect(ob)
                + "，如果想复活你的小宠物吗？找我就对了，用hehuo、relive\n", ob);
                殷老板偷偷对$N说道：这位" + RANK_D->query_respect(ob)
                + "，如果想复活你的小宠物吗？找我就对了，用hehuo、relive\n", ob);
...
int do_fuhuo()
{
        ...
        if (!(0<(int)me->query("pet_died")<86001))
                return notify_fail("你宠物活的吧。\n");
        gold = present("gold_money", this_player());
        if( !gold )
                return notify_fail("你身上没有金子。\n");
        if( (int)gold->query_amount() < 500 )
                return notify_fail("你身上没有那麽多金子。\n");
        
        gold->add_amount(-50);
        me->set("pet_died", 86001);
        me->save();
        if( me->query("created_pet") )
        {
        tell_object(me, PET_D->create_pet(me));
        message_vision(this_object()->name() + "从笼子里抱出" + pet->query("unit")
                        + pet->name() + "交给$N，叮嘱道：“可要好好照顾啊！”\n", me);
        	return 1;
         }     
        return 0;
