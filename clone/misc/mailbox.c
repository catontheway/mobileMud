...
void send_mail(string rcvr, mapping mail)
{
	...
	if (sizeof(mails) > MAX_MAILBOX_SIZE)
	{
		write("你的邮箱已满，请清理旧邮件后再发送。\n");
		return;
	}
	...
	if (objectp(ob = find_player(rcvr)))
	{
			...
			if (mbx->query_size() > MAX_MAILBOX_SIZE)
			{
				write("对方邮箱已满，邮件发送失败。请告知对方及时清理旧邮件。\n");
				return;
			}
...
int do_from()
{
	...
	for(i=0; i<sizeof(mails); i++)
		msg = msg + sprintf("%2d %-50s 寄信人：%-20s\n",
