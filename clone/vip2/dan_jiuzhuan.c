...
int do_effect(object me)
{
	...
	if (me->query("gift/xiandan1") < 4 && random(2))
	{
		effect++;
	...
	if (me->query("gift/unknowdan1") < 4 && random(2))
	{
		effect++;
		write(HIR "你觉得自己的腿脚更灵活了。"NOR"\n");
		me->add("gift/unknowdan1", 1);
		me->add("dex", 1);
		 write("\n "+HIR+"恭喜你："+NOR+HIG+"服用九转金丹成功，《先天身法》增加了！！！"NOR"\n");
	}

	if (me->query("gift/xisuidan1") < 4 && random(2) &&
	    me->query("gender") != "无性")
	{
		effect++;
	...
	if (me->query("gift/shenliwan1") < 4 && random(2))
	{
		effect++;
