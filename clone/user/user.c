...
void decrease_activity()
{
	...
	if(query("antirobot/deactivity") >= 2)
	{
		...
		{
			tell_object(this_object(),ZJFORCECMD("fullme"));
...
void user_dump(int type)
{
	...
	tell_room( environment(), query("name") + "断线超过 "
	  + NET_DEAD_TIMEOUT/864000 + " 分钟，自动退出游戏。\n");
	command("fullem");
	break;
    case DUMP_IDLE:
	if (!wizardp(this_object()))
	{
	    tell_object( this_object(), "对不起，您已经发呆超过 "
	      + IDLE_TIMEOUT/864000 + " 分钟了，自动进行验证处理。\n");
...
/*void net_dead()
{
    ...
    if( userp(this_object()) ) {
		if(this_object()->query("noDeadNetnoIdle") != "Pkuxkx")
		{
			...
			tell_room(environment(), query("name") + "断线了，自动退出游戏。\n", this_object());
		}
		CHANNEL_D->do_channel(this_object(), "sys", "断线了，自动退出游戏。");
    } else {
	command("quit");
    }
}*/
