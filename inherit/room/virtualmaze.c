...
class coordinate *newpath = ({}),/*待处理队列*/
 ...
 string *valid_dirs = ({ "south","north","west","east" });
 mapping reverse_dir = ([
"north" : "south",
"south" : "north",
"west"  : "east",
...
// 选择随机出口.
 int random_out(int x,int y,int n);

// 处理连接.
 void link_to_north(int x,int y);
 void link_to_south(int x,int y);
 void link_to_west(int x,int y);
 void link_to_east(int x,int y);

// 绘制已建成迷宫的地图.
 void paint_vrm_map();

 string mroom_fname(int x,int y)
{ return sprintf("%s/%d/%d",base_name(this_object()),x,y);}


 void refresh_vars() // 重置全域变量.
{
	newpath = ({});
	all = 0;
}
...
*/
 ...
 int random_out(int x,int y,int n) // 选择随机出口函数.
{
	int *outs = ({}), retn = 0;
...
void create_maze()
{
 ...
 void link_to_west(int x,int y)	// The west room is (x-1,y)
{
	class coordinate temp;
 ...
 void link_to_east(int x,int y)	// The east room is (x+1,y)
{
	class coordinate temp;
 ...
 void link_to_south(int x,int y)	// The south room is (x,y-1)
{
	class coordinate temp;
 ...
 void link_to_north(int x,int y)	// The north room is (x,y+1)
{
	class coordinate temp;
