...
// 增加售卖物品折价机制。
// 初步方案：累计售卖500件以上收购价90%，800件80%，1200件70%，1800件60%，2500件50% 
// by lordstar@20171012
...
nosave int all_sell_number = 0; //累计共卖掉了多少件装备，
...
void init()
{
	...
	add_action("do_discount", "discount");
	add_action("do_show_discount", "show_discount");
...
object findrbzmaster()
{
	object *allobj;
	int i;
	object env = find_object("/d/city/wumiao");
	if (!env) return 0;
	allobj= all_inventory(env);
	for (i=0; i<sizeof(allobj); i++)
	{
		if (allobj[i]->query("id")== "rbz master" && allobj[i]->query("discount")) return allobj[i];
	}
	return 0;
//	return present("rbz master", env);
}

// 打折情况记录在 rbzmaster npc 身上。 by lordstar@20171012
int discount()
{
	object rbzmaster;
	int ds;
	rbzmaster = findrbzmaster();
	if (objectp(rbzmaster))
	{
		ds = (int)rbzmaster->discount();
		all_sell_number = (int)rbzmaster->query("rbz_all_sell_number");
	}
	if (ds==0) ds=10;
	else if (ds<5) ds=5;
	return ds;
...
int discount_value(int value)
{
	return value*discount()/10;
}

int do_show_discount()
{
	object rbzmaster;
	...
	{
		tell_object(me, HIM"没有找到存储数据的NPC……使用本地数据。\n"NOR);
	}
	tell_object(me, HIG "现已售卖随机装备 "+all_sell_number+" 件，售价折扣为 "+discount()+"0%。\n" NOR);
	return 1;
}

void save_all_sell_number()
{
	object rbzmaster;
	rbzmaster = findrbzmaster();
	if (objectp(rbzmaster))
		rbzmaster->set("rbz_all_sell_number", all_sell_number);
}
...
int do_discount()
{
	...
	int dv; 
	dv = discount();
	cdv = HIY + chinese_number(dv) + "折" + NOR;
	if (dv>=10) write("掌柜拱手说道：「本店财大气粗不差钱，现原价收购各种奇珍异宝，欢迎各位英雄赏脸前来交易！」\n");
	else write("掌柜说道：「本店已收购各式奇珍" HIM +chinese_number(all_sell_number)+ NOR "件，多得库房都放不下了！按说早该关门歇业的，不过……」\n"
		+"他笼在袖中的双手飞快地掐指一算，又故作矜持说道：「看在多年主顾份上，也不是不能通融一下。这样吧！」\n"
		+"他伸出手来比了个手势，道：「打"+cdv+"！按着这个折扣，如果客官愿意割让，我就吩咐伙计再腾些库房来收购。」\n");
	return 1;
}
		...
		else
		{
			    ...
			    for(i = 0; i < MAX_PAWN; i++)
        	{
        		...
        		if(stuff->restore())
				{
					...
					switch(stuff->query("identified"))
					{
						case 1:
							list += sprintf("│%4d│%-32s│%|8s│%|6d│%-36s│\n", i, tmp, "?", hole, value_string(vpawn[sprintf("%d", i)]* 1/5) );
							break;
						case 2:
							list += sprintf("│%4d│%-32s│%|8d│%|6d│%-36s│\n", i, tmp, effect, hole, value_string(vpawn[sprintf("%d", i)]* 1/5) );
							break;
						default:
							list += sprintf("│%4d│%-32s│%|8s│%|6s│%-36s│\n", i, tmp, "?", "?", value_string(vpawn[sprintf("%d", i)]* 1/5) );
					}
...
int sell_and_move_to_box(object ob)
{
	...
	if (sell_index >= MAX_SELL)     //砍掉最老的 MAX_SELL/2个物品
	{
		for (i = 1; i <= half; i++)
		{
			...
			if ( objectp(old_object) )
            {
                if(old_object->query("forge/max")==3 || ((old_object->query("armor_type")=="finger" || old_object->query("armor_type")=="neck") &&
                    old_object->query("forge/max")==2))
                {
                    CHANNEL_D->do_channel(this_object(), "rumor", "荣宝斋处理掉了一"+old_object->query("unit")+
                                          "可塑性为"+NOR+HIY+chinese_number(old_object->query("forge/max"))+NOR+HIM+"的"+NOR+old_object->query("name")+HIM+"！"+NOR);
                }
                destruct(old_object);
                box->delete("goods/" + i);
			}
...
int do_sell(string arg)
{
	...
	if(arg == "all") //全卖，嘿嘿
	{
		...
		if(j)
		{
			...
			discount = discount_value(allvalue);
			this_player()->start_busy(1);
			message_vision("$N把身上的" + goods + "一共" + chinese_number(j) + "件物品" + (discount==allvalue? "" : "以"+ chinese_number(discount()) +"折售价")
				+ "卖掉，当" + value_string(discount) + "。\n", this_player());
			pay_player(this_player(), discount );
		}
		else
	...
	write("掌柜的唱道："+ sell_msg[random(sizeof(sell_msg))] +
		ob->query("name") + "一" + ob->query("unit") + "，" + (discount==allvalue? "" : chinese_number(discount()) +"折")
		+ "当"+ value_string(discount)+ "。\n");
	...
	if (stringp(arg) && (member_array(arg, weapon) != -1 || member_array(arg, armor) != -1))
		{
			    ...
			    for(j = i; j <= MAX_SELL; j++)
        	{
        		...
        		if(goods->query("skill_type") == arg || goods->query("armor_type") == arg)
				{
					...
					switch(goods->query("identified"))
					{
						case 1:
							list += sprintf("│%4d│%-32s│%|8s│%|8d│%-36s│\n", j, tmp, "?", hole, (hole==1)?value_string(goods->query("value")*80/100):value_string(goods->query("value")) );
							break;
						case 2:
							list += sprintf("│%4d│%-32s│%|8d│%|8d│%-36s│\n", j, tmp, value, hole, (hole==1)?value_string(goods->query("value")*80/100):value_string(goods->query("value")) );
							break;
						default:
							list += sprintf("│%4d│%-32s│%|8s│%|8s│%-36s│\n", j, tmp, "?", "?", value_string(goods->query("value")) );
					}
        		...
        		if(goods->query("weapon_prop"))
       			{
					...
					switch(goods->query("identified"))
					{
						case 1:
							...
							break;
						case 2:
							list += sprintf("│%4d│%-32s│%|8d│%|8d│%-36s│\n", j, tmp, goods->query("weapon_prop/damage"), hole, (hole==1)?value_string(goods->query("value")*80/100):value_string(goods->query("value")) );
							break;
						default:
							list += sprintf("│%4d│%-32s│%|8s│%|8s│%-36s│\n", j, tmp, "?", "?", value_string(goods->query("value")) );
					}
        		...
        		if(goods->query("armor_prop"))
       			{
					...
					switch(goods->query("identified"))
					{
						case 1:
							...
							break;
						case 2:
							list += sprintf("│%4d│%-32s│%|8d│%|8d│%-36s│\n", j, tmp, goods->query("armor_prop/armor"), hole, (hole==1)?value_string(goods->query("value")*80/100):value_string(goods->query("value")) );
							break;
						default:
							list += sprintf("│%4d│%-32s│%|8s│%|8s│%-36s│\n", j, tmp, "?", "?", value_string(goods->query("value")) );
					}
			    ...
			    for(j = i; j <= MAX_SELL; j++)
        	{
        		...
        		tmp = COLOR_D->uncolor(name);
        		list += sprintf("│%4d│%-32s│%-34s│\n", j, tmp,
                                (goods->query("forge/max")==1 && goods->query("identified"))?value_string(goods->query("value")*80/100):value_string(goods->query("value")) );
        		list = replace_string(list, tmp, name);
        	}
        	list +="└──┴────────────────┴─────────────────┘\n";
			}			
...
int do_buy(string arg)
{
	...
	{
		value = (ob->query("forge/max")==1 && ob->query("identified"))?(ob->query("value")*80/100):ob->query("value");
