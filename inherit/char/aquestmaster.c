...
/*        if (me->query("combat_exp",1)>35000000)
	{
...
// Let's see if this player still carries an un-expired task
    ...
    {
if(wizardp(me)) write(levels[j]+"  "+ me->query("combat_exp") +"\n");
...
if(wizardp(me))
tell_object(me,sprintf("tag=%s,j=%d\n",tag,j));
	wsquest = AQUEST_D(tag)->query_quest();
//	to use less mem...
	timep = wsquest["time"];	
...
int accept_object(object who, object ob)
{
	...
	if (val)
	{
		tell_object(who,sprintf("%s",this_object()->name()+"严厉的说：你想贿赂我？这岂是正派弟子所为？\n"));
		return 0;
	}
