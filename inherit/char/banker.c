...
if (!arg)
		...
		if (sscanf(arg, "%d %s", amount, what) != 2)
	{
		if(arg=="cash")
		{
			if((money_limit=me->query("balance")/100000000)<1)
				return notify_fail(ZJOBLONG"你在本店的存款不足一张千两银票，不能支取！\n");
			type = "张百万两银票";
		}
		else if(arg=="cash")
		{
			if((money_limit=me->query("balance")/10000000)<1)
				return notify_fail(ZJOBLONG"你在本店的存款不足一张千两银票，不能支取！\n");
			type = "张十万两银票";
		}
		else if(arg=="cash")
		{
			if((money_limit=me->query("balance")/1000000)<1)
				return notify_fail(ZJOBLONG"你在本店的存款不足一张千两银票，不能支取！\n");
			type = "张万两银票";
		}
		else if(arg=="cash")
