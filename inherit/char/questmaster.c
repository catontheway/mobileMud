...
int give_wsquest(string arg)
{
	...
	{
		command("say 你的经验太低了，最好涨点经验再来做任务！");
		return 1;
	}
...
int accept_object(object who, object ob)
{
	...
	if (val)
	{
		tell_object(who,sprintf("%s",this_object()->name())+"生气的说：难道你想贿赂我吗？\n");
		return 1;
	}
	...
	if ((int) who->query("task_time") < time() ){
		...
		exp = wsquest["exp_bonus"] + random(wsquest["exp_bonus"]/2);
		pot = wsquest["pot_bonus"] + random(wsquest["pot_bonus"]);
		if (who->query("wsquest_number")/4<3)
		{
