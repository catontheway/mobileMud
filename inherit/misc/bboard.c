...
string long()
{
	...
	msg += sprintf("%s[%2d]" "  %-40s %12s (%s)\n"NOR,
	  ( notes[i]["time"] > last_time_read ? HIB: ""),
         i+1, notes[i]["title"], notes[i]["author"] + "" + "" + "", ctime(notes[i]["time"]) );
...
int do_post_afterwarn(string arg, string title)
{
	mapping note;
	...
	if (arg == "y")
	{
	    ...
	    note["title"] = replace_color(title);
...
int do_post(string arg)
{
    ...
    if (query("warning_msg"))
    {
    	tell_object(this_player(),query("warning_msg"));
    	if (query("warning_msg_choice"))
    		input_to("do_post_afterwarn",arg);
    	else do_post_afterwarn("y",arg);
    } else {
	    note = allocate_mapping(4);
