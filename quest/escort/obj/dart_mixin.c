...
void create()
{
  set_name( HIW"密信"NOR , ({"mixin","letter", "xin"}));
  set_weight(500);
  ...
  if (clonep())
    ...
    set("unit", "张");
    set("value", 0);
    set("long","这是一张都大锦给你的密信，需要用特殊的药水才能显形(xian)。\n");
    set("no_store", 1);
    set("no_sell", 1);
    set("no_pawn",1);
    set("no_put",1);
...
int tixing()
{
    ...
    if (environment(ob))
    {
        tell_object(environment(ob),BLINK HIC"你还没有去追回"+this_object()->query("from_name")+"的财物呢。\n"NOR);
    }
...
void init()
{
   ...
   {
        if ( all[i]->query("id")=="mixin" &&
        	   all[i]->query("from_id") &&
        	   this_object()->query("from_id") == all[i]->query("from_id") )
        {
            count=count+1;
            if (count==2)
            {
...
//        where=TASK_D->random_place();
        ...
        {
	        tell_object(me,"你按照"+this_object()->query("from_name")+"的指示，信上的字却没有显现，再试试吧。\n");
	        return 1;        	
        }
        ...
        write(HIW"你按照"+this_object()->query("from_name")+"的指示，打开迷信……原来要你到以下地方"+fullplace+"找(zhao/zhaoxun)到潜逃的伙计并要(yao)回其席卷的财物！\n"NOR);        
...
int do_dig()
{
    ...
    {
            huoji=new("/quest/escort/escort_huoji");
            if (huoji)
            {
                huoji->set("owner",me->query("id"));
                huoji->set("from_id", ob->query("from_id"));
                huoji->set("title",CYN"卷走"+ob->query("from_name")+"财物的伙计"NOR);
