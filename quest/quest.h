...
{	
	...
	if(me->query("quest/quest")==inv[i]->query("name")&&me->query("quest/quest_type")=="寻"
	&&me->query("task_time")>=(int)time()&&inv[i]->query("no_give")) {
		message_vision("$n高兴的迎上前，"+me->name()+",你辛苦了。\n",me,this_object());
		message_vision("$n接过了$N的"+inv[i]->query("name")+"。\n",me,this_object());
...
score=1+random(4);
             ...
             if(me->query("quest_lianxu") >= 7)
               me->set("quest_lianxu",4);
             quest_lianxu = me->query("quest_lianxu");  
             me->set("quest_lianxu",++quest_lianxu);
             reward = quest_lianxu*(int) me->query("quest/score")*(num_online/6+2);  
             me->add("score",score);  
...
int give_quest()
{
	...
	if(me->query("family/family_name")!=obj->query("family/family_name"))
        ...
        if( (me->query("score")<0?-1:1)*me->query("score") * me->query("score") >= 
