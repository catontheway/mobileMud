...
void killer_reward(mixed killer, object victim)
{
    ...
    if(objectp(killer) && userp(killer)) victim->add("fhost",1); // 被玩家就地正法，fhost value加一
    if(objectp(killer) && !userp(killer)) victim->add("ghost",1); // 被NPC就地正法，ghost value加一
...
//被动行凶5分钟之内不增加杀气， by all@2015/1/21
        if(userp(killer) && userp(victim) && !environment(victim)->query("no_die")
  ...
  {        
      ...
      && !(environment(killer)->query("no_die")))
    {
      ...
      if (( time()-killer->query_temp("last_add_shaqi"))<10)
        shaqi=shaqi*2;      
//      if (userp(victim))
	if (userp(victim)&& time()-killer->query("killer/time")<10)//被动行凶5分钟之内不增加杀气， by all@2015/1/21
    ...
    if (objectp(killer) && victim->isvalid_death_reason()==1) 
    {
      ...
      {
        //杀手帮行凶不留姓名
        victim->set("last_killed/killer","被"+killer->query("name")+"(...)就地正法");
      }
      else
      {
        victim->set("last_killed/killer","被"+killer->query("name")+"("+killer->query("id")+")就地正法");
      }
    }
   ...
   /* {
    ...
    }*/
	{
	  ...
	  else if(victim->query("xkxvip/times") > 86400)
      {
        victim->add("xkxvip/times",-86400);
        tell_object(victim, HIY"你触发了扣除一天会员时间，死亡无损失为！当前会员时间剩余"+victim->query("xkxvip/times")+"\n"NOR);
      }
      //有athanasy特技的player只有一半的几率掉经验，潜能 -by whuan 2007-8-26
      //update by yhzzyahoo
      else 
...
*/			
    ...
    if( objectp(killer) && userp(killer) && killer->query_temp("apply/name") )
    {
        ...
        {
            CHANNEL_D->do_channel(this_object(), "rumor",
              sprintf("有人看到就地正法%s的是" + killer->query_temp("apply/name")[0] + "。",victim->name(1)));
        }
    ...
    if (objectp(killer) && victim->isvalid_death_reason()==1 && !killer->query_temp("apply/name"))
      CHANNEL_D->do_channel(this_object(), "rumor",
      sprintf("%s被%s就地正法了"+(real_die?"！":"。"), victim->name(1), killer->name(1)));
    else if (objectp(killer) && victim->isvalid_death_reason()==1 && killer->query_temp("apply/name"))
      CHANNEL_D->do_channel(this_object(), "rumor",
      sprintf("%s被%s就地正法了"+(real_die?"！":"。"), victim->name(1), killer->name()));
    else if(stringp(killer))
      CHANNEL_D->do_channel(this_object(), "rumor",
      ...
      && !(environment(killer)->query("no_die")))
    {
      ...
      {
          shaqi=shaqi+200;
          // tell_object(killer, HIW"就地正法master，额外获得杀气200/10!\n"NOR);
      }
      else if (!ANTIROBOT_D->fullme_invalid() && ( check_victim_value(victim) || victim->query_temp("is_rich")))
      {
          shaqi=shaqi+200;
          // tell_object(killer, HIW"就地正法有钱的主，额外获得杀气200/10!\n"NOR);
      }
