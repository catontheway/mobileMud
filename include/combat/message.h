...
varargs void report_status(object ob,int nouse)
{
    ...
    if(ob->query("newbie_village/done") || ob->query("xuetang") || ob->query("combat_exp") >= 2000)
    {
        ...
        {
            msg1 = ((ob->query_temp("combat/qi_damage")||ob->query_temp("combat/jing_damage"))?"受到伤害:"+ob->query_temp("combat/qi_damage")+"/+"+ob->query_temp("combat/jing_damage"):"");
            msg2 = ((ob->query_temp("combat/qi_wound")||ob->query_temp("combat/jing_wound"))?" 受到创伤:"+ob->query_temp("combat/qi_wound")+"/+"+ob->query_temp("combat/jing_wound"):"");
            msg3 = " 】气血:"+qipct+"%/"+efqipct+"% 精神:"+jingpct+"%/"+efjingpct+"%"NOR"』\n";
        }
        else if( (!ob->query_temp("combat/jing_damage")&&!ob->query_temp("combat/jing_wound"))
                && (ob->query_temp("combat/qi_damage")||ob->query_temp("combat/qi_wound")) )
        {
            msg1 = ((ob->query_temp("combat/qi_damage"))?"受到气血伤害:"+ob->query_temp("combat/qi_damage"):"");
            msg2 = ((ob->query_temp("combat/qi_wound"))?" 受到气血创伤:"+ob->query_temp("combat/qi_wound"):"");
            msg3 = "】 气血:"+qipct+"%/"+efqipct+"%"NOR"』\n";
        }
        else if( (!ob->query_temp("combat/qi_damage")&&!ob->query_temp("combat/qi_wound"))
                && (ob->query_temp("combat/jing_damage")||ob->query_temp("combat/jing_wound")) )
        {
            msg1 = ((ob->query_temp("combat/jing_damage"))?"受到精神伤害:"+ob->query_temp("combat/jing_damage"):"");
            msg2 = ((ob->query_temp("combat/jing_wound"))?" 受到精神创伤:"+ob->query_temp("combat/jing_wound"):"");
            msg3 = " 】精神:"+jingpct+"%/"+efjingpct+"%"NOR"』\n";
        }

        if (ratio==efqipct || ratio==efjingpct)
        {
            combat_msg( "( $N" + eff_status_msg(ratio) + " )\n""『"+ob->name()+YEL"【" + msg1 + msg2 + msg3, 3, ob);
        }
        else
        {
