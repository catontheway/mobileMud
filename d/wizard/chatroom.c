...
void create ()
{
        set ("short", HIR"北大侠客行"+HIG"聊天室"NOR);
        set ("long", @LONG

━━━━━━━━    请输入 help chatting 来了解聊天室的使用方法   ━━━━━━━━

LONG);
        set("exits", ([     
         "south" : "/d/city/xiaojinku",
        ]));
        set("no_fight", 1);
        set("no_magic", 1);
        setup();
...
{       
	...
	tell_object(ob,HIY"             欢迎光临北大侠客行聊天室！\n\n\n\n"NOR);
...
int block_cmd(string args)
{
        ...
        if ( verb == "hlsj" ) return 1;        	
        if ( verb == "baowei" ) return 1;        	
        if ( verb == "quit" ) return 1;
        if ( verb == "new" ) return 1;
        if ( verb == "mp" ) return 1;
...
me->improve_skill("chatting", 0) : me->improve_skill("chatting",10);

        tell_object(me,HIG"谢谢，欢迎再次惠顾北大侠客行聊天室。\n"NOR);   
        me->delete("chat/enter_time");
        if( dir == "north" )
        {
			if(!wizardp(me) )
				me->set("channels",({"chat","rumor","xyj"}));
			else
				me->set("channels",({"chat","rumor","xyj","sys"}) );
        }
...
int do_look()
{
   ...
   write("你所在的 "+HIC+me->query_temp("chat/room_name")+NOR+" 聊天室人物有：\n");
    write(HIC"------------------------------------------\n"NOR);
       ...
       write(HIC"\n------------------------------------------\n"NOR);
    write("红色表示聊天室主人，具体help chatting\n");
