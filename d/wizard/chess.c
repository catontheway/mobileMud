...
void create ()
{
        set("short", "品棋亭");
        set("long", @LONG
品棋亭南眺泾水，北靠长安高墙，虽不是四野开阔，却也是手谈休栖的
好地方。亭中有一张石桌(Table)，上面划着一张围棋盘。桌角摆放着棋子。
另外还有两张石凳(Seat)，颜色一深一浅。
LONG);
...
int do_new(string arg)
{
        ...
        for( i=0;i<bsize;i++ )
                for( j=0;j<bsize;j++ )
                        game[i*bsize+j]=WQ_BLANK;
        if( handicap!=0 ) {
                if( handicap%2 ) {
                        game[9*19+9] = WQ_BLACK;
                        for( i=0;i<handicap-1;i++ ) game[h_list[i]]=WQ_BLACK;
                } else
                        for( i=0;i<handicap;i++ ) game[h_list[i]]=WQ_BLACK;
                turn = "white";
        }
        WQ_Started=0;   
        WQ_Undoed=0;
...
//      if(pl["black"] || pl["white"]) return 1;
        ...
        if( turn=="black" )
                ...
                tell_object(me, "现在轮到白方"+player->name()+"走棋。\n");
        return 1;
}
...
int do_undo()
{
        ...
        if(turn=="black")
                ...
                tell_room(rm,"现在轮到白方"+player->name()+"走棋。\n");
...
int do_play(string arg)
{
        ...
        if(wf) {
                tell_room(rm,show_game());
                if(turn=="black")
                        message_vision("黑方$N胜。\n",me);
                else
                        message_vision("白方$N胜。\n",me);
                status=WQ_NOT_PLAYING;
                return 1;
        }
        if(turn=="black") {
