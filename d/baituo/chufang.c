...
LONG
	...
	set("objects", ([ 
	    ...
	    __DIR__"npc/yahuan" : 2,
	 	...
	 	set("item_desc", ([
"厨房" : "这是白驼山庄的厨房，里面有一个大灶和一口水缸\n"
		 ZJOBACTS2+ZJMENUF(2,2,12,40)+"喝水:drink"+ZJSEP+"食物:order\n",
    ]));
	set("cost", 0);
	setup();
}
...
int do_drink(string arg)
{
	...
	else write("丫环奇怪地瞪着你，慢慢地咧开嘴，捧腹大笑起来：肚子都大了还喝！\n");
...
int do_order()
{
	...
	if(!f) {
	   switch(random(4)) {
	   case 0:
		ob1 = new("/clone/food/baozi");
		break;
	   case 1:
		ob1 = new("/d/baituo/obj/sherou");
		break;
	   case 2:
		ob1 = new("/d/baituo/obj/turou");
		break;
	   case 3:
		ob1 = new("/d/baituo/obj/cake");
		break;
		}
	   	ob1->set("bt_food",1);
		ob1->move(me);
		message_vision("丫环在桌上放了一"+ob1->query("unit")+ob1->name()+"，轻声道：少主请用。\n", me);
		}
	if(!w) {
		ob2 = new("/d/baituo/obj/liqueur");
		ob2->set("bt_liqueur",1);
		ob2->move(me);
		message_vision("丫环替$N斟了一杯酒。\n", me);
		}
	}
	else if (me->query("family/master_id") == "ouyang ke" ) {
	message_vision("丫环摇了摇头，轻声说道："+RANK_D->query_respect(me)+"，这得要庄主或少主指示才行。\n", me);
	}
	else {
	message_vision("不过，丫环们仍然各忙各的，看来完全没人把$N的叫嚷当一回事。\n", me);
	}
	return 1;
}
