...
void create()
{
	...
	set("max_jingli", 2000);
	set("jingli", 2000);
	set("max_neili", 1000+random(2000));
	set("neili", 1000+random(2000));
...
{
        if(!objectp(ob))
                return; //added by iszt@pkuxkx, 2007-01-04
...
void chaning(object me, object ob)
{
	...
	string *chan_msg = ({
		HIR"\n$n暗运劲力，想将缠在身上的$N崩断。但$N身子可伸可缩，只略加延伸，并不会断。"+ZJURL("cmds:struggle")+HIG"挣脱"NOR"\n"NOR,
		HIR"\n$N猛力缠紧，只箍得$n腰骨几欲折断。\n"NOR,
		HIR"\n$N身子一卷到$n，越收越紧，再也不放，$n渐渐给$N缠得喘不过气来。"+ZJURL("cmds:struggle")+HIG"挣脱"NOR"\n"NOR,
		HIR"\n$n被$N缠住，除了呻吟怒骂，再无反抗的能为。"+ZJURL("cmds:struggle")+HIG"挣脱"NOR"\n"NOR,
	});
	string *chan_beast_msg = ({
		HIR"\n$N猛力缠紧，不让$n有机会逃脱。"+ZJURL("cmds:struggle")+HIG"挣脱"NOR"\n"NOR,
		HIR"\n$N身子一卷到$n，越收越紧，再也不放，$n渐渐给$N缠得喘不过气来。"+ZJURL("cmds:struggle")+HIG"挣脱"NOR"\n"NOR,
		HIR"\n$n被$N缠住，慢慢失了抗拒之力，气息越来越是微弱。"+ZJURL("cmds:struggle")+HIG"挣脱"NOR"\n"NOR,
	});

	string *chan_snake_msg = ({
		HIR"\n$N猛力缠紧，牢牢箍住$n的七寸。"+ZJURL("cmds:struggle")+HIG"挣脱"NOR"\n"NOR,
		HIR"\n$N身子一卷到$n，越收越紧，再也不放，$n渐渐给$N缠得喘不过气来。"+ZJURL("cmds:struggle")+HIG"挣脱"NOR"\n"NOR,
		HIR"\n$N将$n紧紧缠住，慢慢施力，$n虽然不断扭动蛇身，却也无法脱困。"+ZJURL("cmds:struggle")+HIG"挣脱"NOR"\n"NOR,
	});
	string *chan_bird_msg = ({
		HIR"\n$N猛力缠紧，只箍得$n双翅几欲折断。"+ZJURL("cmds:struggle")+HIG"挣脱"NOR"\n"NOR,
		HIR"\n$n被$N缠住，吱吱乱叫，翅膀不停扑打。"+ZJURL("cmds:struggle")+HIG"挣脱"NOR"\n"NOR,
		HIR"\n$N愈盘愈紧，$n毛羽贲张，竭力相抗，眼见不支。"+ZJURL("cmds:struggle")+HIG"挣脱"NOR"\n"NOR,
	});
...
//                         ob->receive_wound("qi", (ob->query("qi")+100), "被"+me->name()+"咬死了");
                         ob->receive_wound("qi", (ob->query("qi")+100), me);
//                        ob->receive_damage("qi", (ob->query("qi")+100), "被"+me->name()+"咬死了");
                        ob->receive_damage("qi", (ob->query("qi")+100), me);
			ob->delete_temp("snake_chan");
			me->delete_temp("chaning");
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
	...
	F_POISON->poison_attack(me,victim,"bt_poison",20 + me->query("snake_poison")/5 +
...
int convert(string arg)
{
	...
	if (arg!="snake" && arg!="she" && arg!="mang" && arg!="python" && arg!="dashe") return 0;
...
void die()
{
	...
	if(this_object()->name() == "巨蟒")
	{
		ob = new(__DIR__"obj/jumangdan");
		ob->move(corpse);
	}
