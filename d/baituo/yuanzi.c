...
string look_gate()
{
        if (query_temp("lock") == 0)
                return "门上的铁锁被打开了。\n"
                ZJOBACTS2+ZJMENUF(2,2,12,40)+"锁门:lock door\n";
        return "这扇门被铁锁牢牢锁住。\n"
        ZJOBACTS2+ZJMENUF(2,2,12,40)+"开门:open door\n";
}
int do_open(string arg)
{
        ...
        if( query_temp("lock") == 1 && !present("yao shi", me) )
...
{       if (!wizardp(me)&&me->query("family/family_name")!="白驼山"&&dir=="west")
                return notify_fail("你又不是白驼山庄的人，想白拿东西呀？\n");
