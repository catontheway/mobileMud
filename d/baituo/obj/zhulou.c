...
int do_catch(string arg)
{
	...
	if( living(snake) && snake->query("caught") != me->query("id") ) 
       return notify_fail(snake->name()+"并非你所驯养，直接动手捉相当危险，\n还是先打晕再捉为好。\n");

	if(!snake->query("bt_snake")) 
		return notify_fail("这不是白驼山蛇谷的品种,即使捉了也培养不出怪蛇。\n");

	if( snake->move(obj) ) 
	{
		message_vision("$N将"+snake->name()+"捉了起来，从尾巴起慢慢放进竹篓。\n", me, snake);
		snake->delete("frightened");
		if( !snake->query("caught") || snake->query("caught") != me->query("id") ) 
...
int do_free(string arg)
{
	...
	if(arg=="all") {
		...
		if( me->is_fighting() )
			return notify_fail("你还在战斗中！一次祗能放出一条蛇。\n");
	...
	if( !snake->is_character() || snake->is_corpse() )
		return notify_fail("那并不是活物，直接用手拿就可以了。\n");
