...
void create()
{
        set("short", "碧水潭畔");
        set("long", @LONG
你来到了碧水潭畔，望眼看去，潭水如玉，深不可测。站在潭畔
也能感到一阵阵的寒意。遥想当年光明顶上，碧水潭畔，黛绮丝紫衫
如花，长剑胜雪，不知倾倒了多少英雄豪杰。
LONG
        );
        set("exits", ([
                "south" : __DIR__"houshanxiaolu3",
        ]));
        set("resource/water", 1);
      set("objects",([
                CLASS_D("mingjiao") + "/daiqisi.c" : 1,
            ]));
...
*/
        ...
        for(i=0;i<times;i++)
        {
                ...
                {
                    if (arg=="dagger" && me->query("adv_weapon/class/dagger/lv")>=4)//高级兵器级别大于4级，加25%效率 Zine
                    {
