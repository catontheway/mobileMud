...
//预测局信息记录
//主键是预测局编号，预测局编号由a-z和0-9组成的不超过8位的编码。
//包含内容：name（预测局标题）、desc（预测局描述）、dealer（庄家）、wagerTime（投注截至时间）、createTime（预测局创建时间）、
//          odds（赔率信息，mapping结构）、winOdds（获胜的赔率ID）、endTime（预测局截至时间，一般是获胜赔率公布之后顺延7天）、
//          status（状态：0-初始化、1-接受投注、2-投注截至、3-公布结果、4-预测局结束，庄家领取收益（庄家收益包括之前保证金、
//          投注各赔率未赔尽资金、预测局结束后玩家未领走资金）、5-预测局强制终止、6-系统强行关闭）、wagerType（投注类型：0-仅接受金条、
//          1-仅接受宝石、2-接受金条和宝石）、startTime（投注开始时间）、wagerDays（投注天数，1-7天）、
//          taxes（抽水比例，数字表示N:1，比如10：1，20：1，50：1，比率由预测局大小决定，预测局越大抽水越低）、
...
void create()
{
	...
	set("str", 35);
	set("title", HIW"恶预测鬼"NOR);
	set("gender", "男性");
	set("age", 35);
	set("long", "他是著名的恶预测鬼轩辕三光，只要你敢预测，他什么样的预测局都敢开。\n");
  set("qi", 1500);
  set("max_qi", 1500);
  set("jing", 1200);
  set("max_jing", 1200);
  ...
  set("inquiry", ([
    "预测局" : "只有非常具有声誉的人士才能到我这里开立预测局(bet)，其他人可以查询(find)\n现在可以投注的预测局，选择合适的赔率进行投注(wager)。\n",
    "庄家" : "庄家必须是巫师、新手导师或者富豪，开立的预测局只接受金条或者宝石，预测局公\n布结果后一周内未领取的财物自动归庄家所有。\n",
    "投注" : "玩家可以选择庄家开设的预测局中选择其中一个赔率进行投注，直到庄家保证金不足以支付可\n能赔付的金额的时候才会停止接收投注。\n",
    "保证金" : "庄家开设预测局要投入保证金到自己开立的各个赔率中作为赔付玩家的资本金，当然庄\n家可以通过不断调整赔率来获得合适的收益。\n",
    "赔率" : "庄家开立的预测局可以对多种胜负关系设置不同的赔率，赔付的\n财物来源于同一预测局其他赔率赔付的金额和庄家的保证金。也就是说同一预测局不\n同赔率之间都是对立的，一个预测局不能同时拥有两个或者两个以上获胜的赔率。"
  ]) );
...
//非玩家或者top rich人士才有资格作预测局的庄家。
//bet命令具有以下几种模式：
//open 预测局ID 开启预测局
//modify 预测局ID 维护预测局，可以修改描述性内容
//odds 预测局ID 修改赔率
//add 预测局ID 增加赔率类型
//start 预测局ID 开放投注，同时庄家在各个赔率投入保证金。
//close 预测局ID 关闭预测局，在预测局投注截至时间到达前，庄家可以强制关闭预测局，关闭以后玩家可以取回自己的财物。
...
命令格式：bet {open|modify|odds|add|start|close} 预测局唯一ID
    bet open     创建预测局
    bet modify   修改预测局描述
    bet odds     修改赔率
    bet add      增加赔率ID
...
ERROR);
	...
	for ( lp=0;lp<strwidth(betId);lp++ )
	{
		if ( (betId[lp]>='a' && betId[lp]<='z') ||
	  	...
	  	continue;
  	write("预测局ID仅为a-zA-Z0-9等字母组成。\n");
  	return 1;
	}
	
	if ( strwidth(betId) > 10 )
	{
		write("预测局ID长度最长不能超过10。\n");
  	return 1;
	}
	...
	{
	  message_vision("仅巫师、新手导师、富豪排行榜、100m以上1000根金条以上等四种资质的人士才有资格进行和开启/维护/关闭预测局相关的操作。\n", me);
	  return 1;
	}
  ...
  switch(betType)
  {
  	case "open":
	    ...
	    bet_info[betId]["status"]=-1; //初始的时候预测局是初始化，只有庄家决定开放以后才会开放预测局。
	    tell_object(me, "请输入预测局标题（最多10个汉字）：\n");
	    input_to("openbet_input", me, betId, "name");
	    break;
  	case "modify":
  		if ( member_array(betId, keys(bet_info) )<0 )
	    {
		    write("该预测局并没有开局，无法进行信息维护操作。\n");
		    return 1;
	    }
	    
	    if ( bet_info[betId]["dealer"]!=me->query("id") )
	    {
	    	write("只有庄家才能进行本预测局的信息维护操作。\n");
	    	return 1;
	    }
	    
	    if ( bet_info[betId]["status"]<0 )
	    {
	    	write("未知的预测局状态，只能进行关闭预测局操作。\n");
	    	return 1;
	    }
	    
	    show_bet(betId,0);
	    tell_object(me, "你可以维护的信息有预测局名称（name），预测局描述（desc），赔率名称（赔率ID name），赔率描述（赔率ID desc），\n请输入你要维护的信息类型：\n");
	    input_to("modify_bet", me, betId, 0, 0);
	    break;
	  case "odds":
  		if ( member_array(betId, keys(bet_info) )<0 )
	    {
		    write("该预测局并没有开局，无法进行维护赔率操作。\n");
		    return 1;
	    }
	    
	    if ( bet_info[betId]["dealer"]!=me->query("id") )
	    {
	    	write("只有庄家才能进行本预测局的维护赔率操作。\n");
	    	return 1;
	    }
	  ...
	  case "add":
  		if ( member_array(betId, keys(bet_info) )<0 )
	    {
		    write("该预测局并没有开局，无法进行增加赔率类型操作。\n");
		    return 1;
	    }
	    
	    if ( bet_info[betId]["dealer"]!=me->query("id") )
	    {
	    	write("只有庄家才能进行本预测局的增加赔率类型操作。\n");
	    	return 1;
	    }
	    
	    if ( bet_info[betId]["status"]>0 )
	    {
	    	write("该预测局只有处于初始化未开放投注的状态下才能进行增加赔率类型操作。\n");
	    	return 1;
	    }
	  ...
	  case "start":
  		if ( member_array(betId, keys(bet_info) )<0 )
	    {
		    write("该预测局并没有开局，无法进行开放投注操作。\n");
		    return 1;
	    }
	    
	    if ( bet_info[betId]["dealer"]!=me->query("id") )
	    {
	    	write("只有庄家才能开启本预测局的投注。\n");
	    	return 1;
	    }
	    ...
	    if ( bet_info[betId]["status"]>1 )
	    {
	    	if ( !bet_info[betId]["status"] )
	    	  write("该预测局只有处于初始化未开放投注的状态下才能进行开放投注动作。\n");
	    	else
	    		write("该预测局只有处于接受投注的状态下才能进行修改押金动作。\n");
	    	return 1;
	    }
  	...
  	case "close":
  		if ( member_array(betId, keys(bet_info) )<0 )
	    {
		    write("该预测局并没有开局，无法进行维护或者关闭操作。\n");
		    return 1;
	    }
	    
	    if ( bet_info[betId]["dealer"]!=me->query("id") )
	    {
	    	write("只有庄家才能强行关闭本预测局。\n");
	    	return 1;
	    }
      ...
      autochg_status(betId);
	    ...
	    if ( bet_info[betId]["status"]>1 )
	    {
    		write("该预测局只有处于初始化或者接受投注的状态下才能强行关闭该预测局。\n");
	    	return 1;
	    }
	    ...
	    {
	    	tell_object(me, "该预测局已经强行关闭，请取回押金。\n");
	      CHANNEL_D->do_channel(this_object(), "jy", "预测局『"+bet_info[betId]["name"]+"("+betId+")』已经被庄家"+me->query("name")+"("+me->query("id")+")强行关闭。");
	    ...
	    else
	    {
	      ...
	      tell_object(me, "该预测局已经强行关闭，请取回押金。\n");
	      CHANNEL_D->do_channel(this_object(), "jy", "预测局『"+bet_info[betId]["name"]+"("+betId+")』已经被庄家"+me->query("name")+"("+me->query("id")+")强行关闭，请各位玩家领回押金。");
        show_bet(betId,0);
	      save();
	    }
	    break;
  	default:
  	  write("命令格式：bet {open|modify|close} 预测局唯一ID");
	    return 1;
  }
	return 1;
}
...
//显示预测局列表
//没有参数显示所有预测局，-w 显示可以接受投注的预测局，-e 显示关闭投注的预测局，-s 显示已经公布结果的预测局
//-o 显示处于各种关闭状态的预测局
int do_find(string arg)
{
	object me=this_player();
	string *sst=({"初始化  ","接受投注","投注截至","结果公布","预测局关闭","强行终止","系统强行关闭"}), *betk;
	string user;
	int lp;
	...
	{
		tell_object(me, @ERROR
查询预测局列表命令：find [-w|-e|-s|-o]，各种输入参数含义如下：
未加参数表示显示所有预测局，
-w 显示处于开放投注状态的预测局，
-e 显示处于投注截至状态的预测局，
-s 显示处于结果公布状态的预测局，
...
//首先清理预测局结束超过30天以上的所有预测局。	
	clear_bet();
	tell_object(me, "###############################################################################################\n");
	tell_object(me, "预测局ID     预测局名称             预测局状态 投注截至时间        预测局关闭时间      庄家            \n");
	tell_object(me, "###############################################################################################\n");
	for(lp=0;lp<sizeof(betk);lp++)
	{
		...
		tell_object(me, sprintf("%-10s %-20s %-8s %-19s %-19s %s\n",
		  betk[lp], bet_info[betk[lp]]["name"], bet_info[betk[lp]]["status"]<0?"未知预测局":sst[bet_info[betk[lp]]["status"]], 
		  show_time(bet_info[betk[lp]]["wagerTime"]), show_time(bet_info[betk[lp]]["endTime"]), bet_info[betk[lp]]["dealer_name"] ) );
	}
	
	tell_object(me, "###############################################################################################\n");
	tell_object(me, "显示预测局详细信息请用命令"HIY"b 预测局编号"NOR"，其他输入退出预测局查询命令。\n");
	tell_object(me, "###############################################################################################\n");
...
int do_win(string arg)
{
	...
	{
		write("命令格式：win 预测局ID 赔率ID，宣布哪个赔率获胜。\n");
		return 1;
	}
  ...
  if ( member_array(bet_id, keys(bet_info) )<0 )
	...
	{
		write("只有庄家才有权限宣布预测局获胜结果。\n");
		return 1;
	}
	
	if ( bet_info[bet_id]["status"]!=2 )
	{
		write("只有预测局关闭投注以后才能公布获胜结果。\n");
		return 1;
	}
	...
	bet_info[bet_id]["endTime"]=time()+7*24*3600;//预测局关闭时间是公布结果之后顺延7天
...
//  printf("wgolds:%d wgem %d ugolds %d ugem %d odds %d:%d\n",wgolds, wgem, ugolds, ugem, dodds, uodds);
  ...
  "预测局『"+bet_info[bet_id]["name"]+"("+bet_id+")』已经公布获利赔率，获利赔率是"+odds_info[odds_id]["name"]+"("+odds_id+")"+"。");
  CHANNEL_D->do_channel(this_object(), "jy", "请投注该赔率的玩家尽快过来领取奖励。");
  return 1;
}

//庄家或者玩家领取奖励，庄家可以领取两次奖励，第一次是领取自己剩余的保证金，第二次是预测局结束以后领取玩家未领走的收益
int do_wager(string arg)
{
	...
	if ( !arg )
	{
		write("命令格式：wager 预测局ID，领取预测局收益。\n");
		return 1;
	}
  ...
  {
  	write("玩家只有在预测局处于公布结果、强行终止、系统强行关闭等阶段才能领取收益和保证金。\n");
  	return 1;
  }
  ...
  if ( bet_info[bet_id]["status"]!=4 &&
  	   ...
  	   bet_info[bet_id]["dealer"]==uid )
  {
  	write("庄家只有在预测局处于预测局关闭、强行终止等阶段才能领取收益和保证金。\n");
  	return 1;
  }
...
//庄家不会被抽走收益  
  if ( bet_info[bet_id]["dealer"]!=uid )
  {
    ...
    if ( tgolds > 0 )
      CHANNEL_D->do_channel(this_object(), "jy", me->query("name")+"领取预测局本金和收益中被预测场抽走了"+chinese_number(tgolds)+"两黄金。");
  
    if ( tgem > 0 )
  	  CHANNEL_D->do_channel(this_object(), "jy", me->query("name")+"领取预测局本金和收益中被预测场抽走了相当于"+chinese_number(tgem)+"颗水月的宝石。");
  }
...
//显示某一个预测局的基本信息
// type 0为显示特定预测局模式，不带附属命令
// type 1为通用预测局列表选择预测局模式，附带附属命令
protected string show_bet(string msg, int type)
{
	string output,bet_id,cmd, wodds;
	string *sst=({"初始化  ","接受投注","投注截至","结果公布","预测局关闭","强行终止","系统强行关闭"});
	...
	else
	{
		...
		if ( member_array(bet_id, keys(bet_info)) < 0 )
		{
			tell_object(me, "预测局编号输入错误，请重新输入命令：\n");
			input_to("show_bet",0,1);
			return 0;
		}
	}
  ...
  autochg_status(bet_id);
	output="预测局："HIM+bet_info[bet_id]["name"]+"("+bet_id+")"NOR" \t 庄家："HIW+bet_info[bet_id]["dealer_name"]+"("+bet_info[bet_id]["dealer"]+")"NOR" \n";
	output+="预测局描述：\n";
	output+=bet_info[bet_id]["desc"]+"\n";
	output+="状态："+((bet_info[bet_id]["status"]<0)?"未知预测局":sst[bet_info[bet_id]["status"]])+"  投注类型："+(!bet_info[bet_id]["status"]?"未知":wtype[bet_info[bet_id]["wagerType"]])+
	...
	output+="预测局创建时间："+show_time(bet_info[bet_id]["createTime"])+ "  投注截至时间："+show_time(bet_info[bet_id]["wagerTime"])+"\n";
	output+="预测局关闭时间："+show_time(bet_info[bet_id]["endTime"])+"  预测局抽水比例："+sprintf("%d:1", bet_info[bet_id]["taxes"]) +"\n";
	...
	if ( type )
	{
		...
		tell_object(me, "请选择命令"HIY"o 赔率ID"NOR"显示详细的该赔率的详细信息，命令b返回上一层，其他命令退出查询预测局命令：\n");
...
//显示某一个预测局下某赔率的具体投注明细（包括庄家的保证金信息）
protected void show_odds_detail(string msg, string bet_id)
{
  ...
  if ( cmd!="o" )
	...
	output+=  "会自动寻找五行宝石中火以上等级，日月宝石要求雷以上等级，命令b返回上一层，输入其他命令自动退出预测局查询命令。：\n";
	tell_object(me, output);
	input_to("add_wager", bet_id, odds_id);
	return;
}

//开启预测局的时候和预测局相关的信息输入检核和录入
protected void openbet_input(string msg, object me, string bet_id, string type)
{
	...
	switch(type)
	{
		case "name":
			if ( !chn_check(msg, 20) )
			{
		    tell_object(me, "对不起，预测局标题输入非法，请重新输入（最多10个汉字）：\n");
		    input_to("openbet_input", me, bet_id, "name");
		    break;
			}
	    bet_info[bet_id][type]=msg;
	    tell_object(me, "\n请输入预测局的简单描述（最多100个汉字或者200个英文字母）：\n");
	    input_to("openbet_input", me, bet_id, "desc");
	    break;
	  case "desc":
			if ( strwidth(msg)>200 || strwidth(msg)< 2 )
			{
		    tell_object(me, "对不起，预测局的描述输入非法，请重新输入（最多100个汉字或者200个英文字母）：\n");
		    input_to("openbet_input", me, bet_id, "desc");
		    break;
			}
	    bet_info[bet_id][type]=msg;
	    tell_object(me, "\n请输入预测局的投注小时数（数字1-360表示1小时-15天，从开启投注开始算起的小时数）：\n");
	  ...
	  case "wagerDays":
	  	if ( !msg ||
	  		   !sscanf(msg, "%d", days) )
	  	{
	  		tell_object(me, "请输入预测局的投注小时数（数字1-360表示1小时-15天，从开启投注开始算起的小时数）：\n");
	  		input_to("openbet_input", me, bet_id, "wagerDays");
	  		break;
	  	}
	  	...
	  	tell_object(me, "请选择预测局接受的财物类型（0-仅接受金条，1-仅接受宝石，2-接受金条和宝石）：\n");
	  ...
	  case "wagerType":
	  	...
	  	tell_object(me, "请选择预测局的投注限额（格式是N|M，N表示限制投注金条的数量，M表示限制投注宝石的雷月数，数值0表示不限制或者直接回车也表示不限制）：\n");
	  ...
	  case "wagerMax":
	  	...
	  	tell_object(me, "预测局将开启至少2个以上赔率，现在请输入第一个赔率的赔率ID（a-zA-Z0-9组成的字符串）：\n");
...
//开启预测局的时候和赔率相关的信息输入检核和录入（保证输入两个赔率）
protected void odds_init(string msg, object me, string bet_id, string odds_id, int odds, string type)
{
	...
	switch(type)
	{
	  ...
	  case "decide":
	  	...
	  	{
	  		tell_object(me, @DESC
本预测局所有设定都已完成，确认以后可以用bet modify修改信息，bet add 新增
赔率（开放投注以后禁止新增赔率），bet start 投入保证金开启预测局，bet odds修
改赔率，每个设定好的赔率都至少投入价值100的金条或者18 雷月的宝石作为保证金，
保证金的多少直接影响该预测局玩家投入到各个赔率的资金规模，bet start 命令在接
受投注以后转为追加押金模式，bet close 强制关闭预测局，只能在投注截至之前能够
进行强制关闭预测局，强制关闭之后各方可以无损取回本金，庄家取回的保证金要按照
抽水比例缴纳赔偿金。
DESC);
...
//维护预测局信息，step=0为输入维护类型， 1-预测局名称（name），2-预测局描述（desc），3-赔率名称（name），4-赔率描述（desc）
	...
	switch(step)
	{
		case 0:
			if ( msg!="name" && msg!="desc" )
			{
				...
				{
					tell_object(me, "你仅可以维护预测局名称（name），预测局描述（desc），赔率名称（赔率ID name），赔率描述（赔率ID desc）四种类型，\n请重新输入你要维护的信息类型：\n");
	      ...
	      {
		      write("该预测局并不存在你所要维护的赔率ID，请重新输入你要维护的信息类型\n（name-预测局名称，desc-预测局描述，赔率ID name-赔率名称，赔率ID desc-赔率描述）：\n");
		      input_to("modify_bet", me, bet_id, 0, 0);
		      break;
	      }
	    ...
	    else
	    {
	      if ( msg=="name" )
	      {
	      	tell_object(me, "请输入本预测局的新标题（最多10个汉字）：\n");
	      	input_to("modify_bet", me, bet_id, 0, 1);
	      	break;
	      }
	      else
	      {
	      	tell_object(me, "请输入本预测局的简单描述（最多100个汉字或者200个英文字母）：\n");
...
//维护预测局的所有赔率，按照顺序依次修改各个赔率ID对应的具体赔率
...
protected void modify_odds(string odds, object me, string bet_id, string *odds_list, int serial)
{
	...
	if ( serial<0 )
	{
		...
		{
			tell_object(me, "请输入预测局『"HIM+bet_info[bet_id]["name"]+"("+bet_id+")"NOR"』中『"HIY+
  ...
  {
  	tell_object(me, "修改赔率以后你目前的押金并不能足额支付玩家的预测注，请增加押金或者重新修改赔率。\n");
  	tell_object(me, "请输入预测局『"HIM+bet_info[bet_id]["name"]+"("+bet_id+")"NOR"』中『"HIY+
			  odds_info[odds_list[serial]]["name"]+"("+odds_list[serial]+")"NOR"』的最新赔率\n（旧赔率值："+
			  odds_info[odds_list[serial]]["odds"]+"，不修改该赔率请直接回车。）：\n");
		input_to("modify_odds", me, bet_id, odds_list, serial);
  ...
  odds_info[odds_list[serial]]["odds"]=sprintf("%d:%d",o1,o2);
	save();
	CHANNEL_D->do_channel(this_object(), "jy", "预测局『"HIM+bet_info[bet_id]["name"]+"("+bet_id+")"NOR"』中『"HIY+
...
protected void save_deposit(string deposit, object me, string bet_id, string *odds_list, int serial)
{
	...
	switch(bet_info[bet_id]["wagerType"])
	{
	    ...
	    if ( serial<0 )
	    {
		    if ( sizeof(odds_list)>0 )
		    {
			    tell_object(me, "请输入黄金预测局『"HIM+bet_info[bet_id]["name"]+"("+bet_id+")"NOR"』的赔率『"HIY+
			      odds_info[odds_list[0]]["name"]+"("+odds_list[0]+")"NOR"』要缴纳的保证金\n"+
			      "（根据预测局收取的投注类型不同收取对应的采取，对于仅收取金条或者宝石的预测局\n"+
			      "输入数字表示金条根数或者宝石折算到雷月的数量，对于同时收取金条和宝石的预测\n"+
	    ...
	    igolds=odds_info[odds_list[serial]]["dealer_golds"];
			...
			if ( wager_golds(me, bet_id, odds_list[serial], golds) )
			{
				if ( sizeof(odds_list)<serial+2 )
	      {
		      tell_object(me, HIR"完成所有赔率的保证金缴纳，预测局开放投注。\n"NOR);
		      if ( !bet_info[bet_id]["status"] )
		      {
		      	CHANNEL_D->do_channel(this_object(), "jy", "预测局『"HIM+bet_info[bet_id]["name"]+"("+bet_id+")"NOR"』开放投注，欢迎各位玩家到长安城长乐坊投注。");
	  ...
	  case 1:
	    if ( serial<0 )
	    {
		    if ( sizeof(odds_list)>0 )
		    {
			    tell_object(me, "请输入宝石预测局『"HIM+bet_info[bet_id]["name"]+"("+bet_id+")"NOR"』的赔率『"HIY+
			      odds_info[odds_list[0]]["name"]+"("+odds_list[0]+")"NOR"』要缴纳的保证金\n"+
			      "（根据预测局收取的投注类型不同收取对应的采取，对于仅收取金条或者宝石的预测局\n"+
			      "输入数字表示金条根数或者宝石折算到雷月的数量，对于同时收取金条和宝石的预测\n"+
			      "局需要输入两个数字，中间用|分隔，每个赔率庄家押金中金条不能少于100根，宝\n"+
	    ...
	    igem=odds_info[odds_list[serial]]["dealer_gem"];
			...
			if ( wager_gem(me, bet_id, odds_list[serial], gem) )
			{
				if ( sizeof(odds_list)<serial+2 )
	      {
		      tell_object(me, "完成所有赔率的保证金缴纳，预测局开放投注。\n");
		      if ( !bet_info[bet_id]["status"] )
		      {
		      	CHANNEL_D->do_channel(this_object(), "jy", "预测局『"HIM+bet_info[bet_id]["name"]+"("+bet_id+")"NOR"』开放投注，欢迎各位玩家到长安城长乐坊投注。");
		        bet_info[bet_id]["status"]=1;
	  ...
	  case 2:
	    if ( serial<0 )
	    {
		    if ( sizeof(odds_list)>0 )
		    {
			    tell_object(me, "请输入宝石和黄金预测局『"HIM+bet_info[bet_id]["name"]+"("+bet_id+")"NOR"』的赔率『"HIY+
			      odds_info[odds_list[0]]["name"]+"("+odds_list[0]+")"NOR"』要缴纳的保证金\n"+
			      "（根据预测局收取的投注类型不同收取对应的采取，对于仅收取金条或者宝石的预测局\n"+
			      "输入数字表示金条根数或者宝石折算到雷月的数量，对于同时收取金条和宝石的预测\n"+
...
protected void add_wager(string msg, string bet_id, string odds_id)
{
	...
	if ( bet_info[bet_id]["status"]!=1 &&
		   msg!="b" )
	{
		tell_object(me, "该预测局未处于接受投注状态，无法进行押注。\n");
		printf("\n");
	  show_bet("b "+bet_id, 1);
	  return;
	}
...
protected int wager_golds(object me, string bet_id, string odds_id, int golds)
{
  ...
  {
  	tell_object(me, "庄家限制该预测局每个玩家在每个赔率上最多投注"+bet_info[bet_id]["max_golds"]+"根金条。\n");
		return 0;
	}
             ...
             odds_info["dealer_golds"], bet_id, o2, o1) )
  {
	...
	tell_object(me, "你将"+chinese_number(golds)+"根金条投入到预测局当作。\n");
	save();
	return 1;
}
...
protected int wager_gem(object me, string bet_id, string odds_id, int gem)
{
  ...
  {
  	tell_object(me, "庄家限制该预测局每个玩家在每个赔率上最多投注等值"+bet_info[bet_id]["max_gem"]+"颗雷月的宝石。\n");
		return 0;
	}

  for(lp=0;lp<sizeof(gems);lp++)
  {
  	tell_object(me, "你将"+gems[lp]->query("name")+"投入预测局。\n");
	...
	tell_object(me, "你将"+chinese_number(lei)+"颗等值雷月的宝石投入到预测局当作。\n");
	save();
	return 1;
}

//自动更新预测局状态
//预测局状态自动更新包括如果超过投注截至时间，自动将状态从接受投注修改为投注结束
//如果超过投注截至时间一周，庄家仍然没有公布结果，自动将状态从投注结束修改为系统强行终止预测局，庄家保证金平摊给各玩家。
//如果超过预测局结束时间，自动将预测局从公布结果修改为预测局结束，终止玩家领取收益和赎回押金。
protected void autochg_status(string bet_id)
{
	mapping odds_info, wager_info;
	...
	if ( bet_info[bet_id]["wagerTime"]<=time() &&
	 	   bet_info[bet_id]["status"]==1 )
	{
		CHANNEL_D->do_channel(this_object(), "jy", "预测局『"+bet_info[bet_id]["name"]+"("+bet_id+")』关闭投注，等待庄家公布结果，请庄家在七天内公布结果。");
	...
	if ( time()-bet_info[bet_id]["wagerTime"]>=7*24*3600 &&
		   bet_info[bet_id]["status"]==2 )
	{
		CHANNEL_D->do_channel(this_object(), "jy", "预测局『"+bet_info[bet_id]["name"]+"("+bet_id+")』超过七天未公布预测局结果，被系统强行关闭，请各位玩家领走押金。");
		bet_info[bet_id]["status"]=6;
	...
	if ( time()>=bet_info[bet_id]["endTime"] &&
		   bet_info[bet_id]["status"]==3 )
	{
		CHANNEL_D->do_channel(this_object(), "jy", "预测局『"+bet_info[bet_id]["name"]+"("+bet_id+")』已经关闭，关闭投注者领取收益，请庄家领取预测局收益。");
		bet_info[bet_id]["status"]=4;
		save();
	}
}

//清理所有预测局结束超过30天以上的所有预测局。
protected void clear_bet()
{
