...
void create()
{
        ...
        set("long",
"这是一个古香古色的"+ZJURL("cmds:open box")+HIG"盒子"NOR"，一看便知里面的东西价值不菲。\n");
        set("unit", "只");
        set("open_count", 1);
        set_weight( 2000);
        set("no_get", 1);
...
int do_open(string arg)
{
		...
		if (!this_player()->query_temp("wananta/lvl"))
		{
            ...
            this_player()->receive_damage("qi", 0, "打开盒子触发暗器机关");
			this_player()->die();
        ...
        if (query("open_count") > 0)
        {
			...
			if (lvl==4)
			{
				...
				if (a<3)
				{
					...
					obj->set("sockets/max",random(2)+1);
	...
	if (i < 3) gemtype = "yan";
			...
			if (lvl==6)
			{
				...
				if (a==49)
				{
					obj = new("/obj/gem/armor");
					obj->gen_armor(armortype,obj_min,obj_max);
					obj->set("sockets/max",3);                 //6层可获得3o装备
				}
				if (a==48)
				{
					obj = new("/obj/gem/weapon");
	...
	if (i < 3) gemtype = "yan";
			...
			if (lvl==7)
			{
				...
				if (a>950 && a<999)
				{
					obj = new("/obj/gem/armor");
					obj->gen_armor(armortype,obj_min,obj_max);
					obj->set("sockets/max",3);                 //7层可获得3o装备
				}
				if (a>800 && a<=950)
				{
					obj = new("/obj/gem/weapon");
	...
	if (i < 3) gemtype = "yan";
			...
			obj->move(this_player());
				...
				if (!obj->is_gem())
				{
					if (to_int(obj->query("level")) + to_int(obj->query("sockets/max")) > 7)
					{
						msg = "";
						if (to_int(obj->query("sockets/max")) > 1)
						{
							msg = CHINESE_D->chinese_number(to_int(obj->query("sockets/max"))) + "孔";
							CHANNEL_D->do_channel(this_object(),"rumor","听说"+this_player()->query("name")+"在开宝盒得到一"
				...
				{
					CHANNEL_D->do_channel(this_object(),"baowei",this_player()->query("name")+"开宝盒得到一"
			...
			if (!obj->is_gem())
			{
				msg = this_player()->name()+"在万安塔任务中得到一"+obj->query("unit")+obj->name()
  					   +"[等级："+obj->query("level")+"，孔数："+obj->query("sockets/max")+"]。";
			}
			if (obj->is_gem())
			{
				msg = this_player()->name()+"在万安塔任务中得到一"+obj->query("unit")+obj->name()
