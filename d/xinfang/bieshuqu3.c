...
void create()
{
	...
	set("long","这里是临安府城里住户最集中的地方，大大小小有着上千栋的房子。\n");


	set("exits", ([
		"south":"/d/hangzhou/xiaorenfang",
	]));
...
{ 
	...
	if (arg==me->query("marry/lover") ||  arg==me->query("id") )
	{
		if (arg==me->query("marry/lover"))
		{
            ...
            if(me->move(room)) me->set_temp("xinfang/from","/d/xinfang/bieshuqu3");
            else return 0;
...
*/
	...
	else 
	{
		...
		{
			if(me->move(environment(find_player(arg)))) me->set_temp("xinfang/from","/d/xinfang/bieshuqu3");
            else return 0;
