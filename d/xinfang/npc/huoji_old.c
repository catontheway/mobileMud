...
{
        if(random(2))
        say("驿站伙计微笑着对" + ob->query("name") + "说：这位" + RANK_D->query_respect(ob) +
                "是不是来看信的呀？\n");
        else
...
int ask_sendmail()
{
        ...
        if(ppl->query("songxin/ok")==1) delay=30; //old 60
        else
        if(ppl->query("songxin/ok")==2) delay=90; //old 180
else
        if(ppl->query("songxin/ok")==3) delay=150; //old 600
        else delay=0;
        ...
        mail->start_decay(times+random(100));  //old 300+random(200));
