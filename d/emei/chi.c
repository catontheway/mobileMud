...
void init()
{
    ...
    }*/ //用notice()代替init()触发，避免N人多次反复flood信息 Zine
...
{
	object room = load_object(__DIR__"xixiangchi");
	if (!userp(me))
	return 1;
	else
	{
		...
		tell_object(me, "你扑通一声跳进了池水。\n");    
		tell_room(room, "只听“扑通”一声，"+me->name()+"已经跳进了池水里。\n");        
        if (!me || !present(me) || !living(me)||environment(me)!=this_object()) return 0;       
        ...
        if(me->query_con() < 25)
		{
			...
			me->start_busy(2);
			me->receive_damage("qi", 100, "在洗象池游泳淹");
			me->receive_damage("jing", 100, "在洗象池游泳淹");
			if (living(me))
		...
		me->receive_damage("jing", me->query("con") * 4, "在洗象池游泳淹");
		me->start_call_out(__FILE__, "do_pa", (int)(50-me->query("con")), me);
		me->start_busy(1);
		return 1;
	}
}
...
int do_pa(object me)
{
        ...
        if(!me || !present(me)||environment(me)!=this_object()) return 0;
...
void get_ob(object me)
{
        ...
        object temproom;
        int i;
        if (!me||!me->query_temp("bath/temproom"))
        return;
