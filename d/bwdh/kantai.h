...
int do_bet(string arg)
{
	 ...
	 {
		return notify_fail("对不起，你只能压预测注在比武的两人身上！\n");
	 }

	 obj = find_player(name);
	 if(!obj) return notify_fail("对不起，你压预测注的对象不见了！\n");

	 money = present( unit + "_money", me);

	 if( !money || money->query_amount() < amount )
		return notify_fail("你身上没有那么多钱 。\n");
	 if( amount*money->query("base_value") < 100 )	return notify_fail("预测场不收这么小的预测注，至少也要一两银子。\n");
	  ...
	  if( ( challenger=query("challenger") ) )
		{
				  ...
				  if( ob2)
				  {
					 ...
					 if( !query("fighting") && query("bet") )
					 {
						...
						message_vision(HIY"主持人高声说：$N和$n的预测注比现在是： "+(query("bet_amount/" + host ))+"："+(query("bet_amount/" + challenger ))+"。\n"NOR, ob1,ob2);
					 }
