...
void create() {
    set_name("预测台", ({ "casino table" }));
    if( clonep() )
        set_default_object(__FILE__);
    else {
        set("unit", "张");
        set("long", "这张预测台是控制预测场的：\nCommands:\n  casino east n\n  casino west n \n
  casino bet \n  casino pay \n  casino stop \n");
        set("material", "steel");
        set_weight( 5000);
        set("value", 10000000);
...
int casino_cmds(string arg) {
    ...
    if (!objectp(room)) {
        ...
        if (!objectp(room))
            return notify_fail("找不到预测场。\n");
    }

    if (args == 1) {
        switch (cmd) {
        case "bet" :  if ( room->query("status") == 1)
                          return notify_fail("预测场现在正在接收压注。\n");
                      else {
        ...
        case "stop":  if (room->query("status") == 0)
                          return notify_fail("预测场已经关门啦。\n");
                      else {
                          room->set("status", 0);
                          message_vision("$N关闭了预测场。\n", me);
                          return 1;
                      }
                      break;
