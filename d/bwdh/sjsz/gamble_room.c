...
string long_desc() {
    ...
    if (ob->query("status") == 1 )
        msg = "预测场内人山人海，乌烟瘴气。大家纷纷下注，有人口沫横飞地\n发表意见，有人两眼发直，喃喃自语。\n";
    else if (ob->query("status") == -1 )
        msg = "预测场现在是赔付时间，赢家喜气洋洋，得意地数着银子，输家\n早已急红了眼，面红耳赤。\n";
    else
        msg = "预测场现在没有营业，所以冷冷清清的。\n";

    msg = msg + "预测场正中挂者一个牌子(paizi)。\n";
    return msg;
}
...
int do_collect() {
    ...
    case "east" :   if ( me->query("bwdh_gamble/east_round") == ob->query("round") ) {
                        ...
                        write("预测场伙计对你“哼”的冷笑一声，一脚把你踢出门外。\n");
    ...
    money->move(me);
    me->set("bwdh_gamble/east_round", 0);
    write ("预测场付给你" + chinese_number(bet) + "两白银。\n");
    return 1;
}
