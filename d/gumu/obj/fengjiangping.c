...
int do_qufeng(string arg)
{
        ...
        int myexp, objexp, mylev, objlev, myforce, obforce, mypower, vpower;
        int steps;
        if( me->query("family/family_name")!="古墓派" &&
                  me->query("chushi/last_family")!="古墓派")
                return notify_fail("你哪里懂得驱蜂之术？\n");         
        ...
        mypower = mylev * 2 + myforce * 3;
        vpower = objlev * 2 + obforce;
        ...
        else if (random(mypower + vpower) < vpower) {
                message_vision(HIC "\n$n连忙运起护体真气，使玉蜂无法近身。\n" NOR, me, obj);
                me->start_busy(2);
                if (!obj->is_busy())                    
                        obj->start_busy(1);
