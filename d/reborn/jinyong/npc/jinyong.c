...
string *specialsk=({
    ...
    "might","spirituality","athanasy","intellect","constitution","perceive","lucky","cross",   
    });      
void create()         
{
...
string ask_chushi() 
{             
        int score_level = 3000;

          object me=this_player(); 
          if(time() > 1301011200) 
          {
              //2011年3月25日开始，每天增加10点门忠要求，直到2w点
              score_level = 5000 + (time() - 1301011200) / (7*86400) * 100;
              if(score_level > 20000) score_level = 20000;
          }
          ...
          if (me->query_combatgrade() < 100)   
               {     
                tell_object(me, "你经验太低，怎么出师啊？\n");      
                return "你的经验太低了！";  
               }
          if (me->query("chaoting/stayforever")) //八旗子弟quest,Zine 
          {
              tell_object(me,"你不是答应永远效忠清廷吗，还出师干嘛？\n");
              return "世上没有后悔药卖！"; 
          }
          if ((int)me->query("purefamily/level")>0) //纯门派无法出师,lordstar
          {
              tell_object(me,"人无信不立，你不是发誓永远留在师门吗，还出师干嘛？\n");
              return "世上没有后悔药卖！"; 
          }
          if ((me->query("family/family_name")!="朝廷" && (int)me->query("score") < score_level ) || (me->query("family/family_name")=="朝廷" && (int)me->query("score") < score_level * 20/30 ))
               {     
                return "你对门派的忠诚度不够，不能出师！"; 
               }
               me->set_temp("reborn_chushi",1); 
               command("consider");    
               return "你真的确认要出师吗？请"+ZJURL("cmds:confirm")+HIG"确认（confirm）！";    
}             
int do_confirm()    
{             
          object me=this_player();
...
//title系统记录玩家title by seagate@pkuxkx
	        me->set_title(TITLE_RANK, "无门无派，世外高人");
		  me->set("biography/chushi",time());//个人传记记录出师时间ZINE
          me->set("score",0); 
...
{  
         ...
         if ((int)me->query("level") < 38)
               {     
                tell_object(me, "测试期已过，需要38级。\n");      
                return "你的等级太低了！";  
               }
         for(i=sizeof(inv)-1; i>=0; i--)
                if(inv[i]->is_character()) return "你身上背的是什么啊？放下来先！";  
         if (query("on_chushi")>0 
         	  && time()-query("record_time")<=18000
         	  && (string)query("chushi_id")!=me->query("id"))
         	      return "对不起，有人正在出师，你等一段时间吧。";          	   
         if ((int)me->query("reborn/jinyong_all")<1)   
              {     
                command("consider");       
                command("look "+me->query("id"));
                msg += "在接受考验之前，你要帮我找回《金庸全集》。\n目前还缺少：\n";
                if (!me->query_temp("reborn/jinyong1")) msg+="《飞狐外传》\n";   
                if (!me->query_temp("reborn/jinyong2")) msg+="《雪山飞狐》\n";
                if (!me->query_temp("reborn/jinyong3")) msg+="《连城诀》\n";
                if (!me->query_temp("reborn/jinyong4")) msg+="《天龙八部》\n";
                if (!me->query_temp("reborn/jinyong5")) msg+="《射雕英雄传》\n";
                if (!me->query_temp("reborn/jinyong6")) msg+="《白马啸西风》\n"; 
                if (!me->query_temp("reborn/jinyong7")) msg+="《鹿鼎记》\n";
                if (!me->query_temp("reborn/jinyong8")) msg+="《笑傲江湖》\n";
                if (!me->query_temp("reborn/jinyong9")) msg+="《书剑恩仇录》\n";
                if (!me->query_temp("reborn/jinyong10")) msg+="《神雕侠侣》\n";
                if (!me->query_temp("reborn/jinyong11")) msg+="《侠客行》\n";
                if (!me->query_temp("reborn/jinyong12")) msg+="《倚天屠龙记》\n";
                if (!me->query_temp("reborn/jinyong13")) msg+="《碧血剑》\n";
                if (!me->query_temp("reborn/jinyong14")) msg+="《鸳鸯刀》\n";
                if (!me->query_temp("reborn/jinyong15")) msg+="《越女剑》\n";
                msg += RANK_D->query_respect(me) + "快把剩下的找齐吧！\n";
                return msg; 
              }     
         else  
              {     
...
string ask_canguan() 
{
        ...
        if (me->query_combatgrade() > 120)   
              {     
               tell_object(me, "你都已经出过师了吧，还有啥看的呀。\n");      
               return "你的经验太高了。";  
             }
        else
        	  {     
                command("nod");   
                message("vision","金庸在你眼前一拂袖，你发现自己来到了一个新的地方！ \n",me);        
                me->move("/d/reborn/kantai1");  
                return "好好学习学习吧，以后过劫难不至于太紧张。";   
...
{             
        ...
        if ( (string)ob->query("id") == "shujianenchou lu")   
             {     
...
int ask_special()
{
	      ...
	      if (me->query_combatgrade() >= 30 && !me->query("asked_special1")){
	      		spe=random_special(me);
	      		me->set("special_skill/"+spe,1);
	      		me->set("asked_special1",1);
	      		log_file("static/special",sprintf("%s(%s) 于%s获得特技%s，当前经验(%d)。\n",
            me->query("name"),me->query("id"),ctime(time()),spe,me->query("combat_exp"))); 
	      		CHANNEL_D->do_channel(this_object(), "jh", "恭喜"+me->query("name")+"获得一项特技："+spe+"。");
	      		CHANNEL_D->do_channel(this_object(), "jh", "gongxi "+me->query("id"), 1);
	      		return 1;
	      }
	      if (me->query_combatgrade() >= 60 && !me->query("asked_special2")){
	      		spe=random_special(me);
	      		me->set("special_skill/"+spe,1);
	      		me->set("asked_special2",1);
	      		log_file("static/special",sprintf("%s(%s) 于%s获得特技%s，当前经验(%d)。\n",
            me->query("name"),me->query("id"),ctime(time()),spe,me->query("combat_exp"))); 
	      		CHANNEL_D->do_channel(this_object(), "jh", "恭喜"+me->query("name")+"又获得一项特技："+spe+"。");
	      		CHANNEL_D->do_channel(this_object(), "jh", "addoil "+me->query("id"), 1);
	      		return 1;
	      }
	      if (me->query_combatgrade() >= 100 && me->query("reborn/all") 
	      	&& !me->query("asked_special3")){
	      		...
	      		log_file("static/special",sprintf("%s(%s) 于%s获得特技%s，当前经验(%d)。\n",
            me->query("name"),me->query("id"),ctime(time()),spe,me->query("combat_exp"))); 
	      		CHANNEL_D->do_channel(this_object(), "jh", "恭喜"+me->query("name")+"再获得一项特技："+spe+"。");
	      		CHANNEL_D->do_channel(this_object(), "jh", "admire "+me->query("id"), 1);
	      		return 1;
	      }
	      if (me->query("party_bonus/special")){  //聚会抽中特技奖励的，给这个标记，可以自己找金庸领奖。by lordstar 20180312
	      		spe=random_special(me);
	      		me->set("special_skill/"+spe,1);
	      		me->add("party_bonus/special", -1);
				me->add("party_bonus/got_special_num", 1);
	      		log_file("static/special",sprintf("%s(%s) 于%s获得【聚会奖励】特技%s，当前经验(%d)。\n",
            me->query("name"),me->query("id"),ctime(time()),spe,me->query("combat_exp"))); 
	      		CHANNEL_D->do_channel(this_object(), "jh", "恭喜"+me->query("name")+"获得一项聚会奖励特技："+spe+"。");
	      		CHANNEL_D->do_channel(this_object(), "jh", "admire "+me->query("id"), 1);
	      		return 1;
	      }
	     command("hmm");
