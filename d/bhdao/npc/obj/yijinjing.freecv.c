...
{       object me;
        ...
        if ( (int)me->query_skill("force", 1) < 500)
                return notify_fail("你的基本内功修为不够500级，不能学易筋神功。\n");
        if ( (int)me->query_skill("medical-skill", 1) < 500)
                return notify_fail("你的医术修为不够500级，不能学易筋神功。\n");
        ...
        realtimes = times;
         if ((me->query("jing")-100)/70 < times)
        {
                realtimes = me->query("jing")/70;
        }
