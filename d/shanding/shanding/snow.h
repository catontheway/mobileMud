...
string find_snow(object me)
{
	...
	if( skill > 200 )
		return WHT"积雪虽厚，你却毫不在意，看来积雪对你的轻功修为无所助益了。\n"NOR;
	
	remove_action("walk_snow", "walk");
	add_action("walk_snow", "walk");
	//18 busy can walk once
...
int walk_snow(string arg)
{
	...
	message_vision(HIW"$N提一口气，在积雪上小心的走了起来。\n"NOR, me);
	me->receive_damage("qi", 50);
	me->improve_skill("dodge", me->query("neili"));
	me->start_busy(6);
...
int destroy_snow(object me)
{
	...
	tell_object(me, CYN"你一路走下来，看着脚印回想方才的步法，"BNK+HIC"轻功水平提高了！\n"NOR);
	remove_action("walk_snow", "walk");
