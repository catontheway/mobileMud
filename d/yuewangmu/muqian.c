...
void create()
{
        set("short", HIY"墓前广场"NOR);
	set("long", @LONG
    这是墓前的一小片广场，来这里悼念岳王的人是络绎不绝，但其中也不乏
一些鸡鸣狗盗之徒打算在这里盗墓，偷取传说中的武穆遗书和岳家枪法。
LONG
	);
        set("outdoors", "yuewangmu");

	set("exits", ([
                "north" : __DIR__"yuefeimu",
                "south" : __DIR__"xiaodao1",

	]));

        set("objects", ([
                __DIR__ + "npc/baodating1" : 1,
        ]));
...
*/
        ...
        for(i=0;i<times;i++)
        {
                ...
                {
                    if (arg=="spear" && me->query("adv_weapon/class/spear/lv")>=4)//高级兵器级别大于4级，加25%效率 Zine
                    {
