...
void create()
{
  set_name( HIW"密信"NOR , ({"du's mixin","letter", "xin"}));
  set_weight(500);
...
int tixing()
{
    ...
    if (environment(ob))
    {
        tell_object(environment(ob),BLINK HIC"你还没有去追回都大锦的财物呢。\n"NOR);
    }
...
//        where=TASK_D->random_place();
        ...
        {
	        tell_object(me,"你按照都大锦的指示，信上的字却没有显现，再试试吧。\n");
	        return 1;        	
        }
        ...
        write(HIW"你按照都大锦的指示，打开迷信……原来要你到以下地方"+fullplace+"找(zhao/zhaoxun)到潜逃的伙计并要(yao)回其席卷的财物！\n"NOR);        
...
int do_dig()
{
    ...
    {
            huoji=new("/d/luoyang/npc/huaihuoji");
            if (huoji)
            {
