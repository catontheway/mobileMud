...
int ask_job()
{
  ...
  if ( me->query("combat_exp")<=3000000)
   ...
   killer->do_copy(me);
   killer->set_name(tmp[0],({tmp[1],pinying[0],pinying[1]}));
   killer->set("nick",HIW"白骨龙"NOR);
   where = TASK_D->random_place();
   killer->move(where);
   killer->random_move();
   killer->random_move();
   killer->random_move();
   message_vision("$N说道：“我收到消息，听说"+TASK_D->place_belong(where)+"有盗宝人"+killer->name()+"找到了闯王宝藏的地图,$n可否帮忙找回来！”\n",ob,me);
   command("care");
   return 1;   
}
