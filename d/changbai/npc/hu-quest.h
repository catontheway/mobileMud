...
//过滤非法任务地点和不明区域 by segate@pkuxkx
  ...
  message_vision("$N说道：『我收到消息，听说"+TASK_D->place_belong(where)
               +"有盗宝人"+ZJURL("cmds:tsdx "+killer->query("id"))+killer->query("name")+NOR+"("+killer->query("id")
               +")找到了闯王宝藏的地图,$n可否帮忙找回来！』\n",ob,me);
  command("care");
...
//  me->set("uni_target",killer);
 // if (wizardp(me))
  	tell_object(me, "盗宝人"+ZJURL("cmds:tsdx "+killer->query("id"))+killer->query("name")+NOR+"("+killer->query("id")
...
//过滤非法任务地点和不明区域 by segate@pkuxkx
 ...
 // if (wizardp(me))
  ...
  tell_object(me,"找在"+TASK_D->place_belong(where)+"的"+COLOR_D->uncolor(where->query("short"))+"的"+ZJURL("cmds:tsdx "+suicong->query("id"))+suicong->query("name")+"。"NOR);
  tell_object(me,"找在"+TASK_D->place_belong(where)+"的"+COLOR_D->uncolor(where->query("short"))+"的"+ZJURL("cmds:tsdx "+suicong->query("id"))+suicong->query("name")+"。"NOR);
  command("care");
