...
void create()
{
    ...
    set("long","输入bet开始预测博后，如果赢了，就自动从最低金额开始。
...
int do_yes()
{
   ...
   {
       return notify_fail("你已经开始预测了。要退出就quit或者输入fail\n");
   }        
...
int do_bet()
{
    ...
    //先警告
    write(HIR""BNK"警告！！！十预测九输，本预测博机风险巨大，既有暴富的机会，也有倾家荡产的可能。\n请仔细看规则：\n"NOR);
    write(sprintf(HIR"初始预测注是%d两黄金，如果输了，就自动加倍下注。如果赢了，自动从%d两黄金重新开始。\n"NOR,minmoney/10000,minmoney/10000));
    write(sprintf(HIW"输赢的规则很简单，丢一个筛子，1,2,3点你输，4,5,6 点你赢，如果筛子立起来，也是你输。\n也就是说，你赢的概率接近1/2，少一丁点。\n"NOR));
    write(HIR"一旦开始，银行存款将被冻结，除非你退出游戏或者来这里输入fail指令。否则预测博机会自动从你的银行存款中支钱。结束的条件只有一个：\n"NOR);
    write(sprintf(HIR"你的存款不足以支付下一次的预测注,或者预测注达到上限%d两黄金。\n"NOR,maxmoney/10000));
    write(HIY"输入yes表示已经阅读并理解以上说明，正式开始预测博。\n"NOR);
...
int betforuser(object user)
{
    ...
    else//输了
    {
        ...
        if(user->query("bet/all_money") >=   user->query_temp("bet/next_bid")*2)
        {
			...
			{
				tell_object(user,sprintf("预测注已经超过上限：%d黄金。\n",maxmoney/10000));				
			...
			{
				tell_object(user,sprintf("你将预测注增加一倍到:%d两黄金\n",user->query_temp("bet/next_bid")*2/10000));
