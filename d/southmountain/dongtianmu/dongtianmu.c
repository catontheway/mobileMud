...
string AREA()//本区域目录
{
	return "/d/southmountain/dongtianmu/";
}
void create() 
{ 
        set("short", "东天目");
...
LONG
   ...
   //"south" : __DIR__"zoudao2.c",
		...
		set("drug_level",35);//这里是这个副本出现药材的级别在此以下，副本内可以升级的采药术在35+10以下。
		set("level_bonus",20);//过此级别额外奖励
        set("migong_name","东天目"),
        set("no_clean_up", 1);
        set("no_reset", 1);
...
string welcome_msg(object me)
{
	...
	return HIG"【东天目山】山中有很多药物，最高为35级药物，最高提升玩家采药术到45级。\n"NOR;
}
int start()
{
    ...
    object exit4=load_object(__DIR__"shanlun1");
    set("migong_name","东天目山"),
    gene("山地",1,ob,exit1,exit2,exit3,exit4,0); //样本，随机迷宫8个参数，第一个为迷宫地貌类型，在include/random_map.h可查，第二个为迷宫复杂度，3为迷宫原点，4-7为出口，无出口则需补0,第八个参数为迷宫出口对于真实地图是否可见，不可见为1，可见为0
    return 1;
}
