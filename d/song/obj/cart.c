...
{       
      ...
      //  set("no_get", "把车队捡起来？这个想法很有创意！");
        ...
        set("unit", "队");
        set("long", "这是一队押运大宋物资的车队，你可以领(ling)队前行。\n");
        call_out("destroy",1500);
}

int set_long(object me,object ob)
{
	if (!me)
	return 1;
	set("owner",me->query("id"));
	set("long","这是一队由"+me->query("name")+"押运大宋物资的车队，从"+ob->query_temp("songtasks/gongbu/yuhen_from_area")+"的"+ob->query_temp("songtasks/gongbu/yuhen_from_short")+
		"押往"+ob->query_temp("songtasks/gongbu/yuhen_to_area")+"的"+ob->query_temp("songtasks/gongbu/yuhen_to_short")+"，可以领(ling)队前行。\n");
	set_temp("songtasks",ob->query_temp("songtasks"));
	ob->delete_temp("songtasks");
}
int destroy()
{
...
{        
        ...
        if (environment()==query_temp("songtasks/gongbu/yuhen_to") 
             ||(environment()->query("short")==query_temp("songtasks/gongbu/yuhen_to_short")&&
				TASK_D->place_belong(environment())==query_temp("songtasks/gongbu/yuhen_to_area")))
		{
			tell_object(me,"你历尽辛苦，终于完成了运送物资的任务，快回工部去复命吧。\n");
			me->set_temp("songtasks/gongbu/2/finish",1);
			destruct(ob);
            return 1;
	    }
		else
