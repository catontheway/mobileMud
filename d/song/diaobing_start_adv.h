...
int do_diao()
{
	...
	if (me->query_temp("songtasks/"+this_object()->query("songtasks/task_myself")+"/task")!=1&&me->query_temp("songtasks/"+this_object()->query("songtasks/task_myself")+"/task")!=3)
	{
	...
	if (me->query_temp("songtasks/"+this_object()->query("songtasks/task_myself")+"/task")==1)
	message_vision("$N传令军中，调出了一营"+this_object()->query("songtasks/task_myid")+"士兵，准备换防"+me->query_temp("songtasks/"+this_object()->query("songtasks/task_myself")+"/huanfang_target_name")+"驻地。\n",me);
	if (me->query_temp("songtasks/"+this_object()->query("songtasks/task_myself")+"/task")==3)
