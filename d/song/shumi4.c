...
void over2()
{
	...
	if (pl=find_player(high_ppl))
	{
		...
		tell_object(pl,"你的帅府被设在昆明，请尽快(3分钟内)去"+ZJURL("cmds:open")+HIG"赴任(open)吧。\n");
...
void over()
{
	...
	message_vision(CYN+"$N"+CYN+"接下来扫了场间众人一眼：“我们马上要做的就是"+ZJURL("cmds:decide")+HIG"决定(decide)出兵的主帅人选。\n”"+NOR,officer);
	...
	message_vision("$N道："+ppls+"“功勋卓著，我们要在一分钟内从中选出(decide)"+ZJURL("cmds:decide")+HIG"主帅。”\n",officer);
	set("nanzheng2",pls);
	call_out("over2",60);
}
int start()
