...
int do_zhili()
{
    ...
    if (me->query_temp("songtasks/dushui/task")!=2)
    {
    ...
    message_vision("$N卷起袖子和河工们一起堆高钱塘江堤。\n",me);
    me->set_temp("songtasks/dushui/2/finish",1);
...
{  
    ...
    if (me->query_temp("songtasks/dushui/task")==2 && !me->query_temp("songtasks/dushui/2/finish") && random(10)>5)
    {
        this_object()->delete("exits/"+this_object()->query("juedi_dir"));
        this_object()->set("juedi",1);
        call_out("rebuild",60);
        return notify_fail("你刚要前行，忽然发现江水决堤，你赶紧开始治理(zhili)吧。\n");
    }
