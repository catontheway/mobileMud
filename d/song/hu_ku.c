...
int theif_appear(object me)
{
	...
	me->set_temp("songtasks/hubu/hu_zei",time());
	return 1;
}
int do_zhuo()
{
	object me=this_player();
	if (me->query_temp("songtasks/hubu/task")!=3)
	{
		return 0;
	}
	if (!me->query_temp("songtasks/hubu/hu_zei"))
	{
		return 0;
	}
	if (me->is_busy())
	...
	if (me->query_temp("songtasks/hubu/3/finish"))
	{
		tell_object(me,"你不是已经抓到一个内鬼了吗？水至清则无鱼啊，回去复命吧。\n");
		return 1;
	}
	if (time()-me->query_temp("songtasks/hubu/hu_zei")>5)
	{
		tell_object(me,"你的行动太慢了，内鬼已经溜走了。\n");
		return 1;
	}
	me->set_temp("songtasks/hubu/3/finish",1);
...
string reshow_ansi(string *ansis,object me)
{
	...
	pool-=({o});
	p=pool[0];
	me->set_temp("songtasks/hubu/z/line",({a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p}));
...
int show_mess(object me)
{
	...
	me->set_temp("songtasks/hubu/z/line1",count);
	count=({});
	for (i=0;i<5+random(3);i++)
	{
		n=random(sizeof(ansis));
	...
	me->set_temp("songtasks/hubu/z/line2",count);
	count=({});
	for (i=0;i<5+random(3);i++)
	{
		n=random(sizeof(ansis));
	...
	me->set_temp("songtasks/hubu/z/line3",count);
	me->set_temp("songtasks/hubu/z/total",total);
	...
	me->set_temp("songtasks/hubu/z/replay",HIC+"a排货物      "+NOR+line1+"\n"+HIC+"b排货物      "+NOR+line2+"\n"+HIC+"c排货物      "+NOR+line3+"\n"
...
{
	object me=this_player();
	if (me->query_temp("songtasks/hubu/4/finish"))
	{
		tell_object(me,"别玩了，快回去复命吧。\n");
		return 1;
	}
	if (!me->query_temp("songtasks/hubu/z/replay"))
	{
		return 0;
	}
	tell_object(me,me->query_temp("songtasks/hubu/z/replay"));
	return 1;
}
int do_zhengli()
{
	object me=this_player();
	if (me->query_temp("songtasks/hubu/task")!=4)
	{
		return 0;
	}
	if (me->query_temp("songtasks/hubu/4/finish"))
	...
	if (me->query_temp("songtasks/hubu/hu_zhengli")==2)
	{
		tell_object(me,"快些着手把。\n");
		return 1;
	}
	me->set_temp("songtasks/hubu/hu_zhengli",2);
	show_mess(me);
	return 1;
}
...
int record(object me,string line,int posi)
{
	string *lines=({});
	int *posis=({});
	if (me->query_temp("songtasks/hubu/z/done_line"))
	{
		lines=me->query_temp("songtasks/hubu/z/done_line");
	}
	if (me->query_temp("songtasks/hubu/z/done_posi"))
	{
		posis=me->query_temp("songtasks/hubu/z/done_posi");
	}
	lines+=({line});
	posis+=({posi});
	me->set_temp("songtasks/hubu/z/done_line",lines);
	me->set_temp("songtasks/hubu/z/done_posi",posis);
}
int do_guilei(string arg)
{
	...
	if (!me->query_temp("songtasks/hubu/z/total"))
	{
		return 0;
	}
	if (me->query_temp("songtasks/hubu/4/finish"))
	...
	if (me->query_temp("songtasks/hubu/z/done_line"))
	{
		done_line=me->query_temp("songtasks/hubu/z/done_line");
	}
	if (me->query_temp("songtasks/hubu/z/done_posi"))
	{
		done_posi=me->query_temp("songtasks/hubu/z/done_posi");
	}
	...
	if (line=="a")
	{
		lines=me->query_temp("songtasks/hubu/z/line1");
	}
	else if (line=="b")
	{
		lines=me->query_temp("songtasks/hubu/z/line2");
	}
	else if (line=="c")
	{
		lines=me->query_temp("songtasks/hubu/z/line3");
	}
	else
	{
	...
	base_line=me->query_temp("songtasks/hubu/z/line");
	if (posi<1||posi>sizeof(me->query_temp("songtasks/hubu/z/line1"))+1)
	{
	...
	{
		me->add_temp("songtasks/hubu/z/succ",1);
		record(me,line,posi);
		tell_object(me,"你整理了一件货品入库。\n");
		if (me->query_temp("songtasks/hubu/z/succ")>me->query_temp("songtasks/hubu/z/total")/2)
		{
			tell_object(me,"你归类顺利，快去复命吧！\n");
			me->set_temp("songtasks/hubu/4/finish",1);
			return 1;
		}
		return 1;
	}
	...
	{
		me->add_temp("songtasks/hubu/z/fail",1);
		record(me,line,posi);
		tell_object(me,"乱堆乱放，你把一件货品弄错了地方。\n");
		if (me->query_temp("songtasks/hubu/z/fail")>me->query_temp("songtasks/hubu/z/total")/3)
		{
...
void init()
{
	...
	if (me->query_temp("songtasks/hubu/huku"))
	{
		return;
	}
	me->set_temp("songtasks/hubu/huku",1);
	call_out("desc",1,me);
