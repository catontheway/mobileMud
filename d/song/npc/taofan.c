...
int info_spread()
{
	...
	if (sizeof(obs))
	{
		...
		benefit_people+=({p1});
		tell_object(p1,"听说大宋钦犯"+ob->name()+"正在此地的"+environment()->query("short")+"现踪，你去守着他，等大宋的官员到了，好处不会少了你的。\n");
	}
	obs-=({p1});
	if (sizeof(obs))
	{
		...
		benefit_people+=({p2});
		tell_object(p2,"听说大宋钦犯"+ob->name()+"正在此地的"+environment()->query("short")+"现踪，你去守着他，等大宋的官差到了，好处不会少了你的。\n");
	}
...
void die()
{
    ...
    if (killer->query_temp("songtasks/xingbu/task")==4)
    {
        killer->set_temp("songtasks/xingbu/4/finish",1);
