...
set_name("本参大师", ({ "bencan dashi", "bencan" }));
        set("long",
                "他就是本参大师。\n"
                "他是本因方丈的师弟本参大师，精通少泽剑。\n");
...
void init()
{
        ...
        ::init();
        ob=this_player();
        if((int)ob->query("PKS")>=10&&(string)ob->query("family/master_id")=="bencan")
        {
...
void attempt_apprentice(object ob)
{
	...
	{
       		command("say "+RANK_D->query_respect(ob)+"，你现在拜我似乎会有很大损失，你能拿定"+ZJURL("cmds:answer 是")+HIG"主意吗？(answer)");
	return;		
	}	
