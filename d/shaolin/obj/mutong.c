...
void create()
{
	...
	if( clonep() )
		...
		set("unit", "对");
		set("long", "这是挑水用的木桶，你可以用手"+ZJURL("cmds:carry mu tong")+HIG"提(carry)着它。\n");
		set("value", 1000);
		set("no_get", 1);
		set("material", "wood");
	}
...
//几率是（100-先天身法*2-基本轻功/2)/1000
			if(me->query_skill("dodge",1)<100&&
			 random(2000)<(100-me->query("dex")*2-me->query_skill("dodge",1)/2)) {
				...
				if(query("water")<=0) {
					//set("water",0);
					tell_object(me, "你没有保持住身体平衡，一个倾斜差点把木桶里面剩下那点水全撒了！\n");
				}
				else
					tell_object(me, "你没有保持住身体平衡，一个倾斜差点把木桶里面的水撒了！\n");
			}
			remove_call_out("carrying");
			call_out("carrying",1,me,environment(me));
		}
