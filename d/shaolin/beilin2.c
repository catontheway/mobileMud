	...
	if( sscanf(arg, "stone %s", dir)==1 && (int)query("poem_said")==1 ) {
		if( dir=="up" ) {
			message_vision("$N将石碑往上抬...，忽然「喀」一声石碑又移回原位。\n",
				this_player());
			add("up_trigger", 1);
			if (query("up_trigger")>4) set("up_trigger",1);	//by all，超过4次重新开始
			check_trigger();
			return 1;
		} else {
			write("你要将石碑推向那个方向？\n");
	...
	if( sscanf(arg, "stone %s", dir)==1 && (int)query("poem_said")==1 ) {
		if( dir=="down" ) {
			message_vision("$N将石碑往下压...，忽然「喀」一声石碑又移回原位。\n",
				this_player());
			add("down_trigger", 1);
			if (query("down_trigger")>3) set("down_trigger",1);//by all，超过3次重新开始					
			check_trigger();
			return 1;
		} else {
			write("你要将石碑推向那个方向？\n");
...
//		add("poem_said", 1);
		set("poem_said", 1);  //by all，如果说的次数超过1次，就不会触发洞口机关了。	
		return 1;
	}
}
