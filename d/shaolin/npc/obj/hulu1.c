...
int do_catch(string arg)
{
	...
	{
		write("葫芦在你要上挂着呢，先"+ZJURL("cmds:jie hu lu")+HIG"解(jie)下来再说吧。\n");
		return 1;
	}
...
此刻正挂在你的腰上，里面装着一只"+HIW"冰蚕"NOR+"。\n"+"你可以把它"+ZJURL("cmds:jie hu lu")+HIG"解(jie)下来\n");
					}
...
int do_jie(string arg)
{
	...
	if (arg=="hu lu" && present("hu lu",me))
	{
		...
		if (me->query_temp("bingcan/gua"))
		{
			...
			if (!me->query_temp("bingcan/cansi") && !me->query_temp("bingcan/getsi"))
				{
					ob->set("long","这是慧净和尚用来装酒的葫芦，可以"+ZJURL("cmds:gua hu lu")+HIG"挂(gua)在腰间。\n");
				}
...
int do_cansi()
{
	...
	if (me->query_temp("bingcan/cansi")==80 )
	{
		...
		tell_object(me,HIR"你突然感觉寒气顿消，酒葫芦上的霜也消失殆尽，似乎冰蚕已经死了。快"+ZJURL("cmds:za hu lu")+HIG"砸(za)开葫芦看看吧。\n"NOR);
