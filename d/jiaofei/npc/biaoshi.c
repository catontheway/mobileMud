...
void create()
{
        set_name(get_rndname(), ({get_rndid(),"biao shi","biaoshi" }));
        set("nickname",HIW"镖师"NOR);
        set("long","福威镖局重金请来的镖师，看起来有两下子！\n");
        set("gender", "男性" );
...
int checking(object me, object ob)
{
        ...
        if ( (!present(ob->query("id"), environment()) || !living(ob)) && present("biao dui",environment()) )
           {  
                command("heng");
                message_vision("$N护送着镖队匆匆离开。\n", me);
...
{
				object biaodui=present("biao dui",environment());
				if(biaodui) destruct(biaodui);
