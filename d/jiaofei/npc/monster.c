...
void attack()
{
    ...
    {        
        if(level>random(20))
        {
            ...
            msg = "$N一口咬向$n的$l\n"; 
            this_object()->set_temp("apply/damage",level*10);
            this_object()->set_temp("apply/attack",level*10);
            ...
            msg = "$N对着$n的$l喷出一口毒气。\n"; 
            COMBAT_D->do_attack(this_object(),ob,query_temp("weapon"), TYPE_REGULAR,msg);                        
            this_object()->set_temp("apply/damage",0);
            this_object()->set_temp("apply/attack",0);            
        }        
    }
    remove_call_out("attack");
    call_out("attack",3);    
...
void init_me()
{
    ...
    set("qi",1500*level);
    set("max_jing",1500*level);
    set("eff_jing",1500*level);
    set("jing",1500*level); 
    set("max_jingli",1500*level);
    set("jingli",1500*level);      
    ...
    ob->set("value",random(level*10000)/100*100+10000);
    ob->move(this_object());
