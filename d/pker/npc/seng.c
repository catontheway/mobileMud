...
void create()
{
	set_name("扫地僧", ({ "saodi","seng"}));
	set("str", 30);
	set("gender", "男性");
	set("age", 20);
	set("long", "少林弟子，不过眉宇之间似乎少点善意\n");
...
int get_msg()
{
   ...
   object death,where;
   string deathname;
   if(!me->query("pker")||me->query("spyname")!="少林扫地僧"||!me->query("deathname"))
   {
   return 0;
   }
   ...
   if(!death=find_player(deathname))
    {  
      message("vision",HIR"扫地僧告诉你：你要杀的人现在不在线上!\n"NOR,me);
      return 1;
    }
...
int get_kill(string arg)
{
  ...
  me=this_player();
  ob=this_object();
  if((arg=="saodi"||arg=="seng")&&me->query("combat_exp")<2000000)
   {
     write("扫地僧嘿嘿一笑：你突然看到一把长剑已经架到了你的脖子上!\n");
     me->unconcious();
     return 1;
   }
   return 0;
