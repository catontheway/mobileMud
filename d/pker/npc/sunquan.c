...
void create()
{
	set_name("孙拳", ({ "sun quan","sun"}));
	set("str", 20);
	set("gender", "男性");
	set("age", 20);
	set("long", "天龙弟子，不过好象不太讨人喜欢\n");
...
int get_msg()
{
   ...
   object death,where;
   string deathname;
   if(!me->query("pker")||me->query("spyname")!="天龙孙拳"||!me->query("deathname"))
   {
   return 0;
   }
   ...
   if(!death=find_player(deathname))
    {  
      message("vision",HIR"孙拳告诉你：你要杀的人现在不在线上!\n"NOR,me);
      return 1;
    }
...
int get_kill(string arg)
{
  ...
  me=this_player();
  ob=this_object();
  if((arg=="sun quan"||arg=="sun")&&me->query("combat_exp")<2000000)
   {
     write("孙拳嘿嘿一笑：你突然看到一把长剑已经架到了你的脖子上!\n");
     me->unconcious();
     return 1;
   }
   return 0;
