...
void create()
{
	set_name("小糊涂", ({ "hutu","xiao"}));
	set("str", 20);
	set("gender", "男性");
	set("age", 20);
	set("long", "丐帮弟子，一身脏兮兮的\n");
...
int get_msg()
{
   ...
   object death,where;
   string deathname;
   if(!me->query("pker")||me->query("spyname")!="丐帮小糊涂"||!me->query("deathname"))
   {
   return 0;
   }
   ...
   if(!death=find_player(deathname))
    {  
      message("vision",HIR"小糊涂告诉你：你要杀的人现在不在线上!\n"NOR,me);
      return 1;
    }
...
int get_kill(string arg)
{
  ...
  me=this_player();
  ob=this_object();
  if((arg=="hutu"||arg=="xiao")&&me->query("combat_exp")<2000000)
   {
     write("小糊涂嘿嘿一笑：你突然看到一把长剑已经架到了你的脖子上!\n");
     me->unconcious();
     return 1;
   }
   return 0;
