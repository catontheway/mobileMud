...
LONG
        ...
        set("exits",([
                "eastdown" : __DIR__"yhssg",
        ]) );
        	set("item_desc", ([
"木桩" : "其中一根最北边的木桩(zhuang)显得格外的粗壮\n"
		 ZJOBACTS2+ZJMENUF(2,2,12,40)+"木桩:jump zhuang\n",
    ]));
        set("outdoors","murong");
        set("no_clean_up",0);
         set("no_task",1);
...
{
        object me = this_player();
        string dayphase=NATURE_D->get_current_day_phase();
        mixed *local;
        int i, night, level;
        ...
        night = 0;
        local = localtime((time() % 86400)*60);
//        if (local[2] < 5 || local[2] >= 19) night = 1;
        if(dayphase=="晚上" || dayphase=="午夜" || dayphase=="凌晨") night=1;
        if (!arg) {
                if (me->query_temp("guanxing_zhuang")) {
                	//午夜时分
...
LONG
                        ...
                        //else if (local[2] < 5)
                        	//后半夜
                                ...
                                set("long", @LONG
你站在木桩上向四处张望，此时已是凌晨时分，除了偶尔传来的几声虫鸣
...
LONG
                        ...
                        //else if (local[2] < 15)
                        else if(dayphase=="正午"||dayphase=="晌午")
                        	//正午
                                set("long", @LONG
你站在木桩上向四处张望，已是中午时分，在明亮的阳光照射下，你大致
...
LONG
        ...
        if (arg == "sky") {
                ...
                if (level >= me->query_skill("shenyuan-gong", 1)) {
                        write("你仰首望天，天上繁星点点，你觉得星斗的移动与你所学的“斗转星移”有莫大的联系，却因神元功太低无法领会。\n");
                        return 1;
                }
                ...
                if (me->query("jing") < level * 10 / 7) {
                        write("可惜你太累了，无法集中精神来研究夜空中的星斗。\n");
                        return 1;
                }
                me->receive_damage("jing", level * 10/ 7);
                me->improve_skill("douzhuan-xingyi", me->query("int") * 30);
