...
void create()
{
        set("short", HIC"雁门关"NOR);
        set("long", @LONG
这里是威震天下的一座雄关—雁门关，万里长城上千年以降，一直是
汉家的藩篱，走近城关，你会在城门城砖上看到一幅对联：“三关要隘无
双地，九塞尊崇第一关。”
LONG
        );

        set("exits", ([
               "southdown" : __DIR__"guandaon2",
               "northeast" : "/d/saiwai/guandaosw",
...
*/
        ...
        for(i=0;i<times;i++)
        {
                ...
                {
                    if (arg=="halberd" && me->query("adv_weapon/class/halberd/lv")>=4)//高级兵器级别大于4级，加25%效率 Zine
                    {
