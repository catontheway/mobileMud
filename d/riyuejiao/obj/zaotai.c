...
string long()
{
	...
	msg = replace_string(this_object()->query("long"),"\n",ZJBR);
	
	str = ZJOBLONG+msg+"\n";
	str += ZJOBACTS2+ZJMENUF(2,2,10,30);
	str += "点火:dianhuo"ZJSEP"加柴:add"ZJSEP;
    str += "做豆腐:zuocai dou fu"ZJSEP"做白菜:bai cai"ZJSEP;
    str += "做茄子:zuocai qie zi"ZJSEP"做粉条:fen tiao";
