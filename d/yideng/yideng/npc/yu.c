...
void seeing(object ob)
{
        ...
        switch( random(3) ) {
           ...
           say("
			...
			那鱼受了惊吓，转眼之间，钻进了"+ZJURL("cmds:jump pool")+HIG"水潭"NOR"（pool)底的岩石之下，再也不出来了。\n");
...
int do_jump(string arg)	
{
  ...
  if(user)
  	return notify_fail("你的负重太高了，跳不进水潭。\n");
        	
	if (((int)this_player()->query_temp("marks/渔3")) && ( arg = "pool" ))
	{
    this_player()->move("/d/yideng/yideng/pool",1);
	...
	object boat;
        if (ob->query("id")=="jin wawa")
     {
			...
			write("渔夫匆匆忙忙地跑进小屋，把金娃娃放好。");
                        boat=new("/d/yideng/yideng/obj/boat");
