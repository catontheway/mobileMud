...
LONG
  ...
  set("exits", ([     
	...
	set("item_desc", ([
		...
		ZJOBACTS2+ZJMENUF(2,2,12,40)+"爬石柱:climb shizhu\n",
		"石柱" : "石柱表面并不光滑，似乎可以爬(climb)上去。\n"
		ZJOBACTS2+ZJMENUF(2,2,12,40)+"石柱:shizhu\n",
		"suolian" : "一条粗大的锁链正紧紧地绑住你的上半身，不过在水里泡得久了，锁链上全是锈迹，你胸前的几个环扣都已经锈透了，你可以试试，能不能把它崩断(beng)。\n"
		ZJOBACTS2+ZJMENUF(2,2,12,40)+"崩断:beng suolian\n",	
...
int do_climb(string arg)
{
	...
	if( arg=="shizhu" )
		message_vision(
				"\$N爬上了一根石柱，四下看了看，发现水牢东、西两面的水下各有一道"+ZJURL("cmds:push men")+"闸门(men)。\n", me);
