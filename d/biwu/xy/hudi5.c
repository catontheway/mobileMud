...
LONG
	...
	set("no_clean_up", 0);
	set("item_desc", ([
"湖底" : "湖底潜流冲击，你几乎站立不稳，很难控制游的方向。\n"
		 ZJOBACTS2+ZJMENUF(2,2,12,40)+"游:you\n",
    ]));
	setup();
}
void init()
...
int do_swim(string arg)
{
	...
	if (me->query("qi")<8000) 
		{
			...
			me->set_temp("last_damage_from", "$N被湖底潜流冲击，口吐淤血而死。",me);
	...
	if (me->query_skill("dodge",1) < 400)
		...
		{
			message_vision("$N向西面游了过去，良久，终于看到岸边。\n", me);
			me->move(__DIR__"lakeside");
			me->add_busy(random(2));
			tell_room(environment(me), me->name() + "从水底冒了出来。\n", ({ me }));
		}
	return 1;
}