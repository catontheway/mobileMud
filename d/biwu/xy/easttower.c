...
void create()
{
	set("short", "震华门城楼");
	set("long", @LONG
你来到了城楼之上，极目远眺，城内形势一览无余(lan)。北面是一座箭楼，
飘扬着大宋的旗帜，但是被一扇巨大的铁门(men)堵住了。
LONG
	);
	set("exits", ([ /* sizeof() == 3 */
		"down" : __DIR__"innercitye",
	]));
	set("item_desc", ([
		"men" : "一扇巨大的铁门，怕不止千斤重，你可以试着(tui)开它。\n"
		ZJOBACTS2+ZJMENUF(2,2,12,40)+"铁门:tui men\n",
	]));
	set("no_clean_up", 0);
			...
			if(query("gate_hp") - 1 <= 0)
				{
					...
					set_temp("open",1);
					set("exits/north", __DIR__"shizilou");
				}
			else
				{
					add("gate_hp",-1);
