// NPC: /d/marry/npc/xi-laifu.c  办喜宴出现的喜来福老板
#include <ansi.h>
inherit NPC;
int ask_party();
...
void create()
{
   ...
   set("per", 20);
	...
	set("inquiry", ([
		...
		"here": "喜来福摇头晃脑地说：“我们醉仙楼可是方圆几十里的名楼，办喜宴还得要来我们这里。”\n",
		"宴" : (: ask_party :),
		"喜宴" : (: ask_party :),
		"席" : (: ask_party :),
		"酒席" : (: ask_party :),
...
{
	CHANNEL_D->do_channel(this_object(), "jh", name()+"一拱手道：“良宵一刻值千金，酒不醉人人自醉，今日迎宾楼荣幸地为"+query("name1")+
		"和"+query("name2")+"\n大开喜宴，恭迎各位前来捧场！");
...
int give_money(object me,object ob)
{
	...
	if( !me || environment(me) != environment(ob) )
	{
		...
		write_file("/log/money/jilu",sprintf("%s(%s)%s",me->name(),me->query("id"),ob->query("money")));
		ob->delete_temp("ready_to_party");
		ob->move("/u/advent/workroom");//不要直接dest，以备查
	   return  1;
	}
...
//因此改成进帐户
	message_vision("\n喜老板笑呵呵地走到$N面前，一挥手，旁边的小伙计捧过来一本帐本，喜老板说道：\n"
        ...
        me->add("balance",(int)ob->query("money"));
	ob->delete_temp("ready_to_party");
  	ob->move("/u/advent/workroom");//不要直接dest，以备查
	return 1;
}
...
int do_steal(string arg)
{
	...
	me->set("jianyu/where","yangzhou");//被捕地点
	me->apply_condition("jianyu",20);//做牢时间
        me->move("/d/wizard/punish_room");//进监狱
	return 1;
}
...
int kick(object me)
{
	...
	tell_room("/d/city/guangchang","突然一个人影从西边的迎宾楼二楼飞出来，“蓬”地重重摔在地上，细看却是一个差点摔死的人。\n");
	me->set_temp("kick",1);
	me->move("/d/city/guangchang");//移到楼下的大街房间里即可
