...
void create()
{
        set_name("媒婆", ({ "mei_po","po" }) );
	set("title", HIM"无事忙"NOR);
	set("gender", "女性" );
	set("age", 43);
	set("long","一位精明能干的老媒婆。\n");
	...
	set("inquiry", ([
		...
		"离婚" : "要...离婚(unmarry)？唉...！我可是看你们成婚的呀！\n"
      		"不过你们要是一定要离的话，那得去扬州知府处办理了！”\n",
		"unmarry" : "媒婆脸色一变，有点勉强地笑道：“要...离婚(unmarry)？唉...！我可是看你们成婚的呀！\n"
      		"不过你们要是一定要离的话，那得去扬州知府处办理了！”\n",
		"费用":"媒婆嘻嘻笑道：“结婚这么大的事，花费的地方可不少，要10 gold，,而离婚\n"
      		"更要花费不少钱，所以要100 gold。”\n",
	]) );
...
int do_marry(string arg) //判断结婚
{
	...
	ob->move("/d/marry/yinyuan-tang");

	me->set_temp("pending/pro",1);
	obj->set_temp("pending/pro",1);//婚礼中

        me->set("marry/lover",obj->query("id"));
        obj->set("marry/lover",me->query("id"));
	...
	obj->set("couple/couple_id",me->query("id"));//正式结婚参数
        chan=me->query("channels");
        me->set("channels",chan+({ "ct" }) );
        chan=obj->query("channels");
        obj->set("channels",chan+({ "ct" }) );
...
int ready_marry(object ob,object me,object obj)//准备婚礼
{
	...
	if(!( room = find_object("/d/marry/yinyuan-tang")) )
	room = load_object("/d/marry/yinyuan-tang");
...
"			     ■■■  ■■■\n\n"NOR);
	...
	case 10:
  	  	...
  	  	m_cloth = new("/d/marry/npc/obj/longpao");
  	  	w_cloth = new("/d/marry/npc/obj/yunxiu");
...
int do_back(object ob)//结束
{
	...
	ob->move("/d/marry/hongniang-zhuang");
	return 1;
}
...
int do_bai(string arg)//拜堂
{
	...
	if( !environment()||base_name(environment()) != ("/d/marry/yinyuan-tang") )
	...
	if(arg==" 天地"||arg==" 高堂"||arg==" "+(string)me->query("couple/couple_id"))
		return notify_fail("“拜”与后面的词之间请用半角空格，夫妻对拜请拜对方id。\n");

	if(!arg ||(me->query_temp("pending/bai")==1 &&arg!="天地")
		||(me->query_temp("pending/bai")==2 && arg!="高堂")
		||(me->query_temp("pending/bai")==3 && arg!=(string)me->query("couple/couple_id")))
...
int last_marry(object ob,object me,object obj)//拜堂结束
{
	object moon,room;
	if(!(room = find_object("/d/marry/yinyuan-tang")))
		room = load_object("/d/mary/yinyuan-tang");
	moon = present("yue lao", room);
	moon->move("/u/advent/workroom");
	ob->delete_temp("wait_time");
	...
	message_vision(HIR"伴郎伴娘拥着两位新人，上了门口的两顶大红花轿，只听轿夫们一声“起轿！”\n"
   		"一行人吹吹打打地奔向醉仙楼......\n"NOR,me,obj);
	me->move("/d/marry/jiao1");
	obj->move("/d/marry/jiao2");
...
int go_lou(object me,object obj)
{
	...
	message_vision("\n    过了好长一会儿，只听轿夫们一声“到啦！”轿子整个儿一震，想是落到了地上，\n"
   	"$N和$n从轿子中下来一看，原来已经到了醉仙楼。两人又被拥扶着一直上了三楼喜福楼。\n"
      "只听一阵呵呵笑声，醉仙楼大老板喜来福笑容满面地迎出来：“恭喜两位新人！恭喜恭喜！”\n",me,obj);
	npc=new("/d/marry/npc/xi-laifu");
	npc->set("name1",me->name());
	npc->set("name2",obj->name());//双方姓名
	npc->move("/d/marry/xifu-tang");
	me->move("/d/marry/xifu-tang");
	obj->move("/d/marry/xifu-tang");
	return 1;
}
