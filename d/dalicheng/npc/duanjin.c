...
int accept_object(object who,object ob)
{
	...
	if (who->query_temp("marks/arrestee") != ob->query("name")
		|| ob->get_damage_origin_object() != who
...
{
	object ob, me=this_object();
	if (objectp(ob = me->get_damage_origin_object())) ob->add("dali/trust", -1);
	::unconcious();
}
void die()
{
...
//title系统记录玩家title by seagate@pkuxkx
	    who->set_title(TITLE_RANK, "大理国下府副将");
...
//title系统记录玩家title by seagate@pkuxkx
	    who->set_title(TITLE_RANK, "大理国下府主将");
...
//title系统记录玩家title by seagate@pkuxkx
	    who->set_title(TITLE_RANK, "大理国中府副将");
...
//title系统记录玩家title by seagate@pkuxkx
	    who->set_title(TITLE_RANK, "大理国中府主将");
...
//title系统记录玩家title by seagate@pkuxkx
	    who->set_title(TITLE_RANK, "大理国上府副将");
		...
		if (who->query("dali/arrestmade")>=10000 && who->query("dali/duty")==5 && who->query("dali/rank")==3 && who->query("dali/jobdone")>=6000 && !who->query("dali/findduan") && !who->query_temp("dali/uprank"))
		{
			...
			command("congra");
			ob = new("/d/dalicheng/obj/work-letter1");
			ob->move(this_object());
...
//title系统记录玩家title by seagate@pkuxkx
	    who->set_title(TITLE_RANK, "大理国上府主将");
...
//title系统记录玩家title by seagate@pkuxkx
	    who->set_title(TITLE_RANK, "大理国御林军都督");
			command("congra");
...
//title系统记录玩家title by seagate@pkuxkx
	    who->set_title(TITLE_RANK, "大理国兵马总管");
			command("congra");
