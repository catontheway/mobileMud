...
void create()
{
    ...
    set("item_desc", ([
		...
		ZJOBACTS2+ZJMENUF(2,2,12,40)+"去扬州:qu 扬州"+ZJSEP+"下(xia)车:xia\n",
    ]));
...
int do_qu(string arg)
{
    ...
    if (arg=="yangzhou" || arg=="扬州")
    {
        if (!who->query("newbie_village/arrive"))
        {
    		...
    		{
    			write("你真的决定离开了吗？今日一别，再不能回。如果你下定了决心，请点这里》》"+ZJURL("cmds:qu 扬州")ZJSIZE(25)+"扬州\n"NOR);
    			who->set_temp("newbie_village/confirmleave",1);
    			return 1;
    		}
    		write(GRN"
外面将是一个新的世界。但你对未来的日子满怀信心，因为你知
道，自己的身体中蕴藏了巨大的潜能！\n"NOR);
...
void arrive_yz(object who)
{
    ...
    who->set("combat_exp", 3000);
    who->set("potential", 50000);
    who->set("pot/lxsz", 3500);
    who->set("max_neili", 3000);
    who->set("max_jingli", 3000);
    who->add("balance",10000);
	who->set_slill("dodge",80);
		who->set_slill("force",80);
		who->set_slill("parry",80);
		who->set_slill("strike",80);
		who->set_slill("cuff",80);
		who->set_slill("sword",80);
		who->set_slill("blade",80);
         message_vision("$N获得了一两黄金，3000点经验，5w点潜能作为闯入江湖的依靠。江湖险恶，忘好自为之！！！\n",who);
    tell_object(who,"大车停稳了下来，你可以"+ZJURL("cmds:xia")+"下车(xia)了。\n"NOR);
