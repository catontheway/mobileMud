...
void init()
{
    ...
    tell_object(me,ZJFORCECMD("knock gate"));
...
int show_localmaps()
{
    ...
    tell_object(me,ZJFORCECMD("localmaps"));
}
...
{
    if (1==check_questindex(me,"使用localmaps命令查看票号的位置")) return notify_fail("你还是先用localmaps查下柳秀票号的"+ZJURL("cmds:localmaps")ZJSIZE(25)+"位置"NOR"再走吧。\n");
