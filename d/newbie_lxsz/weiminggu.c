...
"隐隐可以通向外界。\n");
    ...
    set("item_desc", ([
		...
		ZJOBACTS2+ZJMENUF(2,2,12,40)+"喝水:drink hulu"+	ZJSEP+"拿葫芦:get hulu"+ZJSEP+"装水:fill hulu\n",
		"path":HIY"这是一段陡峭的山坡，应该可以向上攀爬出去。"NOR+HBBLU"(climb up)\n"
		ZJOBACTS2+ZJMENUF(2,2,12,40)+"攀爬:climb up\n",
        ]));

    set("exits", ([
		"southeast" : __DIR__"chemahang",
        "west" : __DIR__"shulin",
...
int do_hp()
{
	...
	tell_object(me,"你心里想，我吃完喝完直接去车马行出村岂不是更好，学了技能也带不出去。");
	tell_object(me,ZJFORCECMD("hp"));
	return 1;
}
	...
	for(i=0;i<sizeof(all_inv);i++)
	{
		...
		if(userp(me) && check_questindex(me,"使用look river或者l river命令，根据提示先让自己喝饱吃足。地上的野果也可以捡起来充饥。(get ye guo;eat ye guo)") == 1)
		{
			if ((me->query("food")>=me->max_food_capacity()*80/100) && (me->query("water")>=me->max_water_capacity()*80/100))
			{
				...
				tell_object(me,ZJFORCECMD("get ye guo"));
				tell_object(me,ZJFORCECMD("get hu lu"));
				tell_object(me,ZJFORCECMD("eat ye guo"));
				tell_object(me,ZJFORCECMD("drink hu lu"));
				tell_object(me,ZJFORCECMD("eat ye guo"));
				tell_object(me,ZJFORCECMD("drink hu lu"));
				tell_object(me,ZJFORCECMD("eat ye guo"));
				tell_object(me,ZJFORCECMD("drink hu lu"));
