...
柜台上有个大牌子写着：
    ...
    2008欧洲杯：葡萄牙  VS  德国    不让球  
              赔率代码: 1 主队胜   赔率：  2.40
              赔率代码: 2 打和     赔率：  3.00
              赔率代码: 3 客队胜   赔率：  3.10
      （截止投注时间：2008.6.19 24:00，兑奖时间2008.6.20白天）
\n"NOR
);
...
int  do_bet  (string  arg)
{
    ...
    if  (me->query("duqiu/what4") == 16 && me->query("duqiu/amount4") > 0)
        return  notify_fail("你已经预测过这场了，再玩其他场次吧。\n");

    if  (room_status  >  1)
        return  notify_fail("现在正在预测呢，稍等片刻。\n");

    me->set("duqiu/what4",16);  //确定本场预测局序号，以后预测局依次排序，防止bug
    me->set("duqiu/code4",code);
    me->set("duqiu/amount4",amount);
    ...
    log_file("static/xiazhu",sprintf("%s(%s) 于%s在葡萄牙对德国下注%d张cash，赔率代码为%s！\n",
    me->query("name"),me->query("id"),ctime(time()),me->query("duqiu/amount4"),me->query("duqiu/code4"))); 
...
int  do_betlist  (string  arg)
{
    object me; 
    me = this_player();
    if( me->query("duqiu/what4") != 16 )
    {
       write("你应该先下注这场球! \n");
       return 1;
    }
    if( !me->query("duqiu/amount4") )
       write("你没有下注这场球! \n");       
    else 
       write("你在本预测场买了"+me->query("duqiu/amount4")+" 张银票，赔率代码是"+me->query("duqiu/code4")+"。\n"); 
