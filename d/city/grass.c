...
void init()
{
    ...
    if (!me->query("mingpin")&&!me->query("ming/tj/pin"))
    {
		if (!me->query_temp("skf/rabbitfail"))
		{
			if ((random(50)>33) && (userp(me)))
			{
				...
				tell_object(me,HIW"你决定紧紧"+ZJURL("cmds:zhui")+HIG"追"NOR"(zhui)在小白兔的后面，也许能找到一条近路去桃源。\n"NOR);
	...
	else
	{
		...
		{
			message_vision(HIC"$N一声令下，渡河！大队人马开始准备纵马"+ZJURL("cmds:yuema")+HIG"越过(yuema)池塘。\n"NOR,me);
...
int make2(object me,object *zhu,object sheng)
{
	...
	if (random(10)>1)
	{
		...
		tell_object(me,HIY"也许你可以把竹筏"+ZJURL("cmds:tui")+HIG"推(tui)过池塘。\n"NOR);
	}
	else
	{
		me->add_busy(random(4));
