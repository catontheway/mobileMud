...
void create()
{
  ...
  if (clonep())
    ...
    set("unit", "张");
    set("value", 0);
    set("long","这是一张林镇南给你的密信，需要用特殊的药水才能"+ZJURL("cmds:xian letter")+HIG"显形(xian)。\n");
    set("no_store", 1);
    set("no_sell", 1);
    set("no_pawn",1);
    set("no_put",1);
...
//过滤非法任务地点和不明区域 by segate@pkuxkx
        ...
        write(HIW"你按照林镇南的指示，打开迷信……原来要你到以下地方"+fullplace+"找(zhao)到潜逃的伙计并要(yao)回其席卷的财物！\n"NOR);        
...
int do_dig()
{
    ...
    {
            huoji=new("/d/city/npc/huaihuoji");
            if (huoji)
            {
