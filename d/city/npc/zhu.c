...
void init()
{
    ...
    if (!this_player()->query_temp("move_notice"))
    {
        this_player()->set_temp("move_notice",1);
        t=19-tm[3];
        if (wizardp(this_player()))
        {
            tell_object(this_player(),tm[3]+"and"+t+"\n");
        }
        if (t>0)
        {
            tell_object(this_player(),BLINK+HIR+"朱熹道："+chinese_number(t)+"天后就搬回南昌岳麓书院讲学了。\n"+NOR);
        }
...
int accept_object(object who, object ob)
{
        ...
        {
           message_vision("朱熹道：“无欢说了，现在推行免费义务教育，免收一切学费”。\n", who);        
        }       
        return 0;
}
...
LONG);
  ...
  if (type=="respect")
    return notify_fail("现在尊称只能由江湖同道rank来获得了。\n");
...
int ask_question()
{
    ...
    int sameip = 0;
        ...
        {
            tell_object(me,"朱熹对你摇了摇头，说：猜谜太伤脑筋，你先休息一下吧。\n");
			if(time() - me->query("last_question_time") < 5)
			{
				if(query("startroom") == file_name(environment()))
				{
					me->move("/d/city/dongdajie1");
		            tell_object(me,"朱熹对你说道：过犹不及，你先出去。\n");
				}
			}
            return 1; 
        }
    ...
    if(sameip >= 2)
    {
        tell_object(me,"朱熹对你大吼：欺人太甚！汝以夫子何人也？贪婪若斯！\n");
        if(query("startroom") == file_name(environment()))
        {
            me->move("/d/city/dongdajie1");
            tell_object(me,"你被朱熹赶了出来。\n");            
        }
        return 1;
    }
...
int ask_job()
{
   ...
   tell_object(me,"朱熹说道：很好，你直接去泰山石经峪抄经（write jing）吧，工作完成后问我要奖励（ask zhu about done）。\n");
   tell_object(me,"朱熹说道：记住你的工号：\n");
   info = ANTIROBOT_D->mxpantirobot(me);
   me->set_temp("chaojing/code",info[0]);
   tell_object(me,"朱熹说道："+HIY"请注意，三分钟后查询系统自动清除你的工号！\n"NOR);
...
{ 
   ...
   tell_object(me,"朱熹说道：请到如下网址查询你的工号：\n");
...
int ask_done()
{
        ...
        command("say 报上你的口令！report "+ZJURL("cmds:report "+me->query_temp("chaojing/code"))+"口令！\n");
...
int ask_gift()
{
	...
	me = this_player();
	ob = this_object();
	times = 2 - me->query("gift/2008xmas/times");
	
	if (!me->query("gift/2008xmas/skill_name"))	//第一次要礼物
	{
		book = new("/d/gift/icer/xmasbook");
		book->move(me);
		if (!book->query("desting"))
		{
			message_vision("$n给$N一本" + book->name() + "。\n",me,ob);
			message_vision("$n说道：$N如果对这本书不满意或者不幸无法阅读，请不要读它（否则无法再换）\n再问我要一次礼物(ask zhu about gift)就可以换一本，你还有" + HIR+CHINESE_D->chinese_number(times)+NOR + "次更换机会！\n",me,ob);
			message_vision("$n说道：$N要注意：如果改换了门派，就不能再读这本书了！\n",me,ob);
		} else {
			message_vision("$n对$N说道：$P武功等级比较低，需要在你的师傅出现之后才能选择你能从这本书上学到的功夫，等你师傅出现了再来取礼物吧。\n",me,ob);
			return 1;
		}
	} else if (times >= 0)  {	//换礼物
			...
			if (me->query_skill(skill,1) > me->query("gift/2008xmas/old_skill"))
			{
				message_vision("$n对$N说道：$N已经学过口诀上的知识了，不能再给你换了。\n",me,ob);
				return 1;
			}
			//销毁已经给的书
			for (i=sizeof(inv)-1;i>=0;i--)
			{
				if (inv[i]->query("id")=="koujue book")
				{
					message_vision("$n从$N这取走了" + inv[i]->name() + "。\n",me,ob);
					destruct(inv[i]);
				}
			}
			me->delete("gift/2008xmas/skill_name");
			book = new("/d/gift/icer/xmasbook");
			book->move(me);
			if (!book->query("desting"))
			{
	...
	} else message_vision("$n对$N说道：$N已经用光了三次要礼物的机会，不要太贪心了，知足常乐。\n",me,ob);
	me->set("gift/2008xmas/times",3-times);
	return 1;
}
*/
