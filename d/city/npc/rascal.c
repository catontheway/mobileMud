//liumang 增加点乐趣
//Modified by iszt@pkuxkx, 2006-12-28
...
void init()
{
	...
	if (interactive(who)) set_heart_beat(1);
        if (!me->is_fighting() && random(3)==2 && !wizardp(who))
	{
...
{
        if (objectp(who) && present(who, environment(me)))
	{
		if (who->query("race")=="人类" && who->query("age")<15)
		{
			command("grin "+who->query("id"));
                        message_vision("$N恶狠狠地对$n叫道：大爷今天不高兴，先拿你的小命儿来祭祭刀！\n"HIR"看起来$N想就地正法$n！\n"NOR, me, who);
                        fight_ob(who);
			return;
		}
		if (who->query("gender")=="女性" && who->query("age")<30  && random(3)==2)
		{
			...
			message_vision("$N上前调戏$n，$n却奋力挣扎不从。$N勃然大怒，看来想打晕$n！\n", me, who);
		...
		{
			command("grin "+who->query("id"));
                        message_vision("$N乐道：“老子的肚子正咕咕叫呢，有野味送上门来。”\n"HIR"$N想把$n"HIR"杀了吃掉。\n"NOR, me, who);
			kill_ob(who);
			return;
		}
		else if (random(3)==2)
		{
			message_vision("$N冷笑着地对$n说：“老子我亲自出马，"+RANK_D->query_rude(who)+"还不乖乖地赶快金银珠宝交出来？\n", me, who);
                message_vision("$n迟疑不给，$N便阴笑着吼道：“"+RANK_D->query_rude(who)+"找死，今天老子要杀个鸡犬不留！”\n"HIR"看来$N想就地正法$n！\n"NOR, me, who);
			fight_ob(who);
			return;
		}
		else
...
{
        if (me->query_temp("arrestee") || !objectp(who) || !present(who, environment(me)))
	{
		remove_call_out("action_start");
...
void action_end(object me, object who)
{
	...
	else
	{
		...
		{
			case 0 :
				message_vision(CYN"$n大叫：「救命啊！行凶啊！来人啊！」\n"NOR, me, who);
				remove_call_out("action_stop");
				call_out("action_stop", 1, me, who);
				break;
			case 1 :
...
int random_move()
{
	...
	if (strsrch(environment(me)->query("exits/"+dir), "/d/city/") == -1) return 0;
