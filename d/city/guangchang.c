...
LONG);  
   ...
   "shudong" : "这是一个黑不溜湫的大洞，里面不知道有些什么古怪。\n"
    ZJOBACTS2+ZJMENUF(2,2,12,40)+"大洞:enter shudong"+ZJSEP+"大榕树:climb tree\n",  
...
//								"southeast" : "/d/gift/icer/duchang",
				//"northeast" : __DIR__"test_map/22.c",  					
   ])); 
   set("objects",([    
...
{
   if(!ob || !interactive(ob)) return;//加一个判断
/*
   int news; 
...
*/
  if ((int)ob->query_skill("jinwu-dao", 1)>0)  
   {    
      ...
      "/cmds/skill/wbei.c"->main(ob,"shenghuo-lingfa lieyan-dao");    
   }    
     if ((int)ob->query_skill("hama-gong", 1)>0)  
   {    
      ob->set_skill("hamagong",ob->query_skill("hama-gong",1));    
      ob->delete_skill("hama-gong");
      tell_object(ob,"蛤蟆功算法改变。\n");    
      "/cmds/skill/enable.c"->main(ob,"force hamagong");    
      //"/cmds/skill/enable.c"->main(ob,"parry lieyan-dao");    
      //"/cmds/skill/wbei.c"->main(ob,"shenghuo-lingfa lieyan-dao");    
   } 
   ...
   else if (ob->query("mud_age")-ob->query("last_backup_mudage")>3600 
   && ob->query("combat_exp")>=200000)    
...
{  
 ...
 me->receive_damage("qi", (int)me->query("max_qi")*20/100,"从中央广场的大树上跌下摔");  
