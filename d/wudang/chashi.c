...
"间采摘，并放在怀中用体温焙制，得天地精华，汇处子体香，\n"
"故名＂女儿香＂，据说喝了能提精补神，十分珍贵。屋里四周\n"
"得体地摆着些桌子(table)和椅子(chair)，坐满了武当的男女\n"
"弟子。南边有扇窗子(window)，景色宜人。\n"
	);
...
int do_jump(string arg)
{
    ...
    if ((dex <= 23)&&(random(3)<2) || (dex > 23)&&(dex <= 26)&&random(2) )  
    {
	    ...
	    if ( objectp(ob=present("xiang cha", this_player())) )
	    {
   		...
   		me->receive_damage("qi", 60, me, "跳出茶室摔");
   		me->receive_wound("qi", 40, me, "跳出茶室摔"); 
...
int do_tap(string arg)
{
    ...
    if( me->query_temp("marks/served") )
    {
        ...
        return notify_fail("");
         ...
         if (present("mi tao", environment(me))||present("wan", environment(me)))
                 return notify_fail("桌子上的还人动过呢。\n");
