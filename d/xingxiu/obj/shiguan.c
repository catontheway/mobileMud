#include <ansi.h>
// 炼毒还有点问题。需要设置corpse为自己杀的，同时check coprse是非帮助npc，exp不能太低。
// Add by Jpei, 20081225 for Xmas
inherit ITEM;
...
void create()
{
        ...
        set("no_put",1);
        set("unit", "张");
        set("long", "这是一张用整块巨石凿空而制成的石棺，上面残留了一些"+ZJURL("cmds:liandu")+RED"血迹"NOR"，
散发出的一股腐烂气味，弥漫了整个山洞。\n");
        set("material","stone");
        set("value", 5);
...
// 修改：使用condition的方式不太妥当，改为直接时间控制。by lordstar@2017/10/11
//        if(me->query_condition("xx_liandu"))
        if(me->query_temp("xx_liandu") && ((time()-(int)me->query_temp("xx_liandu"))<300) )
                return notify_fail("你刚刚炼毒完成不久，频繁炼毒会有生命危险的！\n");
        if( (int)me->query("neili") < me->query("max_neili")/2 )
                return notify_fail("你现在的内力不足，强制炼毒会很危险的。\n");
        if( me->query_skill("poison") > me->query_skill("huagong-dafa") )
                return notify_fail("你的化功大法无法控制毒技，无法通过练毒来提升了。\n"); 
        if( me->query_skill("poison") > 400 )
                return notify_fail("你的毒技已经无法提高，炼毒提高对自身会造成莫大伤害！\n"); 
...
{
		object ob;
       int neili_lost = (int)(me->query_skill("force")*10/201);
       ...
       me->improve_skill("poison", neili_lost);
       switch(period) {
       case 1:
       ...
       case 11: 
               ...
               me->improve_skill("force", me->query_con()*me->query_int());
               break;
       case 21:
...
上角，以屈下颊至；又从耳後入耳中，出走耳前，交颊。三路合于一处！\n"NOR);
              tell_room(environment(me), CYN+me->name()+"双掌如画太极般划动，只见腐尸上所呈现的青色开始慢慢聚合于其胸口，其色甚阴！\n"NOR,({ me }));
               me->improve_skill("force", me->query_con()*me->query_dex());
              break;
       case 51:
       ...
       case 71:
        ...
        if((int)me->query("neili") < 0 || (int)me->query("jingli") < 0){
               ...
               me->set("neili", 0);
//               me->apply_condition("xx_liandu", 20 + random(20));
			   if (objectp(ob=present("shi guan", environment(me))))
					over_liandu(ob);
               me->unconcious();
               return 0;
         }
         ...
         if(period==times){
                ...
                me->add("combat_exp", (int)me->query_skill("force")*me->query("con")/3);
                me->add("exp/liandu", (int)me->query_skill("force")*me->query("con")/3);
                me->add("potential", (int)me->query_skill("force")*me->query("con")/6);
                me->add("pot/liandu", (int)me->query_skill("force")*me->query("con")/6);
                me->add("shaqi", 15);
                me->add("shen", -1000);  
                write(YEL"\n你的经验增加了，你的杀气增加了。\n"NOR);
                me->improve_skill("huagong-dafa", me->query_con() * me->query_int());
//                me->apply_condition("xx_liandu", 10);
