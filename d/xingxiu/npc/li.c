...
void create()
{
	...
	set_skill("hand", 350);
	set_skill("xiaowuxiang-gong", 360);
	set_skill("zhaixing-shou", 350);
	set_skill("sword",350);
        set_skill("mingtian-jiushi",350);
	set_skill("parry", 380);
	set_skill("staff", 380);
...
//title系统记录玩家title by seagate@pkuxkx
	  this_player()->set_title(TITLE_RANK, HIR "星宿派" NOR + GRN +"魔头" NOR);
	}
	else
	{
		this_player()->set("title",HIR "星宿派" NOR + GRN +"毒姑" NOR);
//title系统记录玩家title by seagate@pkuxkx
	  this_player()->set_title(TITLE_RANK, HIR "星宿派" NOR + GRN +"毒姑" NOR);
	}
}
