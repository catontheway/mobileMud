// By Zine
               
#include <ansi.h>

inherit FAMILY_NPC;

varargs void wield_weapon(string weapon);
void die();
int init_robber();
void player_name()
{
	string *names=({"one","two","three","four","five","six","seven","eight","nine","ten","eleven","twelve","thirteen"}),post_name="娘",pronoun="她";
	int i=random(13);
	object user,*users=children(USER_OB);
	users=filter(users,(:environment:));
	user=users[random(sizeof(users))];
	if (user->query("gender")=="男性") 
	{
		post_name="郎";
		pronoun="他";
	}
	set_name(COLOR_D->uncolor(user->query("name"))[0..1]+chinese_number(i+1)+post_name,({user->query("id")+" "+names[i],"daomu zei","zei"}));
	set("gender",user->query("gender"));
	set("per",user->query("per"));
	if (!user->query("family"))
	set("long",pronoun+"是一名盗墓贼，听说和江湖上一个叫"+user->query("name")+"的人颇有瓜葛。\n");
	else
	{
		set("long",pronoun+"是一名盗墓贼，是武林中"+user->query("family/family_name")+"高手"+user->query("name")+"的亲戚。\n");
		set("family",user->query("family/family_name"));
	}
}

void create()
{
	set_name("贼",({"zei"}));
        
	set("long",
	"他是一名盗墓贼。\n"
         );
	set("is_KARMA_ROBBER_TOMB",1);
	set("gender", "男性");
	set("title",HIM"盗墓贼"NOR);
	set("age", 40);
	set("attitude", "peaceful");
	set("shen_type", 1);
   	set("str", 30);
	set("int", 25);
	set("con", 25);
	set("dex", 25);
	set("per", 25);
	set("max_qi", 1000);
	set("qi", 1000);
	set("neili", 900);
	set("max_neili", 900);
	set("jiali", 50);
	set("combat_exp", 650000);
	setup();
	carry_object("/clone/armor/pifeng")->wear();
	player_name();
	init_robber();
}
int init_robber()
{
	mapping party=([
		"少林派":"shaolin",
		"武当派":"wudang",
		"丐帮":"gaibang",
		"白驼山":"baituo",
		"星宿派":"xingxiu",
		"华山派":"huashan",
		"峨嵋派":"emei",
		"明教":"mingjiao",
		"日月神教":"riyue",
		"古墓派":"gumu",
		"灵鹫宫":"lingjiu",
		"全真派":"quanzhen",
		"天龙寺":"tianlong",
		"桃花岛":"taohua",
		"姑苏慕容":"murong",
		"朝廷":"chaoting",
		"天地会":"tiandihui",
		"神龙教":"shenlong",
		"大轮寺":"dalunsi",
		"雪山派":"xueshan",]);
	int exp=4000000+random(4000000),skill = to_int(pow(exp/100,0.333)*10),qi=10000+random(5000),jing=qi,neili=10000+random(10000),jingli=neili/2+random(neili/2);
	string *families=keys(party);
	set("combat_exp",exp);
	set("qi", qi);
	set("eff_qi",qi);
	set("max_qi",qi);
	set("jing", jing);
	set("eff_jing",jing);
	set("max_jing",jing);
	set("max_neili", neili);
	set("neili", neili);
	set("max_jingli", jingli);
	set("jingli", jingli);
	set("str", 25+random(5));
	set("int", 25+random(5));
	set("con", 25+random(5));
	set("dex", 25+random(5));
	if (query("family")&&party[query("family")])
	family_skill(this_object(), party[query("family")], skill, 1);
	else
	family_skill(this_object(), random(2)?"xie":"zheng", skill, 1);
	wield_weapon();
}
int Bonus(int base,int pl_num,float exp,float total_exp)
{
	float res;
	int result;
	res=to_float(base*pl_num);
	res=res*(exp/total_exp);
	result=to_int(res);
	return result>50?result:50;
}
void die()
{
	int exp=0,pl_num,bonus_exp=400,bonus_pot=1600,bonus_rep=1600,myExp,myPot,myRep;
	object env=environment(this_object()),*inv=all_inventory(env);
	if (env->query("level")!=2)
	::die();
	inv=filter(inv,(:userp:));
	inv=filter(inv,(:living:));
	if (sizeof(inv)<1)
	::die();
	pl_num=sizeof(inv);
	for (int i=0;i<pl_num;i++)
	exp+=inv[i]->query("combat_exp");
	for (int i=0;i<pl_num;i++)
	{
		myExp=REWARD_D->add_exp(inv[i],Bonus(bonus_exp,pl_num,to_float(inv[i]->query("combat_exp")),to_float(exp)));
		myPot=REWARD_D->add_pot(inv[i],Bonus(bonus_pot,pl_num,to_float(inv[i]->query("combat_exp")),to_float(exp)));
		myRep=REWARD_D->add_rep(inv[i],Bonus(bonus_rep,pl_num,to_float(inv[i]->query("combat_exp")),to_float(exp)));
		inv[i]->add_temp("karma/exp_gained",myExp);
		inv[i]->add_temp("karma/pot_gained",myPot);
		inv[i]->add_temp("karma/rep_gained",myRep);
		tell_object(inv[i],WHT+"打败"+this_object()->name()+WHT+"，你获得了"+myExp+"点经验，"+myPot+"点潜能，"+myRep+"点声望奖励。\n"NOR);
	}
	::die();
}
varargs void wield_weapon(string weapon)
{
	if(!weapon && query("use_weapon1"))
		wield_weapon(query("use_weapon1"));
	if(!weapon && query("use_weapon2"))
		wield_weapon(query("use_weapon2"));
	if(!weapon)
		return;
	switch(weapon)
	{
	case "sword":
		carry_object("/clone/weapon/changjian")->wield();
		break;
	case "blade":
		carry_object("/clone/weapon/gangdao")->wield();
		break;
	case "staff":
		carry_object("/clone/weapon/gangzhang")->wield();
		break;
	case "whip":
		carry_object("/clone/weapon/changbian")->wield();
		break;
	case "zhen":
		set("gender", "无性");
		carry_object("/d/city/obj/flower_shoe")->wear();
		carry_object("/d/city/obj/pink_cloth")->wear();
		carry_object("/d/city/obj/goldring")->wear();
		carry_object("/d/city/obj/necklace")->wear();
		carry_object("/d/quanzhou/obj/xiuhua")->wield();
		carry_object("/d/riyuejiao/obj/xiuhuajia");
		break;
	case "dagger":
                carry_object("/clone/weapon/bishou")->wield();
		break;
        case "xiao":
                carry_object("/d/taohuadao/obj/shizi");
                carry_object("/d/taohuadao/obj/shizi");
                carry_object("/d/taohuadao/obj/shizi");
                carry_object("/d/taohuadao/obj/shizi");
                carry_object("/d/taohuadao/obj/shizi");
                carry_object("/d/taohuadao/obj/shizi");
                carry_object("/d/taohuadao/obj/shizi");
                carry_object("/d/taohuadao/obj/shizi");
                carry_object("/d/taohuadao/obj/shizi");
                carry_object("/d/taohuadao/obj/shizi");
                carry_object("/d/taohuadao/npc/obj/yuxiao")->wield();
         break;
         case "ling":
                carry_object("/d/mingjiao/npc/obj/jinhua");
                carry_object("/d/mingjiao/npc/obj/jinhua");
                carry_object("/d/mingjiao/npc/obj/jinhua");
                carry_object("/d/mingjiao/npc/obj/jinhua");
                carry_object("/d/mingjiao/npc/obj/jinhua");
                carry_object("/d/mingjiao/npc/obj/jinhua");
                carry_object("/d/mingjiao/npc/obj/jinhua");
                carry_object("/d/mingjiao/npc/obj/jinhua");
                carry_object("/d/mingjiao/npc/obj/jinhua");
                carry_object("/d/mingjiao/npc/obj/jinhua");
                carry_object("/d/mingjiao/obj/shenghuo-ling")->wield();
         break;
	case "hammer":
		carry_object("/d/dalunsi/npc/obj/iron-falun")->wield();
		break;
	default:
	}
	return;
}