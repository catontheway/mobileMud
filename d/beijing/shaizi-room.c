...
//allow  坐庄，下注等。
//2009-2-3日，jason指导，skywolf修改.修改显示预测具结果判定规则
...
string  zhuang_id  =  "no  zhuang";  //庄家的id

int  total_bet  =  0;//桌子上的总钱数，以银子为单位

int  max_bet  ;  //每局的最大预测注，庄家决定
...
void  create  ()
{
        set  ("short",  HIC"骰子房"NOR);
        set  ("long",
CYN"屋中间摆着一张看起来相当结实的大长条桌(table)，四周围了
...
　　大家将预测注取回，庄家非正常丢庄。非庄家用此指令则表示退出此
　　局，预测注可取回。
                \n"  ]));
        //牌子内容
        ...
        set("exits",
        ([  
                "east"  :  __DIR__"dating",   //出口
        ]));

        set("objects",
        ([  
                __DIR__"npc/gongsun"  :  1,   //这个npc是哪里的？
        ]));

        set("no_fight",  1);
        set("no_magic",  1);
        set("no_clean_up",  1);
        set("no_caishen",  1);        //这个caishen代表什么？
...
int  do_bet(string  arg)
{
        ...
        if(money=="coin")  return  notify_fail("铜板？还是到大街上去预测吧。\n");
        ...
        {//I  am  not  zhuang-jia.
                ...
                if(  (this_bet  +  (int)me->query_temp("shaizi/bet_amount"))  >  max_bet  )
                        return  notify_fail("庄家太穷了，预测不起。换庄家吧。\n");
...
int  do_cancel()
{
        ...
        if(  me->query_temp("shaizi_cancel_count")  >  5  )
                return  notify_fail("你最近反悔的次数太多了，预测信已经太差。\n");

        if(  me->query("id")  ==  zhuang_id  )
        {//I  am  zhuang-jia.
                message_vision("$N起身说道：这一把我预测不下去了，你们把钱拿回去另找人坐庄吧！\n",  me);
                zhuang_id  =  "no  zhuang";
                me->delete_temp("shaizi");
                ...
                for(i=0;  i<sizeof(inv);  i++)
                {
                        if(  inv[i]->query_temp("shaizi/bet_amount")  >  0  &&  inv[i]->query("id")  !=  zhuang_id  )
                        {
                                ...
                                if(total_bet  >=  0)
                                {
                                        ...
                                        message_vision("$N起身把放在桌子上的预测注拿了回来。\n",  inv[i]);
                                }
                                else
                                {
...
int  do_retire()
{
        ...
        {//not  the  time  to  让庄.
                return  notify_fail("想逃？好歹得预测完这一把。\n");
        }
}

int  do_start(string  arg)
{
        int  max_limit  =  5000;  //1000  gold.
        ...
        if(game_status  ==  1  ||  game_status  ==  0  )
        {
                if(!  arg  ||  sscanf  (arg,"%d",  current_limit)  !=  1)
                        return  notify_fail("请使用：start  <预测注上限，单位：黄金>\n");

                if(current_limit  <  min_limit)
                        return  notify_fail("你也太穷了吧？还是别坐庄了。预测注上限别少于"+chinese_number(min_limit)+"两黄金。\n");
                ...
                message_vision("$N顿了一下：预测注别超过"+chinese_number(current_limit)+"两黄金。\n",  me);
...
void  game_process()
{
        ...
        for(i=0;  i<sizeof(inv);  i++)
        {
                if(  (int)inv[i]->query_temp("shaizi/bet_amount")  >  0  )
                {
						...
						add("show_shaizi_times",1);    //skywolf添加，得到应掷骰子数
                }
        }
      	remove_call_out("game_result");
		call_out("game_result",  1);	           
}
...
"└───┘\n",
        ...
        add("show_shaizi_times",-1);    //skywolf添加，掷骰子一次-1
...
{	
       ...
       if(query("show_shaizi_times") > 0)
	   {
			  call_out("game_result",  1);
			  return;
	   }         
	//skywolf添加，用于判断是否所有人都掷完骰子
