...
void die()
{
    ...
    if (inv)
    {
    	...
    	{
    	    if (userp(inv[i]) && IS_SHIWEI(inv[i]) &&
    //		PIN(inv[i]) == (8 - query("shiwei_level")) &&
    		inv[i]->is_fighting() &&
    		get_damage_origin_object() == inv[i] &&
    		query_temp("outer_gate_killer") &&
    		inv[i]->query_temp("chaoting/can_allow") )
    		{
        		message_vision("\n$N踢了一脚$n的尸体，得意地笑了笑。\n", inv[i], me);
        		player = inv[i];
        		if (env->query("is_outer_gate"))
        		{
        		    kill_num = player->query_temp("chaoting/kill_num");
        		    player->set_temp("chaoting/kill_num", kill_num+1);
                    if(player->query_temp("chaoting/kill_num") < 100)
                    player->add_temp("chaoting/gateexp", (kill_num + 1) * 3);
        		}
        		found_shiwei ++;
                break;
    	    }
