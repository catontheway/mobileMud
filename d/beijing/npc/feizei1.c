...
void die()
{
	...
	if (ob->query("catcher") != me->query("id") && ob->query("catcher") != "auto create")
	{
        ...
        {
        message_vision(WHT+ZJURL("cmds:tsdx "+me->query("id"))+HIG+"$N"NOR"深吸口气，冷笑道，好身手！说完强忍伤痛纵身向「" + dizhis["short"] + "」处逃走。\n"NOR,ob,me);
        ...
        {
        message_vision(WHT+ZJURL("cmds:tsdx "+me->query("id"))+HIG+"$N"NOR"冷笑一声，说道，好俊的功夫！说完纵身向「" + dizhis["short"] + "」处逃走。\n"NOR,ob,me);
