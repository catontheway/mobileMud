     ...
     (["short":                "白驼山蛇谷",
      ...
      (["short":                "日月朱雀堂",
       ...
       ([      "short":                "全真凝雪阁",
                ...
                "place":                "/d/silk/jiayuguan",
        ]),([      "short":                "月儿泉",
                "place":                "/d/silk/yueerquan",
        ]),([      "short":                "巴依家客厅",
...
void create()
{
    ...
    set("nickname", NAME_D->create_color_nick());
...
"官府的捕头吧？！大家都是武林中人，今天能否高抬贵手？我必有重谢！你好好斟酌一下，是否愿意？\n",ob,me);
			write (HIY"你可以选择"+ZJURL("cmds:yes")+HIG"同意"NOR"(yes)或者"+ZJURL("cmds:no")+HIG"拒绝(no)。\n"NOR);
	...
	{
 message_vision(HIR"$n气急败坏的对$N嚷道：“行凶不过头点地，你一再苦苦相逼，大爷今天就和你拼个鱼死网破。”\n"NOR,ob,me);
		me->set_leader(ob);
		remove_call_out("kill_ob");
		call_out("kill_ob", 1, ob);
		return ;
...
void die()
{
	...
	CHANNEL_D->do_channel(this_object(), "sys",sprintf("%s抓获飞贼。", me->query("id")));
    if(!random(30))
    {
        command("chat* hero");
        CHANNEL_D->do_channel(this_object(), "chat",sprintf("%s，来世老子不会放过你！", me->name()));
    }
        ::die();
}
