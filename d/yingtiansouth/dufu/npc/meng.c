...
string cipher_place(object me,string place)
{
	...
	int line_no=0,col_no=0,x1,y1,x2,y2,x3,y3,x4,y4,x5,y5,start_bit,len=sizeof(word),line=6+random(6),col=6+random(6),total;
	...
	for (int i=0;i<total;i++)
	{
		...
		msg+=place[1..1];
		else if (sizeof(place)>2&&line_no==x3&&col_no==y3)
		msg+=place[2..2];
		else if (sizeof(place)>3&&line_no==x4&&col_no==y4)
		msg+=place[3..3];
		else if (sizeof(place)>4&&line_no==x5&&col_no==y5)
		msg+=place[4..4];
		else
		{
			start_bit=random(len-2);
			if (start_bit%2==1) start_bit+=1;
			printf("word %s  bit：%i \n",word[start_bit..start_bit],start_bit);
			msg+=word[start_bit..start_bit];
...
int give_job(object me,string wait_place)
{
	...
	if (rdm<75)
	{
		...
		else
		{
			...
			tell_object(me,"对照(duizhao)此页"+me->query_temp("mzj/pic_answer")+"，你就知道你要刺杀的人在哪了。\n");
...
int ask_job()
{
	...
	else
	{
		...
		tell_object(me,"快去"+ZJURL("cmds:walk "+where->query("short"))+where->query("short")+"。\n"NOR);
