...
void create()
{
        set_name("陆世清",({ "lu shiqing", "lu", "shiqing" }) );
        set("gender", "男性" );
        set("title",HIY"金陵镖局 "+NOR+"总镖头"+NOR);
        set("age", 42);
...
int do_getesc(string serial)
{
  ...
  return get_dart_fb("jiankangfu", this_player(), escort_info, sno);
}
...
int do_listesc()
{
	...
	msg=ob->show_list("jiankangfu", escort_info);
...
//		return "本功能暂时只接受wizard级别的玩家。\n";
	...
	return get_dart("jiankangfu", this_player(), escort_info);
}
