...
//Add some more new features by Zine 2013/8/11
...
LONG );
    ...
    set("item_desc", ([
		...
		ZJOBACTS2+ZJMENUF(2,2,12,40)+"挂上:gua"+ZJSEP+"下(xia)帆:xia"+ZJSEP+"升起帆:sheng"+ZJSEP+"收逆风帆:shouqi\n", 
        "jiang" : "两支船桨，无风的时候也可以划动(hua)。\n"
		ZJOBACTS2+ZJMENUF(2,2,12,40)+"划动:hua"+ZJSEP+"停船:ting"+ZJSEP+"下锚:xiamao"+ZJSEP+"打捞:dalao\n",
...
int selfcheck()
{
    ...
    for (i=0;i<sizeof(all);i++)
    {
        ...
        if ( userp(all[i]) && all[i]->query_temp("pyh/boat_flag")!=this_object() )
        {
            rm=load_object("/d/jiangzhou/hubian");
            all[i]->move(rm);
            tell_room(all[i],"渔夫发现你混上了小船，把你赶回了岸边！\n");
        }
    }
...
int do_dalao()
{
    ...
    if (random(10)>8 || random(me->query("kar"))>29) //加入福源对打捞的影响,Zine
    {
		...
		{
			message_vision("$N等一行人终于找到了鄱阳湖中沉船的宝物，决定一分为"+chinese_number(sizeof(team))+"，各自保管！\n",me);
    ...
    else
    {
        ...
        tell_object(me,"你决定再接再厉，"+ZJURL("cmds:dalao")+HIG"越挫越勇(dalao)。\n");
        me->add_busy(4);
        me->add("pyh/dalao",1);
...
int gene_sz(object me,string type)
{
	...
	{
		sz->set("yanei",1);
		sz->set("title",HIM+"鄱阳水贼少当家"+NOR);
		sz->set_name(sz->query("name")[0..1]+"天一",({sz->parse_command_id_list()[1]+" tianyi",sz->parse_command_id_list()[1],"tianyi"}));
	}
	sz->set("killer",me);
    sz->do_copy(me);
	sz->move(ob);
...
int gene(object me)
{
	...
	if ((!random(60)||(wizardp(me)&&me->query("want_yanei"))||(!random(15)&&time()<1376542498))&&(me->query("combat_exp")>=100000000))
	{
		gene_sz(me,"yanei");
	}
    return 1;
...
int distance(object ob)
{
	...
	else
	{
		...
		&&present(me,ob))
		{
			...
			tell_object(me,HIR+BLINK+"钓鱼人传音"+word+"\n"NOR);
...
int actions(object me,int act)
{
	...
	if (ob->query(action["mark"])==action["value"])
		return notify_fail(action["fail_msg"]+NOR);
	else
	{
...
int huacheck(object me)
{
    ...
    tell_object(me,"如果你觉得到了，就可以"+ZJURL("cmds:xiamao")+HIG"下锚"NOR"(xiamao)了，如果还没到那就继续划(hua)吧。\n");
    me->set_temp("pyh/couldstop",1);
    if (!random(5)&&(int)this_object()->query_temp("pyh/stepcount")<(int)me->query_temp("pyh/step")&&ob->query("pyh/distance")>18)
	{
		if (team=me->query_temp("pyh/team"))
		{
			for (int i=0;i<sizeof(team);i++)
...
int do_hua(string arg)
{
	...
	if (arg=="back")
    {
        ...
        {
			if (ob->query("pyh/distance")<3)
			{
				tell_object(me,"刚开始就回去？那只好就这样了。\n");
				return 1;
			}
			bonus(me);
			me->delete("pyh/start");
	...
	if (!me->query_temp("pyh/report_distance"))
	{
		if (team=me->query_temp("pyh/team"))
		{
			CHANNEL_D->do_channel(me, "bd", me->query("name")+HIG+"一行人乘小船顺流而下，已经成功漂出了"+chinese_number(ob->query("pyh/distance"))+"里开外了。" NOR, -1);
			for (i=0;i<sizeof(team);i++)
			if (team[i]&&userp(team[i])&&living(team[i])&&present(team[i],this_object()))
		...
		{
			CHANNEL_D->do_channel(me, "bd", me->query("name")+HIG+"乘小船顺流而下，已经成功漂出了"+chinese_number(ob->query("pyh/distance"))+"里开外了。" NOR, -1);
...
int bonus(object me)
{
	...
	exp=exp*(event_bonus+1);
	pot=(ob->query("pyh/team_member")?exp/3+random(exp/2):exp/4+random(exp/2));
	...
	if (ob->query("pyh/member_number"))
	{
		...
		else if (old_exp<exp)
		{
			...
			tell_object(me,HIC+"因为这次寻宝任务由组队完成，你获得了比单人完成高出"+sprintf("%.2f",inc_percent)+HIC+"%的奖励。\n"+NOR);
		}
    }
	if (adv_adjust)
	...
	CHANNEL_D->do_channel(ren, "rw", me->query("name")+"在鄱阳湖寻宝任务中获得经验"+ chinese_number(exp) +"点、"+
                      "潜能"+ chinese_number(pot) +"点、"+
                      "声望"+ chinese_number(rep) +"点。   "+HIR+flag_str);
    ...
    if ((int)me->query("newjobs/completed/pyh")%10==0&&!me->query("pyh/simpletask"))
    {
