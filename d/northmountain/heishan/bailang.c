...
LONG
   ...
   //"south" : __DIR__"zoudao2.c",
		...
		set("drug_level",70);//这里是这个副本出现药材的级别在此以下，副本内可以升级的采药术在35+10以下。
		set("level_bonus",65);
        set("migong_name","白狼山"),
        set("no_clean_up", 1);
        set("no_reset", 1);
...
string welcome_msg(object me)
{
	...
	return HIW"【白狼山】山中有很多药物，最高为70级药物，最高提升玩家采药术到80级。\n"NOR;
}
int start()
{
    object ob=this_object();
    object exit1=load_object(__DIR__"nandigou2");
    //object exit2=load_object(__DIR__"changgengshang");
    //object exit3=load_object(__DIR__"shanluw1");
    object exit4=load_object(__DIR__"bailangn1");
    set("migong_name","白狼山"),
    gene("山地",2,ob,exit1,0,0,exit4,0); //样本，随机迷宫8个参数，第一个为迷宫地貌类型，在include/random_map.h可查，第二个为迷宫复杂度，3为迷宫原点，4-7为出口，无出口则需补0,第八个参数为迷宫出口对于真实地图是否可见，不可见为1，可见为0
    return 1;
}
int over()
{
