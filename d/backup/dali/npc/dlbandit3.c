...
void create()
{
	switch (random(5))
	{
		case 0 :
			...
			set("title", "西域少林高僧");
			set("long", "这家伙是西域少林派高人，被西夏官府收买后专门到南方武林行凶作恶。\n他内外兼修，功力深厚。\n");
			break;
		case 1 :
...
void killvictim(object me, object who)
{
	if (present(who, environment(me)))
	{
		if (who->query("race")=="人类" && who->query("age")<15)
		{
			...
			message_vision("$N恶狠狠地对$n叫道：大爷今天不高兴，先拿你的小命儿来祭祭刀！  $N看来想就地正法$n！\n", me, who);
			kill_ob(who);
			return;
		}
		if (who->query("gender")=="女性" && who->query("age")<30  && random(3)==2)
		{
			...
			message_vision("$N上前调戏$n，$n却奋力挣扎不从。$N勃然大怒，看来想就地正法$n！\n", me, who);
		...
		else if (random(3)==2)
		{
			...
			message_vision("$n迟疑不给，$N便阴笑着吼道：“"+RANK_D->query_rude(who)+"找死，今天老子要杀个鸡犬不留！”  看来$N想就地正法$n！\n", me, who);
			kill_ob(who);
			return;
		}
		else
