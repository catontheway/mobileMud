...
int do_open(string arg)
{
        ...
        if(me->query("chaoting/newjob/jianzhan/in_dart"))
          return notify_fail("你想私纵钦犯吗？\n");
        if (ob->query("be_robbed"))
            return notify_fail("什么？\n");
        if(player = present(mid,environment()))
          if (living(player))
          return notify_fail("监斩的大宋官员还在旁边，先把他做掉吧。\n");

        message_vision("$N将$n推走了。\n", me, this_object());
        CHANNEL_D->do_channel(this_object(), "rumor", mname+"护送的囚车被"+me->query("name")+"劫去了！", -1);
...
int do_gan(string arg)
{
	mixed *indexs;
		...
		if (environment()->query("dirs_to_song")&&mapp(environment()->query("dirs_to_song")))
		{
			...
			{
				if (stringp(indexs[i][1])&&indexs[i][1]+"面"==dir&&rm=load_object("/d/song/"+indexs[i][0]))
				{
					tell_room(environment(),HIW+me->name()+HIW+"赶着"+this_object()->query("name")+HIW+"往"+rm->query("short")+HIW+"驶去。\n"+NOR);
					tell_room(rm,HIG+me->name()+HIG+"赶着"+this_object()->query("name")+HIG+"向这里驶了过来。\n"+NOR);
					me->move(rm);
					this_object()->move(rm);
					remove_call_out("check1");
					call_out("check1",1,ob,me);
					if (base_name(environment())=="/d/song/xing_prison" )
					return 1;
        ...
        if(!me->query("chaoting/newjob/jianzhan/can_go"))
        ...
        if(me->follow_path(dir))
	{
		...
		ob->move(obj);
        	...
        	me->set("chaoting/newjob/jianzhan/can_go",0);
	}
	else
	{
        	return notify_fail("这个方向过不去。\n");
...
int check1(object ob, object me)
{
	...
	int i;
        if ( base_name(environment()) == "/d/song/xing_prison" )
           {
              message_vision(HIY"$N终于把囚车押送到了刑部大牢。\n\n"NOR,me);
	...
	for(i=0; i<sizeof(obj); i++)
	{
		...
		destruct(obj[i]);
              ...
              me->set("chaoting/newjob/jianzhan/over_dart",1);
	      remove_call_out("destroy1");
              call_out("destroy1",1,ob);
              return 1;
            }
        me->set("chaoting/newjob/jianzhan/can_go",1);
...
void to_rob(object me)
{
  ...
  me->start_busy(2);
	...
	for (i = 0; i < j; i++)
	{
	robber = new("/d/hangzhou/npc/robber");
