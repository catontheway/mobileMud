...
string do_task()
{
	...
	if(!ob->query_temp("chaoting/newjob/xuncheng"))
		return ("你没巡逻跑来领什么功？");
	if (interactive(ob)
		&& time()-ob->query_temp("chaoting/newjob/xuncheng_start")<10)
...
string give_reward(object ob,string type)
{
	...
	if (type=="jianzhan")
	{
		add_score(ob);
		if (ob->query("combat_exp") < 1000000)
		exp = (random(70) + 500) * ob->query("chaoting/newjob/jianzhan/dart_value");
		else
		exp = ( random(80) + 500) * ob->query("chaoting/newjob/jianzhan/dart_value");
		pot = exp / 2;
		ob->add("combat_exp",exp);
		ob->add("exp/chaoting",exp);
		ob->add("potential",pot);
...
string ask_feizei()
{
	...
	if (feizei->move(where))
	{
		...
		return "飞贼" +ZJURL("cmds:tsdx "+feizei->query("id"))+ feizei->short() + NOR+"在「" +TASK_D->place_belong(where)+"的"+where->query("short") + "」出现，请速去将他捉拿归案！";
	}
