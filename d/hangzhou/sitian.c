...
string look_machine()
{
	...
	ZJOBACTS2+ZJMENUF(2,2,12,40)+"计算:jisuan"+ZJSEP+"兑换:duihuan"+ZJSEP+"降等:jiangdeng"+ZJSEP+"甄别:zhenbie\n";
}
void init()
{
...
int do_duihuan(string arg)
{
    ...
    if (!arg)
	return notify_fail(INPUTTXT("请输入转化成修行经验的实战经验：","duihuan 浑天仪 $txt#")+"\n");
...
int do_jiangdeng(string arg)
{
    ...
    if (me->is_busy() || me->is_fighting())
		return notify_fail("你正忙着呢！\n");
    if (!arg)
		return notify_fail(INPUTTXT("用来确定降等使用消耗的修行经验","jiangdeng $txt#")+"\n");
