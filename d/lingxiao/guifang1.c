...
string window_desc()
{
        if (!this_player()->query_temp("jinwudao/ask_axiu3"))
           return "木窗紧紧的关着，窗外临着足有万丈深的雪谷。\n";
        else
           this_player()->set_temp("jinwudao/ask_axiu4",1);
           return "木窗紧紧的关着，窗外临着足有万丈深的雪谷。\n窗台上似乎有翻动过的痕迹。\n";
}
void init()
{
        ...
        if(interactive(me) && !me->query_temp("jinwudao/ask_axiu1"))
        {
        yahuan=present("ya huan",environment(me));
        if (!objectp(yahuan)) return;
        tell_object(me,"你一踏进房门，看到一个丫鬟晕倒在血泊之中。\n");
        yahuan->unconcious();
        }
}
