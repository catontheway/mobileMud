...
// Modified by  : whuan@pkuxkx, 2008-5-3,to add jinwu-dao secret
// Modified by  : whuan@pkuxkx, 2008-7-10, to fix the baihong-jian bug
#include <ansi.h>
#include <room.h>
...
string look_pingfeng()
{
        ...
        ZJOBACTS2+ZJMENUF(2,2,12,40)+"屏风:kan 屏风\n";
}
...
int do_pick(string arg)
{
        ...
        if(me->is_busy())
                return notify_fail("先忙完别的事再说吧。\n");
        if (query("bhj_count")<0)
        	      return notify_fail("好像已经被人取走了。\n");        
...
int valid_leave(object me, string dir)
{
        ...
        if (me->query("family/master_id")!="bai zizai" 
           || me->query_skill("xueshan-neigong",1)<200) //武功太低不能触发剧情
           return ::valid_leave(me, dir);  
        else
           if (!random(5) || me->query_temp("jinwudao/ask_axiu3"))
           {
           me->move(__DIR__"guifang1");//设置成另外一个闺房
           if (!me->query_temp("jinwudao/ask_axiu3"))
           return notify_fail("你一踏进房门，心中立刻觉得非常不安，似乎发生了什么事情。\n");
           else
           return notify_fail("你踏进房门，开始寻找蛛丝马迹。\n");
           }
        return ::valid_leave(me, dir);
}
