...
int ask_axiu()
{
        ...
        if (!me->query_temp("jinwudao/ask_axiu1")) 
        {
        command("say 阿绣是我的独生女儿，乖巧可爱，现在在自己的闺房里。\n");
        command("say 你可不要欺负她哟。");
        return 1;
        }
        message_vision("$N走上前去，对着$n施了一礼，道：“白师兄，大事不好，阿绣失踪了。”\n",me,ob);
        command("ah");
        command("say "+me->query("rank_info/respect")+"帮忙去家父那里看看，并询问一下阿绣是否在他哪里。\n");
        command("bow");
        me->set_temp("jinwudao/ask_axiu2", 1); 
        return 1;
}
...
void attempt_apprentice(object ob)
{
        ...
        CHANNEL_D->do_channel(me, "jh", ob->query("name")+"从今日起，投入我雪山派白万剑门下，还请各方江湖朋友多加照顾。");
