...
void init()
{
        ...
        ob = this_player();
        if( interactive(ob) ) {
                remove_call_out("check_present");
                call_out("check_present", 1, ob);
        }
}

void check_present(object ob)
{
        if(!ob || !living(ob) || !present(ob, environment()))
        {
                message_vision("$N轻蔑的冷笑一声，转身离开了。\n", this_object());
                ob->set("marks/xieyanke", -1);
		log_file("static/xieyanke",sprintf(" %s(%s)于%s突然消失并错失机缘。\n", ob->query("name"),ob->query("id"),ctime(time())));
...
void teach_skill(object teacher, object learner)
{
        ...
        if(learner->query("marks/xieyanke") != 5)
                return 0;
		if (learner->query_temp("marks/xyk_stay"))
		{
        msg = HIG"$N"HIG"低头考虑了一会，抬起头一声不发的给$n演示了一套掌法。低声解释了其中的运气之道。\n"NOR;
        message_vision(msg, teacher, learner);
        learner->set_skill("bizhen-qingzhang", 1);
		learner->delete_temp("marks/xyk_stay");
        msg = HIY"$N哈哈哈大笑三声，倏忽远逝。\n"NOR;
        message_vision(msg, teacher, learner);
        ...
        CHANNEL_D->do_channel(this_object(), "jh", HIM"听说"+learner->query("name")+HIM"凭"HIB"玄铁令"HIM"向谢烟客习得「碧针清掌」！", -1);
		}
		else
		{
        msg = HIG"$N"HIG"怒道：‘老夫平生最恨贪奸耍滑之人，还想学我功夫，做梦！’\n说完转身离去！\n"NOR;
        message_vision(msg, teacher, learner);
        log_file("static/xieyanke",sprintf(" %s(%s)于%s试图bug习得碧针清掌。\n", learner->query("name"),learner->query("id"),ctime(time()))); 
		}
