...
int ask_tasks()
{
    ...
    if (me->query_temp("mingtasks/dudufu/task")!=1)
    {
    ...
    if (me->query_temp("mingtasks/dudufu/lcaolian_degree")>50)
    {
...
void init()
{
    ...
    if ((base_name(environment(ob))=="/d/yingtianfu/neicheng/junying1"||base_name(environment(ob))=="/d/yingtianfu/neicheng/junying2")
        &&me->query_temp("mingtasks/dudufu/lcaolian_degree")>50)
    {
        message_vision("$N向$n抱拳道：青山不改，绿水长流，我们后会有期！\n",ob,me);
        message_vision("$N转身走进了军营。\n",ob);
        me->set_temp("mingtasks/dudufu/task1finish",1);
        destruct(ob);
        return;
    }
}
...
int gen(object me)
{
    ...
    tell_object(me,"新军即成，你把他们带到城西的军营去吧。\n");
    return 1;
}
...
int do_caolian()
{
    ...
    me->add_busy(3);
    me->set_temp("mingtasks/dudufu/lcaolian_degree",(int)me->query_temp("mingtasks/dudufu/lcaolian_degree")+n);
