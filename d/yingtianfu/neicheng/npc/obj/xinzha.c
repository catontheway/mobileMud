...
void create()
{
        ...
        set_weight(3000);
        set("unit", "封");
        set("long","这是一封信扎，你可以"+ZJURL("cmds:open xin")+HIG"打开(open)它。\n");
        set("material", "paper");
...
int do_open(string arg)
{
    ...
    int n;
    string* taskwhere=({"镇江","苏州","杭州","扬州","江州","岳阳","南昌","泉州","闽南"});
    string* taskid=({"库房看守","冯正东","裴老板","唐楠","温峤","李保长","楚大业","戚家军千总","林平之"});
    if (!arg)
    {
    ...
    else
    {
        ...
        me->set_temp("mingtasks/hubu/task_where",taskwhere[n]);
        destruct(ob);
        return 1;
    }
}