...
int examing()
{
    ...
    if (me->query_temp("mingtasks/libu2/examround")>12)
    {
    ...
    me->add_temp("mingtasks/libu2/examround",1);
    tell_object(me,"似乎"+room->query("short")+"有些动静，如果有人作弊，一定要"+HIR+"抓"+NOR+"住！\n");
    remove_call_out("examing");
    call_out("examing",10);
    return 1;
...
int do_kaikao()
{
    ...
    {
        tell_object(me,"你不是大明官员，有何权利宣布开考？\n");
        return 1;
    }
    if (!me->query_temp("mingtasks/libu2/exam"))
    {
    ...
    if (me->query_temp("mingtasks/libu2/exambegin"))
    {
    ...
    else
    {
        ...
        me->set_temp("mingtasks/libu2/exambegin",1);
        ob->set("using",1);
