...
void create()
{
	set_name(WHT"九阴真经——摧心掌篇"NOR, ({ "jiuyin zhenjing","jing","book"}));
	set_weight(500);
    set("value",1);
	if (clonep())
		...
		{
			set("long", "这是一本九阴真经的摧心掌篇，你可以研究(yanjiu)一下。\n");
			set("unit", "本");
		}
}
...
int do_yanjiu()
{
    ...
    {
        tell_object(me,"你已经掌握了摧心掌的精义。\n");
        if (me->query_skill("cuixin-zhang",1)<10)
        {
            me->set_skill("cuixin-zhang",10);
        }
        return 1;
    }
        if (me->query_skill("cuixin-zhang",1)<10)
