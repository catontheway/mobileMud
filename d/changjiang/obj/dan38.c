...
void create()
{
	set_name(HIG"奇门八卦阵-遁形"NOR, ({ "dunxing pian","pian","book"}));
	set_weight(500);
    set("value",1);
	if (clonep())
		...
		{
			set("long", "这是奇门八卦阵布阵的用法之一：遁形的使用方法。\n");
			set("unit", "本");
		}
}
...
int do_yanjiu()
{
    ...
    if (me->query("safari_skill/thd_dunxing"))
    {
        tell_object(me,"你不是已经学会了奇门八卦阵-遁形的使用方法了么？\n");
        return 1;
    }
    ...
    message_vision("$N开始认真研读$n，领悟了奇门八卦阵遁形的使用方法！\n"NOR, me,this_object());
    me->set("safari_skill/thd_dunxing",1);
    me->add("thd_qmbgz",1);
