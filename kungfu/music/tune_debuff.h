//下面的两个函数是乐曲演奏对敌人Debuff的标准函数，一般情况下不要随便修改这两个函数
//弹奏效果主函数
...
//	printf("test: %O, %O, %d\n", weapon->query("music"), instrument, times);
  ...
  if ( !sizeof(listeners) )
  {
	  end_play(player, listeners, tune, master);
	  player->start_busy(0);
		tell_object(player, "你发现这个房间里面没有任何听众，你放弃了弹奏『"+tName+"』。\n");
    return;
  }
...
//重新给所有人计算演奏效果
	  ...
	  {
	  	reslis=search_listeners(player, listeners, tune, "ENEMY");
	  	play_effect(player, reslis, instrument, tune, pri_coef, aux_coef,1);
