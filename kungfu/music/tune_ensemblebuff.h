//下面的两个函数是乐曲合奏组队Buffer的标准函数，一般情况下不要随便修改这两个函数
//弹奏效果主函数
  ...
  if ( master )
  {
    ...
    if ( sizeof(listeners)==0 )
  	...
  	if ( ensemble_coef>0 )
  	{
  	  ...
  	  call_out("in_playing", 1, player, listeners, environment(player), instrument, tune, (coef+ensemble_coef)*2/3, aux_coef, master, 0);
  	}
  	else if ( ensemble_coef<=0 )
  	{
  		message_vision("$N拿起$n开始准备演奏『"+tName+"』,等待另一个乐器一起合奏『"+tName+"』。\n", player, player->query_temp("weapon") );
...
//	printf("test: %O, %O, %d\n", weapon->query("music"), instrument, times);
		   ...
		   weapon->query("music")!=instrument )
  {
	...
	if ( master==1 )
	{
		...
		if ( ensemble_coef<=0 )
		{
			tell_object(player, "你的合奏者停止了演奏『"+tName+"』，你的合奏曲演奏不下去了。");
			end_play(player, listeners, tune, master);
			player->start_busy(0);
			return;
		}
...
//重新给所有人计算演奏效果
      play_effect(player, reslis, instrument, tune, (primary_coef+ensemble_coef)*2/3, n_aux_coef,2);
  	  ...
  	  player->add("jing",SectSpiRed);
  	  player->add("neili",SectForceRed);
		  message_vision(query_tune(tune, times/sectT,0), player);
		  call_out("in_playing", 1, player, reslis, env, instrument, tune, (primary_coef+ensemble_coef)*2/3, n_aux_coef, master, times+1);
	  }
	  else
	  {
	  	play_effect(player, listeners, instrument, tune, (primary_coef+ensemble_coef)*2/3, aux_coef,1);
	...
	else if(master>=2 )
	{
		...
		if ( master==3 &&
			   ensemble_coef<=0 )
		{
			tell_object(player, "合奏曲『"+tName+"』的演奏结束了。");
			end_play(player, listeners, tune, master);
			player->start_busy(0);
			return;
		}
		...
		if ( ensemble_coef>0 )
		{
  	    ...
  	    player->add("jing",SectSpiRed);
  	    player->add("neili",SectForceRed);
		  	message_vision(query_tune(tune, times/sectT,1), player);
		  }
		  play_effect(player, ({player}), instrument, tune, (primary_coef+ensemble_coef)*2/3, aux_coef,1);
		  call_out("in_playing", 1, player, reslis, env, instrument, tune, (primary_coef+ensemble_coef)*2/3, n_aux_coef, 3, times+1);
		}
		else
		{
			message_vision("$N拿起等待另一个乐器一起合奏『"+tName+"』，你可以放下乐器停止等待。\n", player );
			call_out("in_playing", 1, player, reslis, env, instrument, tune, (primary_coef+ensemble_coef)*2/3, aux_coef, master, times);
		}
	  player->set_temp("play_music/end_time", time());
	}
	else
