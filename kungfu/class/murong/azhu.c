...
void create()
{
    ...
    set_skill("disguise",300);
    set_skill("anti-disguise",300);
...
int prevent_learn(object me, string skill)
{
	...
	if( skill=="anti-disguise" ) {
	  ...
	  {
	  	tell_object(me, "火眼金睛术只有忠心于慕容的才能传授。\n");
	  	return 1;
	  }
	  
	  tell_object(me, "阿朱想起你以往对师门做出的贡献，决定传授你一点火眼金睛术的心得。\n");
	  if ( random(2) )
	  ...
	  if ( me->query("score")<=3000 )
	  {
	  	...
	  	me->query("chushi/done") )
	  {
	  	tell_object(me, "易容术只有忠心于慕容的才能传授。\n");
	  	return 1;
	  }
	  else if ( me->query_skill("anti-disguise", 1)<300 )
	  {
	  	tell_object(me, "想学易容术必须先学习足够的火眼金睛术才行。\n");
	  	return 1;
	  }
	  
	  tell_object(me, "阿朱想起你以往对师门做出的贡献，决定传授你一点易容术的心得。\n");
	  if ( random(50) )
