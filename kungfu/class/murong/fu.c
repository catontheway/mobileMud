...
int ask_job()
{
    ...
    for(i=0; i<sizeof(skl); i++)
        if (intp(skl[sname[i]]) && skl[sname[i]] >= skill && sname[i] != "literate" && sname[i] != "taoism")
            skill = skl[sname[i]];
    f = 50 + random(25);
    skill = skill * f / 100;
    ...
    {
        command("say " + RANK_D->query_respect(me) + "找大燕传国玉玺是很难的，没有10年功力叫我怎么放心让你去?");
        return 1;
    }
...
//if (busytime>300) busytime = 120;
    ...
    if (me->query("murong/yuxi") == 1) {
        ...
        if ( me->query_combatgrade()/6 > random(10))
        {
//分配随机地点
...
int ask_fail()
{
    ...
    else if(me->query_combatgrade() < 60)
    {
        exp = exp / 2;
        pot = exp / 5;
    }
    ...
    if (me->query("murong/yuxi") == 1) {
      ...
      if (me->query("potential") < me->query("learned_points"))
    	...
    	me->delete("murong/yuxi");
		  ...
		  if(objectp(me->query_temp("murong/pantu")))
		  {
			  ob = me->query_temp("murong/pantu");
			//销毁掉这个叛徒，降低系统负担
			  tell_room(environment(ob),ob->name()+"长笑一声：天大地大，四海为家。再会！\n");
			  ob->move(VOID_OB);
			  destruct(ob);
			  me->delete_temp("murong/pantu");
		  }
      return 1;
    }
    else
...
void destroying(object obj, object me)
{
    ...
    kar = me->query("kar");
    if ((int)me->query("kar")<30)
    {
      kar = 30;
    }
    kar = 30 * (10 + random(5)+2*sqrt(kar-30));
    exp = 2 * kar;
