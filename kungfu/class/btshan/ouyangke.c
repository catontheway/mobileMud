...
{
    if((string)ob->query("family/family_name")=="白驼山" && (string)ob->query("family/master_name") == "欧阳锋")
    {
        return 1;
    }
	return 0;
...
//   }
	...
	{
		command("say " + RANK_D->query_respect(ob) + "不敢当、不敢当。你我兄弟一场，如有闲暇可以切磋切磋。\n");
		tell_object(ob, HIC"欧阳克同意你直接向他学习或者切磋技能。\n"NOR);
		return;
	}
...
int ask_fail()
{
	...
	ppl->add("score", -sc);
	if(ppl->query("combat_exp") > 2000+sc*300)
		ppl->add("combat_exp", -sc * 300);
	ppl->add("potential", -sc * 30);
...
string ask_job()
{
	...
	if( ob->query("score") <= 1)
	{
		...
		return "你看看你，对本派有什么贡献？我哪里敢把怪蛇交给你！";
	}
		...
		if(stringp(beauty = ob->query("baituo/beauty")))
		{
			...
			if(!obj->is_character())
			{
				...
				return 0;
                        ...
                        if(obj->query("hooker"))
                        {
                                message_vision("$N大怒道：「这种烂婊子你留着自己用吧！」\n", this_object());
                                return 0;
                        }
...
string ask_score()
{
	object me = this_player();
	if(me->query("family/family_name") != "白驼山")
		return "你想尽忠，先拜入我白驼山门下才行。";
	if(me->query("baituo/ask_score"))
		return "你找死啊？我不是说过了么？";
	me->set("baituo/ask_score", 1);
	me->add("score", 2);
	if(me->query("betrayer") < 1)
		return "很好，你多做一些工作，自然不会亏待你。";
	return "好吧，暂且相信你一次看看。";
}
