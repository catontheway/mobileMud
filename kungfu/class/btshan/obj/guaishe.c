...
//西域怪蛇 guaishe
//sdong, 11/19/98
// Modified by iszt@pkuxkx, 2007-03-10, 怪蛇吸exp<100k玩家的血无效果
...
protected int is_suitable(object victim, object killer);
...
int auto_check()
{
	...
	if( !is_busy() && strsrch(base_name(here), "/d/") != -1   )
	{
	  ...
	  if( random(8)==1 )
			random_walk();
           else if (!objectp(find_player(ob)) ) return 0;
           else if (!present(find_player(ob),here)) return 0;
	  else if( random(5)==1 )
	  {
...
void set_owner(object owner)
{
	...
	set_skill("training", (int)owner->query_skill("training",1)+1);
...
protected int is_suitable(object victim, object killer)
{
		...
		if(  !clonep(victim) || userp(victim)
			...
			|| victim->query("shen") < 0
			|| victim->query("winner")
                         || strsrch(base_name(victim), "/adm/") != -1
                        || strsrch(base_name(victim), "/kungfu/class/") != -1
			|| strsrch(base_name(victim), "/clone/") != -1
			|| strsrch(base_name(victim), "/u/") != -1
                        || strsrch(base_name(victim), "/d/xinfang/") != -1
                         || strsrch(base_name(victim), "/d/wizard/") != -1
                         || strsrch(base_name(victim), "/d/death/") != -1
			|| strsrch(base_name(victim), "/d/xytaskd/") != -1
			|| victim->query("combat_exp") < killer->query("combat_exp") * 1/2
...
/*
	...
	{
              exp = (me->query("combat_exp")) *( 50+random(100) ) / 100  ;
		obj->set("combat_exp", exp);
	}
*/
       exp = obj->query("combat_exp");
	level = to_int(pow( exp*10,0.33333 ));

	if(level < 10) level = 10;

	if( obj->query("combat_exp") < 100000 )
	{
		level = to_int(level/3 + random(to_int(level/5)));
	}
	else if( obj->query("combat_exp") < 300000)
	{
		level = to_int(level/3 + random(to_int(level/4)));
	}
	else if( obj->query("combat_exp") < 500000)
	{
		level = to_int(level/3 + random(to_int(level/3)));
	}
	else if( obj->query("combat_exp") < 1000000)
	{
		level = to_int(level/3 + random(to_int(level/2)));
	}
	else
		level = to_int(level/2 + random(to_int(level/2)));

	skills = obj->query_skills();

	if (sizeof(skills) == 0) 
	{
		remove_call_out("killer-show");
		call_out("killer_show", 5, me);
		return;
	}
	names = keys(skills);
	for (i=0; i<sizeof(names); i++) {
	  if ( !mapp(skills[names[i]]) )
	    obj->set_skill(names[i], level);
	}

	call_out("do_leave",30+random(60),obj);
...
void killer_show(object me)
{
	...
	if (sizeof(all_inventory(here)) > 50) return; //icer added in case of too many objects created
...
int do_yao(string arg)
{
	...
	string target_dir, dir, dest, victim,staff;
	mapping exit;
	...
	if (!ob) return notify_fail("这里并无此人！\n");
	if (!snake) return notify_fail("你必须放下怪蛇，让它自如行动才能攻击"+ob->query("name")+"。\n");
	if (!living(snake))
	return notify_fail("你必须等怪蛇醒来，才能让它攻击"+ob->query("name")+"。\n");
	if (me->query_temp("weapon")&& (me->query_temp("weapon")->query("id")=="shezhang"||me->query_temp("weapon")->query("id")=="guai shezhang"))
		message_vision(HIR "$N用"+me->query_temp("weapon")->query("name")+HIR+"驱赶蛇朝$n咬去。\n" NOR, me, ob);
	else if (me->query_temp("secondary_weapon")&&(me->query_temp("secondary_weapon")->query("id")=="shezhang"||me->query_temp("secondary_weapon")->query("id")=="guai shezhang"))
		message_vision(HIR "$N用"+me->query_temp("secondary_weapon")->query("name")+HIR+"驱赶蛇朝$n咬去。\n" NOR, me, ob);
	else
...
//	if (!living(ob)) return notify_fail("这不是活物！\n");
	...
	if( ob->query("combat_exp") < me->query("combat_exp")*3/5 )
		return notify_fail("怪蛇只对武功高手感兴趣！\n");
	if (ob->query("id") == "gu jia" || ob->query("id") == "mu ren" || ob->query("id") == "tong ren")
		return notify_fail("怪蛇只对活人感兴趣！\n");



	if( random(20) != 0)
	{
		message_vision( HIR "只见$N"+HIR"闪电般的向$n扑击过去。\n" NOR, snake, ob );
		snake->kill_ob(ob);
...
int convert(string arg)
{
	...
	if (ob->query("master_id") != me->query("id")) return notify_fail("蛇杖不是你的。\n");
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
	...
	if(me->query("owner")&&(owner=present(me->query("owner"),here)) )
	{
		if( owner->query_skill("training") < 400 )
			owner->improve_skill("training",owner->query("int")*10);

		owner->add("combat_exp",20+random(30) );
		owner->add("potential",1+random(5) );
...
int auto_perform()
{
	...
	if( random( me->query("combat_exp") ) > random( target->query("combat_exp") ) || !living(target) )
	{
			...
			F_POISON->poison_attack(me,target,"bt_poison", 15 + me->query_skill("poison",1)/20 +
			...
			if(bonus > 30) bonus = 30;
                        if(userp(target) && target->query("combat_exp") < 100000) bonus = 0;
                        if(target->query("id") == "body guard") bonus = 0;
