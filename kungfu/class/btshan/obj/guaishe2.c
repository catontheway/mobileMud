...
//		if( owner->query_skill("training") < 400 )
                breed = owner->query("baituo/breed");
                breed = breed/100;
                if(breed == 0) breed = 1;
                if(victim->query("id") == "body guard")
                        breed = 1;
		owner->improve_skill("training",random(owner->query("int")) * 30);
                owner->add("combat_exp",14+random(breed)*14 );
                owner->add("potential",random(breed*5) );
		me->add("food",random(10));
	}
	F_POISON->poison_attack(me,victim,"bt_poison",10 + me->query_skill("poison",1)/20 +
...
int auto_perform()
{
	...
	if ( !objectp(target)
	  || !random( (int)me->query("combat_exp") * 2
			/ ((int)target->query("combat_exp")+1) + 1) ) return 0;

        if(me->query("neili")<200) return 0;
	...
	if( random( me->query("combat_exp") ) > random( target->query("combat_exp") ) || !living(target) )
	{
			...
			bonus = damage/20 + random(damage/20);
                        ...
                        //added by whuan,限制挂机的效率
                        if(target->query("id") == "body guard") bonus /= 14;
...
//				if( owner->query_skill("training") < 400 )
				owner->improve_skill("training", owner->query("int") * 50 + owner->query("kar") * 20);
                                ...
                                breed = (breed)/100;
                                if(breed == 0) breed = 1;
                                if (owner->query("combat_exp")>=3000000) bonus = to_int(bonus * 2 / 3);
                                owner->add("combat_exp", (bonus + random(bonus))* breed);
				owner->add( "potential", random(bonus) * 5 );
			}
                        add("combat_exp", (bonus + random(bonus)) * 10 );
