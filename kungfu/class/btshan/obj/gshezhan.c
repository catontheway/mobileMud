...
void create()
{
        set_name(HIB"怪蛇杖"NOR, ({ "guai shezhang", "staff", "zhang","shezhang","she zhang" }));
	set_weight(4000);
	set("unit", "根");
	set("long", (: long_desc :));
	if (clonep())
...
string long_desc()
{
		  ...
		  desc = HIB"一根弯弯曲曲的黑色粗杖，似是钢铁所制，杖头铸着个裂口而笑的人头，\n人头口中露出尖利雪白的牙齿，模样甚是狰狞诡异，\n"NOR;
...
int convert(string arg)
{
		  ...
		  if (arg!="shezhang" && arg!="staff" && arg!="zhang" && arg!="guai shezhang") return 0;
	...
	if (!present("guai shezhang", me))
		return notify_fail("你身上没有怪蛇杖。\n");
		 ...
		 if (query("master_id") != me->query("id")) return notify_fail("蛇杖不是你的。\n");
		  ...
		  //seteuid(ROOT_UID);
		  set_default_object(__FILE__);
		  ob = new(CLASS_D("btshan")+"/obj/guaishe2");
		  ob->set_owner(me);
                ob->set("food",200);
       	  ob->set("who_breed", me->query("id"));
                ob->move(here);
		  add("snake",-1);
