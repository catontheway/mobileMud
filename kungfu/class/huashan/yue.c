...
#include <no1master.h>
#include "/kungfu/class/huashan/bonus.h"
#include "/kungfu/class/huashan/random_npc.h"
#include "/kungfu/class/huashan/random_user_quest.h"
int ask_job();
int ask_fail();
int ask_times();
int ask_finish();
...
int hsnewjob(object me)
{
	...
	while(!target && count < 5)
		...
		msg += CYN"$N说道：「多谢" + respect + NOR+CYN"，请将这封密函火速送到"
			...
			CYN"附近的"+ZJURL("cmds:tsdx "+target->query("id"))+HIR+target->query("name")+NOR+CYN"手上。」\n"NOR;
...
int ask_job()
{
	...
	else if(shen >= 0 && random(5))
	{
		...
		msg += CYN"$N对$n道：「我这里正好有封密函，麻烦你跑一趟，交给"
			+TASK_D->place_belong(where)+where->query("short")+NOR
			CYN"附近的"+ZJURL("cmds:tsdx "+target->query("id"))+HIR+target->query("name")+NOR+CYN"。」\n"NOR;
		message_vision(msg, this_object(), me);
	...
	else if(shen>=0 || (shen >= -100000000 && random(5)))
	{
		...
		msg += CYN"$N道：「多谢" + respect + NOR+CYN"，你赶紧把它送到"
			+TASK_D->place_belong(where)+where->query("short")+NOR
			CYN"附近的"+ZJURL("cmds:tsdx "+target->query("id"))+HIR+target->query("name")+NOR+CYN"手中。」\n"NOR;
		message_vision(msg, this_object(), me);
		command("care");
		me->set_temp("huashan/songxin/level", 1);
	}
...
command("whisper "+me->query("id")+" 这封机密函件，火速送给"
			...
			"附近的"+ZJURL("cmds:tsdx "+target->query("id"))+HIR+target->query("name")+NOR"，好处少不了你的。");
