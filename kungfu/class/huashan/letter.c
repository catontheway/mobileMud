...
void create()
{
        set_name(RED"密函"NOR, ({ "min han", "han", "xin", "letter" }));
        set_weight(10);
        if( clonep() )
...
{
        object killer = new(__DIR__"killer");
        killer->do_copy(me);
        return;
}
void stealer(object me)
{
        object stealer = new(__DIR__"killer");
        stealer->do_copy(me);
...
int do_give(string arg)
{
        ...
        message_vision(HIY"$N从怀中掏出信交给$n"HIY"，道：「这是君子剑岳不群先生托"
...
int do_look(string arg)
{
        ...
        add("long",HIC"           收信人："+query("target_name")+"("
                +HIG+capitalize(query("target_id"))+HIC")\n");
        if(time < 600)
