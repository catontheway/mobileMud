...
string ask_for_leave()
{
	...
	if( (string)me->query("family/family_name") != "日月神教")
		return "你非我日月神教弟子，脱离教派的事情我作不得主。";

	me->set_temp("pending/leave_riyue", 1);
  return "如果" + RANK_D->query_respect(me) + "真想离我日月神教而去，就请下决心(leave)吧！";
}
...
int do_leave()
{
	...
	if (me->query_temp("pending/leave_riyue") != 1)
		return 0;
	message_vision("$N对$n说道：「从此你就不再是我日月神教门下了，请好自为之！」\n",this_object(), me );
	tell_object(me,HIR "你决定叛出日月神教门下！\n" NOR);
	me->delete_temp("pending/leave_riyue");
  me->expell_family("日月神教");
  ...
  if( ::recruit_apprentice(ob) )
  {
    if((string)ob->query("gender")=="女性")
      ob->set("title",MAG "日月神教" NOR + HIR +"神女" NOR);
    	...
    	ob->set("title",MAG "日月神教" NOR + HIG +"长老" NOR);
...
int accept_object(object me,object ob)
{
  ...
  if ((string)ob->query("id") == "shou ji")
    if (me->query_temp("riyue/killer") == 1)
    {
      ...
      { 
      	me->set("title",MAG "日月神教" NOR + HIG "总管" NOR); 
...
void consider()
{
         ...
         switch( random(4) )
        {
            ...
            case 2:
            command("wield zhen");
            command("perform sword.ciwan " +target->query("id"));
            command("unwield zhen");
             break;
            case 3:
            command("wield zhen");
            command("perform sword.feiying ");
            command("unwield zhen");
             break;
             }
}
