...
void create()
{
    ...
    setup();
    carry_object("/d/dalicheng/obj/tiedi")->wield();
...
void unconcious()
{
	...
	if (objectp(ob = me->get_damage_origin_object()))
		ob->add("dali/trust", -1);
...
void attempt_apprentice(object ob)
{
	...
	if ((int)ob->query_skill("kurong-shengong", 1)< 50) {
...
string ask_me_1()
{
	...
	ob = new("/d/dalicheng/obj/allow-letter2");
...
string ask_me()
{
	object ob,me=this_player(),room=find_object("/d/dalicheng/wuhua2.c");
	mapping fam, log;
	...
	add("book_count", 1);
	ob = new("/d/dalicheng/obj/fojing2");
	ob->move(me);
...
//		return "你来晚了，铁链已经发完了。";
	...
	add("chain_count", 1);
	ob = new("/d/dalicheng/obj/chain.c");
	ob->move(me);
...
{
	object me=this_object(),room=find_object("/d/dalicheng/wuhua2.c");
