...
//      ::init();
        if (interactive(ob=this_player()) && 
                ob->query("family/master_id") == "duan yanqing"&& ob->query("shen") > -100000 ) {
                command("say " + RANK_D->query_rude(ob) + "胆敢不尊师训，不去为非作歹，你好大的胆子！" );
...
void attempt_apprentice(object me)
{
       ...
       if( me->query("combat_exp") < 100000 ) {
        ...
        if ((string)me->query("gender") != "男性") {
                command("say 我只收男徒，" + RANK_D->query_respect(me) + "还是另投高明吧。");
                return;
        } 
        if( me->query("PKS") < 50) {
                command("say " + RANK_D->query_respect(me) + "你先去干掉几个白道人物再来拜我，局时我一定收你。");
                return;
        }
