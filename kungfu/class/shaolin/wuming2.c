 // wuming.c 无名老僧
 // Create by chui
 // Add riyue-bian/ranmu-dao by jpei
#include <ansi.h>
inherit NPC;
inherit F_MASTER;
#include <title.h>
...
//title系统记录玩家title by seagate@pkuxkx
        ...
        ob->set("name" , "渡" + ob->query("name")[1..2]);
...
message_vision("无名老僧对$N缓缓念道：这位施主这又何苦呢？\n",ob);
        ...
        if ((int)ob->query("shen") < 0 && (string)ob->query("family/master_id") == "wuming laoseng") 
        {
                ...
                command("say 你多次行凶越货，胡作非为，再这样下去，我岂能仍是你的师傅。\n");
