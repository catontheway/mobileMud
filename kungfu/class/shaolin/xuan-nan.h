...
int waiting(object me)
{
	...
	else if( !present("xuanku dashi", environment(me)) )
	{
		if( random(10) == 0 ) 
	...
	else
    {
        ...
        call_out("preparing", 0, me, fighter);
    }
...
int preparing(object me, object fighter)
{
  ...
  if ( !fighter->query("family/family_name") )
  	CHANNEL_D->do_channel(this_object(), "jh", "百姓" + fighter->query("name") +
                "于今日" + NATURE_D->game_time() + "挑战少林十八罗汉大阵！");
  else
    CHANNEL_D->do_channel(this_object(), "jh", fighter->query("family/family_name") + "弟子" + fighter->query("name") +
                "于今日" + NATURE_D->game_time() + "挑战少林十八罗汉大阵！");
