...
int do_copy(object me,int maxpot,int type)
{
    ...
    ob->set("combat_exp", me->query("combat_exp")/10*7);
...
void die()
{
    ...
    exp= random(2000) + 1000;//增加500K以前奖励 Zine Nov 10 2010
          if(this_player()->query("combat_exp")>600000) {exp= random(800) + 800;} //降低500K以后奖励
          if(this_player()->query("combat_exp")>1100000) {exp= random(900) + 900;} //1M后奖励略高于500k，还是比之前低
