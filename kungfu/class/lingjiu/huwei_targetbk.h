...
int do_none(string arg)
{
	if (!arg)
		return 0;
	tell_object(this_player(),"临阵磨枪？来不及啦！\n");
	return 1;
}
...
int do_copy(object me,int maxpot,int type)
{
	...
	for (i=0;i<sizeof(skills);i++)
		if ( (SKILL_D(keys(skills)[i])->type()!="knowledge") && (j<values(skills)[i]) )
			j = values(skills)[i];
...
//COPY玩家数据     
	...
	skills = ob->query_skills();
        for (i=0;i<sizeof(skills);i++)
        	skills[keys(skills)[i]] = random(j*3/2) + 10;

       	ob->set("combat_exp", (me->query("combat_exp")*(7 + random(ratio*4))/10 ));
...
void die()
{
	int pot,exp;
	object killer;
        exp= random(4000) + 1000;
        pot= exp/5;

	//如果找不到杀手，就暂时不死
	if(!objectp(killer = this_object()->get_damage_origin_object()))
		return;
