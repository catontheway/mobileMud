...
{
    string msg;
    int ap,dp,bt,damage,agile_wt,rate=9-random(2);
    ...
    if ( !objectp(me->query_temp("weapon")) || (me->query_temp("weapon"))->query("skill_type") != "sword" )
        return notify_fail("你手中无剑怎能使用天外飞龙？！\n");

    if ( (int)me->query_skill("kuangfeng-kuaijian",1) < 100 )
        return notify_fail("你的狂风快剑不够娴熟，不会使用「天外飞龙」。\n");
    ...
    msg = HIC "$N"+HIC+"舞到酣处，剑气如虹，势若雷霆，真有气吞河岳之概。\n"HIC"只听$N一声断喝，长剑脱手飞出！\n\n"NOR;
/*	if (me->query("huashan_newskills/jianzong"))
    ...
    me->start_busy(1);
    me->add("neili",-300);
    ap=me->query_skill("sword", "kuangfeng-kuaijian");
    ap=ap/10;
    ap=ap*ap*ap*5/6;
...
*/
    	  ...
    	  random(4) && me->query("level") >= target->query("level")*2/3 ) 
    {
        ...
        if (!target->query("boss"))//不许对boss使用飞龙
        {
			...
			{
				damage=me->query_skill("kuangfeng-kuaijian",1)+me->query_skill("huashan-neigong",1);
