...
{
		if (me->query_skill("hunyuan-gong"))
			return notify_fail("你已经学会混元功，无法在再次练习混元掌了。\n");
...
mapping query_action(object me, object weapon)
{
        ...
        for(i = sizeof(action); i > 0; i--)
                if(level > action[i-1]["lvl"])
					{if (!me->query("huashan_newskills/jianzong")&&me->query_skill("hunyuan-zhang",1)>1200)
                        return action2[NewRandom(i, 20, level/5)];
					else
						return action2[NewRandom(i, 20, level/5)];}
}
...
{
		if (me->query_skill("hunyuan-zhang",1)>1200&&me->query("huashan_newskills/jianzong"))
		{
			me->set_skill("hunyuan-gong",me->query_skill("huashan-neigong",1)>me->query_skill("zixia-shengong",1)?me->query_skill("huashan-neigong",1):me->query_skill("zixia-shengong",1));
			me->delete_skill("hunyuan-zhang");
			tell_object(me,"你由内而外，修炼混元功有成。\n");
			return 0;
		}
		if (me->query_skill("hunyuan-gong"))
			{
