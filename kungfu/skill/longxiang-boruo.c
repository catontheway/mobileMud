...
{
        int skill,my_jiali,my_neili,hxc;
        int i = 1,j;
	//	mapping my;
        object weapon;    
	...
	//	my = me->query_entire_dbase();
		...
		hxc = me->query_skill("huanxi-chan");
...
mapping buffer_applied(object ob)
{
    ...
    lv1=ob->query_skill("longxiang-boruo",1);
    lv2=ob->query_skill("huanxi-chan",1);
    gint=ob->query("con");
    buf=([]);
   ...
   // if ( !ob->query("huashan_newskills/qizong") )
    ...
    if ( mapp(ob->query_temp("passive/longxiang-boruo")) )
      return buf;
    
	pflv = (int)ob->query("purefamily/level");
    if (pflv>0 && ob->query("purefamily/family_name") != "大轮寺")	pflv = 0;

    buf["max_qi"]=(lv2/15 + lv1/15)*(100+gint)*2/5;
	 buf["strength"]=lv1*1/20;
    buf["max_jing"]=(lv2/15 + lv1/15)*(100+gint)*3/5;
    ob->set_temp("passive/longxiang-boruo", buf);
    ob->set_temp("active_force_buffer/longxiang-boruo/name", "大龙象护体");                              //buff名称
    ob->set_temp("active_force_buffer/longxiang-boruo/effect1", "最大气血+"+(buf["max_qi"])+"点");     //效果描述1
    ob->set_temp("active_force_buffer/longxiang-boruo/effect2", "最大精神+"+(buf["max_jing"])+"点");
    ob->set_temp("active_force_buffer/longxiang-boruo/effect5", "战斗臂力+"+(buf["strength"])+"点");	//效果描述2
	if (pflv>=5)
	{
		buf["kf_hit_rate"] = 10;
		buf["deep_injure"] = 5;
		ob->set_temp("active_force_buffer/longxiang-boruo/effect3", "技能命中提升+10%");   //效果描述2
		ob->set_temp("active_force_buffer/longxiang-boruo/effect4", "普通伤害增益+5%");   //效果描述2
	}
    return buf;
}
mapping buffer_removed(object ob)
{
    ...
    buf=ob->query_temp("passive/longxiang-boruo");;
    if ( mapp(buf) )
    {
    ...
    ob->delete_temp("passive/longxiang-boruo");
    ob->delete_temp("active_force_buffer/longxiang-boruo");
    return buf;
}
