...
inherit F_SSERVER;
string query_name() { return " 虎啸"ZJBR"山林 "; }
int perform(object me, object target)
{
        string msg; int extra;

   	extra = me->query_skill("taizu-changquan",1);
   	if( !target ) target = offensive_target(me);
        ...
        if( (int)me->query_skill("taizu-changquan", 1) < 120 )
                return notify_fail("你的太祖长拳不够娴熟，不会使用「虎啸山林」。\n");
        ...
        if (me->query_skill_prepared("cuff") != "taizu-changquan"
        || me->query_skill_mapped("cuff") != "taizu-changquan")
