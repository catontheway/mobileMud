...
int perform(object me, object target)
{
	int level;
      ...
      level = me->query_skill("yiyangzhi", 1);

	if( !target ) target = offensive_target(me);
	
	if( target == me ) 
		return notify_fail("使用一阳指不能给自己疗伤。\n");
         if((int)me->query_skill("kurong-shengong", 1)<20 && (int)me->query_skill("beiming-shengong", 1)<20)
	...
	HIY"只见"+(string)me->query("name")+"深深的吸了一口气，脸色顿时变得紫红，伸出右手食指，运指如飞，\n"
        "沿着"+(string)target->query("name")+"的任、督二脉一路点将下来，只见"
	+(string)target->query("name")+"“哇”的吐出一口鲜血，脸色看起来好多了。\n"NOR,({me,target}));

	tell_object(me,HIY"你盘膝坐下，调匀了气息，使出一阳指的独门功夫，开始疗伤。\n"NOR);
        tell_object(target,HIY"只见"+(string)me->query("name")+"盘膝坐下，使出一阳指的独门功夫，开始给你疗伤。\n"NOR);
	me->add("neili", -200);
