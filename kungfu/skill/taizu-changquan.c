...
int valid_combine(string combo) { return combo=="yuejia-sanshou"; }
int valid_learn(object me)
{
        if( me->query_temp("weapon") || me->query_temp("secondary_weapon"))
                return notify_fail("练[太祖长拳]必须空手。\n");
        return 1;
}
...
{
        if( me->query_temp("weapon") || me->query_temp("secondary_weapon"))
                return notify_fail("练[太祖长拳]必须放下兵器。\n");
...
{
	return __DIR__"taizu-changquan/" + func;
}
mixed hit_ob(object me, object victim, int damage_bonus, int factor, string limb)
{
	int ap,dp, damage1,damage2,damage,liannum;

    ap = me->query_skill("taizu-changquan",1)*(me->query("breakup")+me->query("breakup")*me->query("breakup")+100)/100;
	dp = victim->query_skill("parry",1)*(victim->query("breakup")+victim->query("breakup")*victim->query("breakup")+100)/100;
    damage1= (int) me->query_skill("taizu-changquan",1);
	damage2 = (int)me->query_skill("taizu-changquan",1)*(me->query("breakup")+me->query("breakup")*me->query("breakup")+100)/100;
	...
	if (  me->query_skill("taizu-changquan",1)<100) return 0;
//	if ( me->query_skill_mapped("force") != "yijing-force") return 0;
	...
	if (random(7)==1 && random(ap) > dp/3 && me->query("jiali") > 0)
	{
		...
		me->add("neili", -100);
		message_vision(HIY"只见$n口吐一道鲜血，已被太祖长拳的拳劲打成内伤！\n"NOR, me,victim,damage,HBWHT+HIR"特效伤气"NOR);
        return ;
	}
	...
	if( random(ap) > dp/3 && (int)me->query("jiali")>0 && random(7)==3 && (int)me->query("jm/shaolin") )
	{
		...
		victim->receive_damage("qi", damage,me);
		victim->receive_wound("qi", damage/3,me);
		message_vision(HIM"$N双拳急速攻出，短时间内发出无数拳影。\n" NOR, me,victim,damage,HBWHT+HIR"特效伤气"NOR);
		return ;
	}
}
