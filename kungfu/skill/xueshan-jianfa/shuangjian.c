...
inherit F_SSERVER;
#define Menpai "雪山派"
string query_name() { return "霜  剑 "; }
string perform_name(){ return HIY" 霜  剑 "NOR; }
...
int perform(object me, object target)
{
	...
	if( (int)me->query("neili") < 1000 )
		...
		return notify_fail("你真气运转方式不对，使不出「霜剑」。\n");
	if (!objectp(weapon = me->query_temp("weapon")) || weapon->query("skill_type") != "sword")
//	    || me->query_skill_mapped("sword") != "xueshan-jianfa")
		return notify_fail("你不装备一把剑，使不出「霜剑」。\n");

	if( me->query_temp("shuangjian") )
		return notify_fail("你正在使用「霜剑」！\n");
  ...
  me->set_temp("active_buffer/xueshan-jianfa.shuangjian/name", "霜剑");                          //buff名称
  me->set_temp("active_buffer/xueshan-jianfa.shuangjian/last_time", time() + me->query_skill("xueshan-jianfa",1)/4 );                //持续时间
  me->set_temp("active_buffer/xueshan-jianfa.shuangjian/effect1", "攻击伤害+"+(me->query_skill("xueshan-jianfa",1)/4)+"点");         //效果描述1
	msg += HIY"\n$N剑法一转，便见朔风呼号之势，正是雪山剑法之「霜剑」！\n"NOR;
	message_vision(msg,me,target);
	
	call_out("remove_effect", to_int(me->query_skill("xueshan-jianfa",1)/4) , me);
	return 1;
}
