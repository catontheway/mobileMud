...
inherit F_SSERVER;
#define Menpai "雪山派"
string query_name() { return " 梅  剑 "; }
string perform_name(){ return HIM" 梅  剑 "NOR; }
...
int perform(object me, object target)
{
	...
	if( (int)me->query("neili") < 800 )
		...
		return notify_fail("你真气运转方式不对，使不出「梅剑」。\n");
	if (!objectp(weapon = me->query_temp("weapon")) || weapon->query("skill_type") != "sword")
//	    || me->query_skill_mapped("sword") != "xueshan-jianfa")
		return notify_fail("你不用雪山剑法，使不出「梅剑」。\n");

	if( me->query_temp("meijian") )
		return notify_fail("你正在使用「梅剑」！\n");
  ...
  me->set_temp("active_buffer/xueshan-jianfa.meijian/name", "梅剑");                          //buff名称
  me->set_temp("active_buffer/xueshan-jianfa.meijian/last_time", time() + me->query_skill("xueshan-jianfa",1)/4 );                //持续时间
  me->set_temp("active_buffer/xueshan-jianfa.meijian/effect1", "轻功技能+"+(me->query_skill("xueshan-jianfa",1)/2)+"级");         //效果描述1
	msg += HIM"\n$N剑法一转，便见红梅摇曳之景，正是雪山剑法之「梅剑」！\n"NOR;
...
int help(object me)
{
        write(WHT"\n雪山剑法「梅剑」："NOR"\n");
        write(@HELP
	雪山剑法有四种变化：梅、雪、风、霜，这「梅剑」便是其中之一。红梅摇
	曳之景，雪花飞舞之姿，朔风呼号之势，冷霜铺地之态，均可分别在这四种
	剑法中找到。「梅剑」可以在与敌作战时，提高自己的躲闪能力。
