...
int valid_combine(string combo) { return combo=="xueshan-jianfa"; }
int valid_learn(object me)
{
        ...
        if (me->query_skill("xueshan-jianfa",1) < 100)
                return notify_fail("你对雪山剑法的领悟太浅，无法学习金乌刀法。\n");               

        if ((int)me->query_skill("blade", 1) < 30)
                return notify_fail("你的基本剑法火候太浅，无法学习金乌刀法。\n");

        if (me->query_skill("blade", 1) +10< me->query_skill("jinwu-daofa", 1))
