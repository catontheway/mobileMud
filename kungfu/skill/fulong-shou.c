...
int valid_learn(object me)
{
	if (me->query_temp("weapon") || me->query_temp("secondary_weapon"))
		return notify_fail("练乾坤伏龙手必须空手。\n");
	if ((int)me->query_skill("cangming-gong", 1) < 50)
		return notify_fail("你的浩气苍冥功火候不够，无法学乾坤伏龙手。\n");
	if ((int)me->query_skill("force", 1) < 50)
		return notify_fail("你的基本内功火候不够，无法学乾坤伏龙手。\n");
	if ((int)me->query("max_neili") < 250)
		return notify_fail("你的内力太弱，无法练乾坤伏龙手。\n");
	if ((int)me->query_skill("hand",1) <30)
		return notify_fail("你的基本手法火候不够，无法学乾坤伏龙手。\n");
	if (2*(int)me->query_skill("cangming-gong",1) <(int)me->query_skill("fulong-shou",1))
		return notify_fail("你的浩气苍冥功火候不够，无法继续学乾坤伏龙手。\n");
	return 1;
}
...
{
	int i, level;
	level   = (int) me->query_skill("fulong-shou",1);
	for(i = sizeof(action); i > 0; i--)
		if(level > action[i-1]["lvl"])
...
int practice_skill(object me)
{
	...
	if ((int)me->query("neili") < 20)
		return notify_fail("你的内力不够练乾坤伏龙手。\n");
...
{
	return __DIR__"fulong-shou/" + action;
}
