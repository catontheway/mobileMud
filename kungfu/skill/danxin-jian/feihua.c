...
inherit F_SSERVER;
string query_name() { return "雨打" ZJBR "飞花剑"; }
...
void checking(object me, object target)
{
	...
	mapping attack_val = (["base":(["weapon":"sword"]),		  // 基本功夫类型
	...
	if ((!objectp(weapon = me->query_temp("weapon")) || (string)weapon->query("skill_type") != "sword") && (!objectp(weapon = me->query_temp("secondary_weapon")) || (string)weapon->query("skill_type") != "sword"))
	{
		tell_object(me, "\n你手中无剑，无法继续使用「雨打飞花剑」。\n" NOR);
		me->delete_temp("xunlei");
		call_out("xunlei_end", 5 + random(3), me);
			...
			while (i)
			{
				...
				if (da > 0)
				{
					...
					if (random(2))
					{
						...
						message_vision(HIC "$N顺着刺中" + target->query("name") + HIC "这一剑，身随剑转，" + weapon->name() + HIC "紧接着划出「雨打式」，闪闪寒星如扑面骤雨！\n" NOR, me, target);
						for (j = 0; j < count; j++)
						{
					...
					else
					{
						...
						message_vision(HIW "$N顺着刺中" + target->query("name") + HIW "这一剑，掌中" + weapon->name() + HIW "顺势一挽，「飞花式」剑光如雪四散而飞！\n" NOR, me, target);
						for (j = 0; j < count; j++)
						{
							if (F_KUNGFU->hit_rate(me->query_skill("force", "cangming-gong"), enemy[j]->query_skill("dodge"), rate_fix, me, enemy[j]))
							{
