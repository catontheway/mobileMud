...
mapping buffer_applied(object ob)
{
    ...
    lv1=ob->query_skill("hama-gong",1);
  ...
  if ( ob->query_attr_version()<140913 )
    ...
    if ( mapp(ob->query_temp("passive/hama-gong")) )
      return buf;
    ...
    ob->set_temp("active_force_buffer/hama-gong/name", "蛤蟆护体");                              //buff名称
    ob->set_temp("active_force_buffer/hama-gong/effect1", "最大气血+"+(buf["max_qi"])+"点");     //效果描述1
    ob->set_temp("active_force_buffer/hama-gong/effect2", "最大精神+"+(buf["max_jing"])+"点");
    ob->set_temp("active_force_buffer/hama-gong/effect5", "战斗臂力+"+(buf["strength"])+"点");	//效果描述2
...
mapping buffer_removed(object ob)
{
    ...
    buf=ob->query_temp("passive/hama-gong");;
    if ( mapp(buf) )
    {
    ...
    ob->delete_temp("passive/hama-gong");
    ob->delete_temp("active_force_buffer/hama-gong");
    return buf;
}
