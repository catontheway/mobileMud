...
inherit F_SSERVER;
string query_name() { return "断"ZJBR"云"; }
int perform(object me, object target)
{
	...
	if( me->query("jing") <= 300 )
		return notify_fail("你的精神不够使用「断云」！\n");

	if ( !objectp(weapon = me->query_temp("weapon"))
	|| ( string)weapon->query("skill_type") != "whip" )
		return notify_fail("你手中无鞭如何使用断云鞭法的绝招？\n");

	if(me->query_temp("duanyun"))
		return notify_fail("你正在使用「断云」绝招。\n");

	message_vision(HIW"\n$N心中平和，渐渐将断云鞭法使得圆转如意，手中"+(weapon->query("name"))+HIW"在内力催动下闪出点点银光，有如云彩将$n逐渐包围在其中。\n\n"NOR,me,target);
...
//C-
		if( F_KUNGFU->hit_rate(ap, dp, 9, me, target) )
		{
			...
			damage = me->query_skill("duanyun-bian",1) * (random(3) + 1);
