...
int valid_learn(object me)
{
        ...
        if ((int)me->query_skill("blade", 1) < (int)me->query_skill("luojia-dao", 1))
                return notify_fail("你的基本刀法水平有限，无法领会更高深的骆家刀法。\n");
...
{
        int i, level;
        level = (int) me->query_skill("luojia-dao",1);
...
int practice_skill(object me)
{
        ...
        if ((int)me->query("qi") < 60)
                return notify_fail("你的体力不够练骆家刀法。\n");

        if ((int)me->query("neili") < 60)
                return notify_fail("你的内力不够练骆家刀法。\n");
...
{
        return __DIR__"luojia-dao/" + action;
}
