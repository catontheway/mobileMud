...
protected int remove_effect(object me, object weapon);
string query_name() { return "一阳"ZJBR"指力"; }
int perform(object me, object target)
{
        object weapon;
        int skill, skill1;

        if (me->query_skill_mapped("force") != "kurong-shengong")
        ...
        if (me->query_skill("force",1)<100
                || me->query_skill("kurong-shengong",1)<100
        ...
        skill = me->query_skill("kurong-shengong",1);

        me->set_temp("yiyang", 25+skill/6);

        skill = me->query_skill("yiyang-zhi",1) + me->query_skill("kurong-shengong",1);
...
void checking(object me, object weapon)
{
        ...
        if (me->query_temp("yiyang")) {
           ...
           else if (me->query_skill_mapped("force") != "kurong-shengong") {
                remove_effect(me, weapon);
