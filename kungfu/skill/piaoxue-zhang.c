...
int valid_learn(object me)
{
        if (me->query_temp("weapon") || me->query_temp("secondary_weapon"))
                return notify_fail("练飘雪掌法必须空手。\n");

        if ((int)me->query("con") < 18)
                return notify_fail("你的先天根骨欠佳，不能练飘雪掌法。\n");

        if ((int)me->query("str") < 20)
                return notify_fail("你的先天臂力孱弱，不能练飘雪掌法。\n");

        if (me->query_skill("force") < 200)
                return notify_fail("你的内功火候不够，不能练飘雪掌法。\n");

        if (me->query_skill("strike", 1) < 100)
                return notify_fail("你的掌法根基不够，不能练飘雪掌法。\n");

        if (me->query("max_neili") < 1200)
                return notify_fail("你的内力修为太浅，无法练飘雪掌法。\n");

        if (me->query_skill("strike", 1) < me->query_skill("piaoxue-zhang", 1))
                return notify_fail("你的基本掌法太差，无法领会更高深的飘雪掌法。\n");
...
int practice_skill(object me)
{
        ...
        if ((int)me->query("neili") < 120)
                return notify_fail("你的内力不够练习飘雪掌法。\n");

        if (me->query_skill("piaoxue-zhang", 1) < 100)
        {
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
        ...
        {
                F_KUNGFU->hit_wound("qi", (damage_bonus - 140) / 3, 0, me, victim);
