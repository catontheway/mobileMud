...
([  "action" : "$N挪步小退，一招"+HIC"「风花雪月」"NOR+"，手中$w轻描淡写地挥出一刀，
...
int valid_combine(string combo) { return combo=="kuangfeng-jian"; }

int valid_learn(object me)
{
    if ((int)me->query_skill("bixue-xinfa", 1) < 150)
        return notify_fail("学习狂风刀法需要碧血心法 150 级。\n");
    return 1;
}
...
{
    int i, level;
    level   = (int) me->query_skill("kuangfeng-dao",1);
...
int practice_skill(object me)
{
    if (me->query("neili") < 50 || me->query("qi")< 50)
        return notify_fail("你的内力或体力不足以练「狂风刀法」。\n");
...
{
    return __DIR__"kuangfeng-dao/" + action;
}
