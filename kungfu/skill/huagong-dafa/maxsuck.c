...
#include <ansi.h>
string query_name() { return "吸人"ZJBR"丹元"; }
...
//命中率级别C-
        if( F_KUNGFU->hit_rate(sp, dp, 9, me, target) ) 
        {
                ...
                target->add("max_neili",  -1*(1+(me->query_skill("huagong-dafa", 1)-90)/15) );
            ...
            if( (int)me->query("potential") - (int)me->query("learned_points") < 290 )
                ...
                me->add("neili", -20);
            //稍微提高走火的门槛 by seagate
                if ( me->query("max_neili") > me->query_max_neili() )
                {
            //当内力超过最大内力的时候随机产生走火隐患，走火隐患概率级别D级 by seagate
                        if ( F_KUNGFU->hit_rate(me->query("max_neili"), me->query_skill("force")*(10+me->query("con")), 11, me, me) )
                        {
                                tell_object(me,HIR "你忽然觉得膻中气海中的内息有些异样，似乎不太能够听你使唤，你开始有些烦躁...\n" NOR);
                                tell_object(me,HIY "可能是你吸收的丹元过多你无法承受！" NOR);
                                tell_room(environment(me),"你看到" + me->name() + "忽然皱了皱眉，深深吸了几口气。\n",me);
                                me->add_temp("suck/zouhuo",1);
                        }
                        if ( random(me->query_temp("suck/zouhuo")) > 8 )
                        {
