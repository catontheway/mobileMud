...
{
        int grade, min_def, max_def, def, weight = 1000 + random(600);
        string id;
        string name;
        if(!min) min = 0;         
        if(!max) max = 10000;     
...
void generate(object me,string name,int suc,int smith,int err,int lv)//suc是成功打造的。
{
	...
	return; //错误类型
	gen_armor(member_array(name,all_types)+1);
	set("value",500);
	...
	ReGen("armor/min_def","armor/max_def","armor/armor","armor_prop/armor",suc,err,smith,lv);
	return;
}
