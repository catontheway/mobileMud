...
// modified by Zine@pkuxkx to allow player to generate new weapon.
#include <ansi.h>
#include <weapon.h>
...
int get_grade(int min, int max)
{
		...
		if (tmp_grade=query_temp("gen_grade"))
		{
			delete_temp("gen_grade");
			if (tmp_grade>1&&tmp_grade<8)
			return tmp_grade-1+random(3);
			if (tmp_grade<=1)
			return 1+random(2);
			if (tmp_grade>=8)
			return 8-random(2);
...
{
        int i,j;
        int grade, rigidity, min_dam, max_dam, dam, weight = 2000 + random(600);
...
string spec_effect(object me, object victim, int damage_bonus, string act)
{
        ...
        case "weapon_effect_poison":
				...
				F_POISON->poison_attack(me,victim,"emerald_poison", victim->query_condition("emerald_poison") + query("weapon_effect/poison"));
...
void ReGen(string prop1,string prop2,string prop3,string prop4,int suc,int err,int smith,int lv)
{
	int drop;
	if (err)
	{
	...
	if (!suc)
	{
		set(prop3,query(prop1));
		set(prop4,query(prop1));
	}
	if (smith)
	{
		drop=random(20-lv);
		if (query(prop3)<query(prop2)-drop)
		{
			set(prop3,query(prop2)-drop);
			set(prop4,query(prop2)-drop);
		}
	}
	if (query(prop1)<1) set(prop1,1);
	if (query(prop2)<1) set(prop2,1);
	if (query(prop3)<1) set(prop3,1);
	if (query(prop4)<1) set(prop4,1);
}
void generate(object me,string name,int suc,int smith,int err,int lv)//suc是成功打造的。
{
	string longdesc="此"+name+"为";
	if (member_array(name,all_types)==-1)
	return; //错误类型
	gen_weapon(member_array(name,all_types)+1);
	set("value",100);
	if (!me->query("env/CastNoSignature"))
	{
		if (smith)
		longdesc+=RANK_D->query_rank(me);
		longdesc+=me->query("name")+"的第"+chinese_number(me->query("myWeaponCast/stat/TotalWorks"))+"件作品。\n";
		set("long",query("long")+"\n"+longdesc);
	}
	if (err)
	set("long",query("long")+name+"上似乎有"+chinese_number(err)+"道细纹。\n");
	ReGen("weapon/min_dam","weapon/max_dam","weapon/damage","weapon_prop/damage",suc,err,smith,lv);
	return;
}