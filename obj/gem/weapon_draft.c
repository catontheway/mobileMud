...
// modified by Zine 增加一道工序，掉兵器草图而不直接掉武器了，为新职业准备
#include <ansi.h>
#include <weapon.h>

inherit F_GEMS;
inherit EQUIP;
int is_draft() {return 1;}//兵器草图，不能wield
...
varargs void gen_weapon(int seed,int min,int max,string type) 
{
        int i,j;
        int grade, rigidity, min_dam, max_dam, dam, weight = 600 + random(600);
        string name,id,new_name;
        ...
        if (!type||member_array(type,all_ids)==-1)//加上个功能可以指定生成的weapon类型 Zine 20110726
        {
            type=all_ids[random(sizeof(all_ids))];
        ...
        else
        {
            ...
            for (j=0;j<sizeof(all_ids);j++)
            {
                if (all_ids[j]==type)
                {
                    new_name=all_types[j];
                }
            }
            name=all_colors[grade-1]+all_grades[grade-1]+"之"+new_name+NOR;
        }
        
        set("seed",i); //icer added to clone the same weapon when player using this random weapon
        min_dam = 50+random(5*grade);
        max_dam = 91+random(20*grade);
        ...
        set_weight( 10);//草图不应该重       
        set("material", "paper");
        set("rigidity", 200+random(grade*15));
        set("value",10000+random(20000+grade*10000));  
        set("weapon/min_dam", min_dam);
        set("weapon/max_dam", max_dam);
        set("weapon/damage", dam);         
        set("weapon_prop/damage", dam);
        set("forge/times_max",1);//可锻造次数靠后天任务增加
        set("generation",2);
        forge_capacity();
}

object get_weapon_out()
{
    object me=this_object();
    object ob=new("obj/gem/weapon");
    ob->set("level",me->query("level"));
    ob->set("skill_type",me->query("skill_type"));
    ob->set("seed",me->query("seed"));
    ob->set_name(me->query("original_name"),({me->query("original_id")}));
    ob->set_weight(me->query("base_weight"));
    ob->set("material","steel");
    ob->set("rigidity", me->query("rigidity"));
    ob->set("value",me->query("value"));  
    ob->set("weapon/min_dam", me->query("weapon/min_dam"));
    ob->set("weapon/max_dam", me->query("weapon/max_dam"));
    ob->set("weapon/damage", me->query("weapon/damage"));         
    ob->set("weapon_prop/damage", me->query("weapon_prop/damage"));
    ob->set("forge/times_max",me->query("forge/times_max"));//可锻造次数靠后天任务增加
    ob->set("generation",me->query("generation"));
    ob->set("forge/max", me->query("forge/max"));
    ob->set("unit", me->query("original_unit"));
    ob->set("long", me->query("original_long"));
    ob->set("max_rigidity",me->query("max_rigidity"));
    ob->set("wield_msg", me->query("wield_msg"));
    ob->set("unwield_msg", me->query("unwield_msg"));
    ob->set("actions",me->query("actions"));
    ob->set("verbs",me->query("verbs"));
    return ob;
}
...
void setup_weapon(object ob, int flag)
{
        ...
        if(stringp(ob->query("weapon/name")))
        {
            ...
            set("original_name",ob->query("weapon/name"));
            set("original_id",ob->parse_command_id_list());
        }
        set_name(ob->query("original_name")+" 草图", ({"cao tu","draft"}) );
        ob->set("original_unit", "柄");
        ob->set("unit","张");
        ...
        set("original_long", longdes);
        set("long","这是一张记录了"+ob->query("name")+"(可朔性："+ob->query("forge/max")+")"+"打造方法的草图。\n");
        //材料
