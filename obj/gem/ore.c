...
// upgraded by becool
// 把直接掉落宝石改成掉落矿石，增加一个步骤转换成宝石，其实还是宝石，只是不能用，名字换一下。为以后增加职业做铺垫 by Zine
#include <ansi.h>
...
int is_gem() {return 0;}// /feature/gems.c里面是is_gems() 原矿预防在特定场合被当成gem
int is_ore() {return 1;}
void create()
{
	int lev, i;
	...
	//掉落率：炎晶:木灵:玉髓:精金:玄冰:神龙骨:凤凰羽:麒麟角:玄武甲 = 3:3:3:3:3:1:1:1:2
	i = random(20);
	if (i < 3) gem = "yan";
	else if (i < 6) gem = "mu";
	else if (i < 9) gem = "sui";
...
void set_level(int min, int number, string gem)
{
	...
	set_weight( 100 *lev*lev*(2+random(3)));
    set("original_name",query("name"));//需还原
    set_name("矿石",({"kuang shi","shi","ore"}));
...
object recover()
{
    object ob=this_object(),new_ob;
    string gem_type,yinyang;
    int gem_level;
    gem_type=ob->query("ore_type");
    yinyang=ob->query("yinyang");
    gem_level=(int)ob->query("level");
    new_ob=F_GEMS->new_gem(gem_type,gem_level,yinyang);
	return new_ob;     
}