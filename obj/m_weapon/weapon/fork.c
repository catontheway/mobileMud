...
void create()
{
    ...
    set_desc();
	set("rigidity",500);
	set("wield_msg", "$N从背后拿出一把$n，然後握在手中。\n");
    set("unwield_msg", "$N放下手中的$n。\n");
	
	init_fork(this_object()->query("level")*10);
	setup();
}
void init()
{
...
int do_duanlian(string weapon_id)
{
	...
	message_vision(HIY "叉身忽的一亮，一道金光隐入$N的叉体，不见了！\n" NOR,me);
	message_vision(HIG "$N的叉的等级提高了！\n" NOR, me);
	if(weapon->query("equipped") ) 
		weapon->unequip();
	weapon->add("level",1);
	weapon->set_desc();
	weapon->init_fork(weapon->query("level")*10);
	weapon->save();
	weapon->restore();
	return 1;
}
