...
void create()
{
	...
	init_staff(this_object()->query("level")*10);
	setup();
}
void init()
{
...
int do_duanlian(string weapon_id)
{
	...
	weapon->add("level",1);
	weapon->set_desc();
	weapon->init_staff(weapon->query("level")*10);
	weapon->save();
	weapon->restore();
	return 1;
}
