...
void create()
{
	...
	init_hammer(this_object()->query("level")*10);
	setup();
}
void init()
{
...
int do_duanlian(string weapon_id)
{
	...
	if( (int)me->query("gin") < 30 )
		return notify_fail("你的精不够，无法锻炼锤气！\n");
	
	if( (int)me->query("sen") < 30 )
		return notify_fail("你的神不够，无法锻炼锤气！\n");
	...
	if( me->query("eff_sen")*100/me->query("max_sen") < w_level*25 )
		return notify_fail("你现在的神气不足，不能锻炼锤气！\n");
	...
	if( ((int)me->query("combat_exp") < 125*w_level)||((int)me->query("business_exp") < 125*w_level) )
		return notify_fail("你的经验不够，无法锻炼锤气！\n");

	message_vision(HIR "$N手指锤脊，一股内力丝丝的传了进去。\n" NOR, me);
	...
	weapon->add("level",1);
	weapon->set_desc();
	weapon->init_hammer(weapon->query("level")*10);
	weapon->save();
	weapon->restore();
	return 1;
}
