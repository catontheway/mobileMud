...
void create()
{
	...
	init_axe(this_object()->query("level")*10);
	setup();
}
void init()
{
...
int do_duanlian(string weapon_id)
{
	...
	message_vision(HIY "斧身忽的一亮，一道金光隐入$N的斧体，不见了！\n" NOR,me);
	message_vision(HIG "$N的斧的等级提高了！\n" NOR, me);
	if(weapon->query("equipped") ) 
		weapon->unequip();
	weapon->add("level",1);
	weapon->set_desc();
	weapon->init_axe(weapon->query("level")*10);
	weapon->save();
	weapon->restore();
	return 1;
}
