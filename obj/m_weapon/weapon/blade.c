...
void create()
{
	...
	init_blade(this_object()->query("level")*10);
	setup();
}
void init()
{
...
int do_duanlian(string weapon_id)
{
	...
	if( (int)me->query("gin") < 30 )
		return notify_fail("你的精不够，无法锻炼刀气！\n");
	
	if( (int)me->query("sen") < 30 )
		return notify_fail("你的神不够，无法锻炼刀气！\n");
	...
	if( me->query("eff_sen")*100/me->query("max_sen") < w_level*25 )
		return notify_fail("你现在的神气不足，不能锻炼刀气！\n");
	...
	if( ((int)me->query("combat_exp") < 125*w_level)||((int)me->query("business_exp") < 125*w_level) )
		return notify_fail("你的经验不够，无法锻炼刀气！\n");
	...
	message_vision(HIY "刀身忽的一亮，一道金光隐入$N的刀体，不见了！\n" NOR,me);
	message_vision(HIG "$N的刀的等级提高了！\n" NOR, me);
	if(weapon->query("equipped") ) 
		weapon->unequip();
	weapon->add("level",1);
	weapon->set_desc();
	weapon->init_blade(weapon->query("level")*10);
	weapon->save();
	weapon->restore();
	return 1;
}
