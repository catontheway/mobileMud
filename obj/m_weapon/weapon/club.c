...
void create()
{
    ...
    set_desc();
	set("rigidity",500);
	set("wield_msg", "$N往腰中一摸，拿出了一把$n端在手中。\n");
	set("unwield_msg", "$N手中$n抖出一个棍花，眨眼间已然不见影踪。\n");
	
	init_club(this_object()->query("level")*10);
	setup();
}
void init()
{
...
int do_duanlian(string weapon_id)
{
	...
	message_vision(HIY "棍身忽的一亮，一道金光隐入$N的棍体，不见了！\n" NOR,me);
	message_vision(HIG "$N的棍的等级提高了！\n" NOR, me);
	if(weapon->query("equipped") ) 
		weapon->unequip();
	weapon->add("level",1);
	weapon->set_desc();
	weapon->init_club(weapon->query("level")*10);
	weapon->save();
	weapon->restore();
	return 1;
}
