...
varargs int skill_power(object ob, string skill, int usage)
{
	...
	level = ob->query_skill(skill);//有效等级
...
//		level += level*(ratio-50)/150;
	...
	case SKILL_USAGE_DEFENSE:
		...
		if (ob->is_fighting())
			//dodge 100的话就翻倍，是不是太高了？///所以武功的设定一定要合理-----Vast
...
*****************************************************************************/
    ...
    //此算法是否合理，值得深入研究,应该有必要加入先天或者后天或者其他条件对ap,dp,pp的影响
    power = power + (int)ob->query("combat_exp")/300;//经验是否应该作为一个计算条件？是不是应该削弱？
...
//避免溢出规定上限。mudy
	if (usage == SKILL_USAGE_ATTACK)
	{
		...
		//可不可以这样考虑，先天属性的影响是后天的5倍： value=(int)power * (ob->query("dex") + (ob->query_dex()- ob->query("dex"))/5)
...
{       object where,where2;
	...
	if( userp(victim) ) {
     write_file("/log/static/kill_player",sprintf("%s(%s) killed by %s(%s) on %s\n",
		...
		if(killer->query("pker")&&killer->query("deathname")==victim->query("id")&&!killer->query("accomplish1")&&!killer->query("accomplish2")
		&&!where->query("no_die")&&!where2->query("no_die"))
	   	{
        	...
        	if (userp(killer) && (killer->query_temp("killing") == victim->query("id")) && (victim->query_leader() != killer) )
        	{
                ...
                if (!environment(victim)->query("no_die") ) victim->clear_condition();
		...
		victim->delete("vendetta");
            	...
            	if (environment(victim)->query("no_die"))
                	killer->add("PKS",-1);
		if( (int)victim->query("mud_age") > 86400 && !environment(victim)->query("no_die"))
		{
		...
		bls = 10;
                if( killer->query_temp("apply/name") )
                {
                        CHANNEL_D->do_channel(this_object(), "rumor",
                        	sprintf("%s被" + killer->query_temp("apply/name")[0] + "就地正法了。",victim->name(1)));
		...
		// End of apprendence
                if (!killer->query_temp("apply/name"))
			CHANNEL_D->do_channel(this_object(), "rumor",
				sprintf("%s被%s就地正法了。", victim->name(1), killer->name(1)));
...
//取消此限制	//战斗方式最大可以达到的潜能
	...
	if (!prepare) prepare = ([]);
	if( objectp(weapon) )	attack_skill = weapon->query("skill_type");//兵器功夫
	else if ( sizeof(prepare) == 0)	attack_skill = "unarmed";
	else if ( sizeof(prepare) == 1)	attack_skill = (keys(prepare))[0];
	else if ( sizeof(prepare) == 2)	attack_skill = (keys(prepare))[me->query_temp("action_flag")];//空手功夫互备
	...
	ap = skill_power(me, attack_skill, SKILL_USAGE_ATTACK);// my ap
	//经过修改后skill_power()返回的ap已经不可能小于1，但仍保留此句，dp,pp同理
	if( ap < 1) ap = 1;
	if( victim->query_temp("combat_factor") )
		ap = to_int(ap * victim->query_temp("combat_factor"));//my ap修正

	if (userp(me) && intp(action["dodge"]))
		me->set_temp("fight/dodge", action["dodge"]);//武功文件设定中的dodge项

	dp = skill_power(victim, "dodge", SKILL_USAGE_DEFENSE);//enemy dp
	if( dp < 1 ) dp = 1;
//取消此限制	//计算expchance
...
//	if (expchance >= ExpTopChance) expchance = ExpTopChance - 1;
	...
	//     or be hit.
	//
	if( random(ap + dp) < dp ) { 		// Does the victim dodge this hit?//躲闪成功
...
//		if( (ap < dp) && random(ExpTopChance) >= expchance && !userp(me) )
		...
		if( victim->query_temp("weapon") ) {
			...
			if( !weapon ) pp *= 2;//攻击方空手，则对手的pp翻倍(空手太吃亏了)
		} else {
			//攻击方有武器而对手空手，则对手pp减为1/10(兵器很占便宜，呵呵)
			if( weapon ) pp = skill_power(victim, "parry", SKILL_USAGE_DEFENSE) / 10;
			else
			{
				pp = skill_power(victim, "parry", SKILL_USAGE_DEFENSE);//双方都空手
				pp = pp * 2 / 3;
			}
		}
		//应该考虑到武功里面的parry项对pp的影响，建议作如下修改：
		//擒拿类手法增大parry值，武器类功夫不设定
		//if( action["parry"] )
		//		pp += action["parry"] * pp / 100;(这句话放什么位置待定)

		if( victim->is_busy() ) pp /= 3;//busy时pp更低
		if( pp < 1 ) pp = 1;

		if( random(ap + pp)< pp ) {//招架成功

			//增加武器的相互碰撞损耗(武器对武器时的格挡)
			...
			//招架时的格挡信息：（编写新武功时多写点新鲜的语句:P）---另：有些功夫没有parry_msg,需要补写
       			if( !find_object(SKILL_D(parry_skill)) && file_size(SKILL_D(parry_skill)+".c") < 0)//判断是否是自创武功
			{
				if(!stringp(PLAYER_SKILL_D(parry_skill)->query_parry_msg(victim->query_temp("weapon")))) parry_skill = "parry";
				result += PLAYER_SKILL_D(parry_skill)->query_parry_msg(victim->query_temp("weapon"));
			}
        		else
        		{
        			if(!stringp(SKILL_D(parry_skill)->query_parry_msg(victim->query_temp("weapon")))) parry_skill = "parry";
...
//			if( dp < ap && random(ExpTopChance) >= expchance && (!userp(victim) || !userp(me))
		...
		//这里如果武功文件没有定义hit_ob(),好像也不会报错，foo结果为0(不太确定，我没发现有预定义的hit_ob())---Vast
		//如果是这样，那么某些特殊武功编写相应的hit_ob()是相当有必要的
			...
			//比如白驼山的大蛇定义了hit_ob()
			if( weapon ) {
		//这里如果武器文件没有定义hit_ob(),也不会报错，foo结果为0(也没有发现预定义的hit_ob())---Vast
		//那么特殊武器编写相应的hit_ob()是也是相当有必要的，^_^
			...
			//身上的盔甲要起作用哦，嘿嘿(damage修正,不是damage_bonus)
			...
			//damage和damage_bonus合成,从效果看来damage_bonus在武功设置得不是很合理时，影响比较大
			...
			// Let combat exp take effect//建议减小经验对伤害的影响
			defense_factor = your["combat_exp"];
			while( random(defense_factor) > my["combat_exp"] ) {
				damage -= damage / 3;
				defense_factor /= 2;//以后再也没有用到，不知道此处赋值意义何在？
			}
			...
			if(absorb_skill == "qiankun-danuoyi" && !me->query("no_use/qiankun-danuoyi"))
			{
	                	...
	                	jibie =(int)victim->query_skill("qiankun-danuoyi",1) + (int)victim->query_skill("jiuyang-shengong",1)/5;
                                jibie = jibie * 4 /5;
			   	...
			   	if( randnum > (int)me->query("combat_exp") )
			  	{
					...
					victim->add("neili", -qdamage/10);
	                               	qdamage=0;
					damage = 0;
					msg = RED "$n的乾坤大挪移自然激发而出，将$N的攻势反击了回去。\n" NOR;
				        message_vision(msg, me, victim);
				}
	   		       	else if( randnum > (int)me->query("combat_exp")/2 )
				{
					damage = 0;
				        msg = HIB "$n的乾坤大挪移自然激发而出，将$N的攻势移了开去。\n" NOR;
			  	        message_vision(msg, me, victim);
	                        ...
	                        {
					damage = damage/2;
				        msg = GRN "$n的乾坤大挪移自然激发而出，将$N的攻势大半移了开去。\n" NOR;
			  	        message_vision(msg, me, victim);
				}
			}
                        }
			...
			//
			//是否有必要设置一个参数以便在玩家可以不必非要使用硬功
			//硬功应该有利弊两端，低级别是自杀、高级别很强，但是很难达到
	                ...
	                if( victim->query_skill("iron-cloth") && !me->query("no_use/iron-cloth"))
			{
                        ...
                        //建议修改基本硬功对伤害的影响，并且增加特殊的硬功
                        if ((damage = damage - absorb_vic * 1.3 - random(absorb_vic) )< 0 )
			{
                        	if ( damage < 0 ) damage = 0;
			...
			{
				//
				//	(6) Inflict the damage.//伤害最终在这里造成
				//
				damage = victim->receive_damage("qi", damage, me );
				if( random(damage) > (int)victim->query_temp("apply/armor")//damage超出了armor的防御范围
					&& ( (me->is_killing(victim->query("id")))
					   && ( (!weapon) && !random(4) || weapon && !random(2) )//拼命时打伤的机率大(有兵器比空手机率大)
			    		   || ( (!weapon) && !random(7) || weapon && !random(4) ) )  )//不拼命几率小(同样有兵器比空手机率大)
				{
				// We are sure that damage is greater than victim's armor here.//这在上面已经保证了
...
//			tell_object(me,"你战胜了"+victim->query("name")+"!\n\n");
	...
	&&	victim->query_temp("guarding") ) {
		...
		if( random(my["dex"]) < 5 ) {
			...
			do_attack(victim, me, victim->query_temp("weapon"), TYPE_RIPOSTE);//这个反击的参数有什么用？---Vast
		}
	}
}
...
void fight(object me, object victim)
{
	object ob;
	object weapon;
  if( !living(me) || me->query_temp("combat_yield"))//参见cmds/imm/yield.c
	  return;
	...
	} else if( random( (int)victim->query_dex() * 3 ) > (int)me->query_str()) {//这里到底应该 > 还是 <啊?---Vast
		me->set_temp("guarding", 0);
		if( !victim->is_fighting(me) ) victim->fight_ob(me);
