...
string ask_story()
{
	...
	if( me->query_temp("zyhb/zhenjing"))
	{
		...
		tell_object(me, "你听得入神，没有说话，周伯通看你不问后来怎样，便预测气不说了。\n");
...
string ask_houlai()
{
	...
	if( me->query_temp("zyhb/story"))
	{
		...
		return "「但是这十五年的时光我也没有白费，我冥思苦想，创出一套左右和右手打架的功夫。」";
	}
	tell_object(me, "周伯通两只手正打得热火朝天，显然不会听你说话。\n");
	return "我现在正忙着呢！";
}
...
int do_draw(object me)
{
	...
	wis = me->query("int");
        ...
        if (random(100000 + (kar / 2 - wis) * 480) > 90000)
		me->add("hubo_practice", 1);
	 if ((wis - kar / 2) > 20 && !random(10) )
		tell_object(me,HIW"以你的资质似乎不太适合学习「双手互搏」，还是早早放弃为妙！\n"NOR);
