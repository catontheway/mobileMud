...
int do_heal(string arg)
{
        ...
        me = this_player();
        if (me->query_skill("medical-skill", 1) < 1 )
                return notify_fail("你针灸术都没学，去行凶啊？\n");

        if (this_object() != me->query_temp("weapon") && this_object() != me->query_temp("secondary_weapon"))
        ...
        if (! userp(ob))
        {
                if (me->query_skill("medical-skill", 1) < 60)
                {
        ...
        me->improve_skill("medical-skill", 5 + random(30));
        ...
        if (random(120) > (int)me->query_skill("medical-skill", 1))
        {
        ...
        } else
        {
                ...
                heals = me->query_skill("medical-skill", 1) + random(me->query_skill("medical-skill", 1));
