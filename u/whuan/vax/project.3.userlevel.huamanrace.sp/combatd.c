...
// 新的 skill_power 不再大概正比于 exp ，而是 level，也就是说，
// exp_user_A : exp_user_B = 1:8 的原来命中率是 1/9
// 现在 由于level = pow(exp, 0.333), 命中变为 1/3

	int user_level, skill_level, level, power, enemy_num;
	
	if( !living(ob) ) return 0;
	
	// pow(exp, 0.333) , cus NPC do not have level
	user_level = pow(ob->query("combat_exp")*10.0, 0.3333)/10;
	if (user_level > 50) user_level = 50;
	
	skill_level = ob->query_skill(skill);
	if (skill_level > 750) skill_level = 750;
	
	level = user_level*5 + skill_level;
	
	// enable apply/*** , mainly for powerup
	switch(usage)
	{
		...
		case SKILL_USAGE_DEFENSE:
			...
			if (ob->is_fighting())
				...
				// vax :: yess, 但是还没有发现特别严重的+ fight/dodge 的功夫，
				//           有可能的话，可以做成 sqrt(fight/dodge * 10)
				level = level * (100 + ob->query_temp("fight/dodge")) / 100;
			break;
	}
	
	// no skill  && exp < 100
 	if( !level ) return 1 + (int)ob->query("combat_exp") / 20;

	power = level;
	
	// power factor :: 1 . yinyang 的影响
	YINYANG_D->set_time(time());
	power += power * YINYANG_D->yinyang_factor(ob);
	
	// max power here would be expected at :: (50*5 + 750)*2*1.5 = 3000
	// min power here is 1
	// so power here is expected at [ 1, 3000]
	if (power <= 0) power = 1;		

	if (usage == SKILL_USAGE_ATTACK)
	{
		// str facilate attack
		power = power * ob->query_str() / 20;
		// power factor :: 2 . master-attack-skill
		power += power * SPECIAL_D->query_effect(ob, skill) / 100;
	}
	else
	{
		// dex facilate defence
		power = power * ob->query_dex() / 20;
		//一打多的时候降自己dp。Pal
		// 但是超过10个 按照 10个计算。 vax /2006
		enemy_num=(int)sizeof(ob->query_enemy());
		if ( enemy_num>10 ) enemy_num = 10;
		
       	power = power * 2/ (1 + enemy_num);
	}
	// here power is expected at [1, 8000]
	// while 12000[attack] = 3000 * 80(str)/20
	if (power < 1)
	{
		power = 1;
	}
	else if (power > 12000 * 2)
	{
		// double exp_value, 留出些裕度
		power = 12000 * 2;
	}
...
（此公式可能需要修改，以防溢出）
*****************************************************************************
 	if( !level ) return 1 + (int)ob->query("combat_exp") / 30;

	power = (int)level/3*level/10*level/10;
	...
	// yinyang 的影响
	YINYANG_D->set_time(time());
	power =(int)power*(1.0 + YINYANG_D->yinyang_factor(ob));
...
//取消此限制	//战斗方式最大可以达到的潜能
	...
	ap = skill_power(me, attack_skill, SKILL_USAGE_ATTACK)/2;
	...
	dp = skill_power(victim, "dodge", SKILL_USAGE_DEFENSE)/2;
	if( dp < 1 ) dp = 1;
//取消此限制	//计算expchance
...
//		if( (ap < dp) && random(ExpTopChance) >= expchance && !userp(me) ) {
		...
		if( victim->query_temp("weapon") ) {
			pp = skill_power(victim, "parry", SKILL_USAGE_DEFENSE)/2;
			if( !weapon ) pp *= 2;
		} else {
...
//			if( dp < ap && random(ExpTopChance) >= expchance && (!userp(victim) || !userp(me))
			...
			// vax :: use sqrt(exp), instead of exp
			defense_factor = pow(your["combat_exp"] * 10 ,0.5);
			while( random(defense_factor) > pow(my["combat_exp"] * 10 ,0.5) ) {
				damage -= damage / 3;
				defense_factor /= 2;
			}

			//qiankun-danuoyi's damage
			// last edit by dewolf 20051211
			...
			if(absorb_skill == "qiankun-danuoyi" && !me->query("no_use/qiankun-danuoyi"))
			{
				...
				jibie = ceil(pow(jibie*10,0.5)); // sqrt(level*10), 1000 级对应 100
				randnum = (int)random(victim->query("combat_exp"))/100*jibie;
				//qdamage = damage*(jibie/150) + jibie/2;//这个是否有些夸张？--vast
				qdamage = damage*jibie/200+random(jibie/2);	// 挪移=20% -- 50%
				if (me->query("id") == DADIZI_D->query_name("mingjiao"))
					qdamage*=2;	// 明教大弟子挪移 =40% -- 100%
				if (qdamage > 2000) qdamage = 2000; //vast暂时设置上限 , dewolf保留
			   	if( jibie>20 && randnum > random((int)me->query("combat_exp"))*2 )
			  	{
