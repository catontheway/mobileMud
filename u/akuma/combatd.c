...
//对年?湓?5岁以下的player,被杀不降skills,exp,potential
// modified by akuma@mudnow 5/13/2001
// 修改一下skill_power,因为经验太高,造成溢出的情况
//#pragma optimize all
#include <ansi.h>
#include <skill.h>
#include <weapon.h>
...
"$N被$n的小腹反弹回来的力量震得半身发麻。\n";
                else if( damage < 120 ) return
"$N被$n的反弹力一震，胸口有如受到一记重击，连退了五六步！\n";
                else if( damage < 160 ) return
"$N被$n的内力劲风一震，眼前一黑，身子向後飞出丈许！！\n";
                else return
"$N被$n的内力劲风一震，眼前一黑，狂吐鲜血，身子象断了线的风筝向後飞去！！\n";
                break;
...
varargs int skill_power(object ob, string skill, int usage)
{
	...
	power = ((level/40)*level*level + ob->query("combat_exp")/4)/30;
	//这个限制一下，要不然太夸张了......
	//原来的算式很容易溢出
	if (usage == SKILL_USAGE_ATTACK)
		return power*ob->query_str();
	else
		return power*ob->query_dex();
}
...
//                me->reset_action();
		 ...
		 if(absorb_skill == "qiankun-danuoyi")
		   ...
		   if( randnum > (int)me->query("combat_exp") )
			...
			if( (!userp(me) || !userp(victim))) {
				...
				my["combat_exp"]<2000000 ) {
                                        my["combat_exp"] += 1;
					if( my["potential"] - my["learned_points"] < 600 )
                                                my["potential"] += 1;
					me->improve_skill(attack_skill, 1);
				}
				if( random(your["max_qi"] + your["qi"]) <damage&&your["combat_exp"]<4000000  ) {
                                        your["combat_exp"] += 1;
					if( your["potential"] - your["learned_points"] < 600 )
                                                your["potential"] += 1;
				}
			}
	...
	if( damage > 0 ) {
		...
		victim->query("qi")*2 <= victim->query("max_qi")) {
			...
			if(me->query("quest/quest_type") == "胜"
			   && me->query("quest/quest") == victim->name(1)
			   && !userp(victim))
...
//			message_vision( winner_msg[random(sizeof(winner_msg))], killer, victim);
			...
			if(killer->query("quest/quest_type") == "胜"
			   && killer->query("quest/quest") == victim->name(1)
			   && !userp(victim))
			killer->set_temp("quest_finish",1);
		}
...
// End of apprendence
		...
		bls = 1;
                if(killer->query("quest/quest") == victim->name(1)
                && killer->query("quest/quest_type") == "杀" )
                        killer->set_temp("quest_finish",1);
	}
	if ( killer->query_temp("killing") == victim->query("id") )
	{
