...
string *guard_msg = ({
	CYN "$N"CYN"凝神聚气，紧紧的盯着$n"CYN"，提防$n"CYN"出冷招。\n" NOR,
        ...
        CYN "$N"CYN"目不转睛地盯著$n"CYN"的动作，寻找就地正法的最佳时机。\n" NOR,
...
void killer_reward(object killer, object victim)
{
	...
	if( userp(victim) ) {	 	
		...
		my["shen"]-=your["shen"]/ 10;
                if(userp(killer)) {
	     		...
	     		if(gain>0&&query_ip_number(killer)!=query_ip_number(victim)) {
	     			...
	     			my["combat_exp"]+=gain;
	     			my["potential"]+=gain/5;
	     			tell_object(killer,HIC"你终于就地正法了"+your["name"]+HIC"，报了昔日的一箭之仇！为此，\n"
		  		...
		  		sprintf("%s亲手就地正法了%s,报了昔日之仇！",my["name"], your["name"]));
		...
		/*任务系统*/
        	...
        	{
           		message("channel:gab", HIC"【任务频道】"+your["name"]+"被"+my["name"]+"就地正法了。\n"NOR,users());
